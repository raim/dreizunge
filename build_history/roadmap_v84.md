# Dreizunge roadmap — v84

**This is the `v84` line.** Cut from `roadmap_v83.md` at its last commit (the `v83_s` release plus
one code-free roadmap-notes commit right after it), at the user's own explicit request ("push to v84
for a fresh session") — not a milestone-completion cut, but the same shape as every prior cut for the
same reason: the `v83` line ran the whole `PLAN §7.0`/Track A migration to completion (CP1–CP5, six
point releases `v83_h`…`v83_m`), the first real script that WRITES into `lessons.json`
(`apply-cp-lessons.js`, `v83_n`, plus two real bugs found reading its actual output, `v83_o`/`v83_p`),
a distribution convenience unrelated to any of that (`install.sh`, `v83_q`, then its default model
and startup behaviour, `v83_r`/`v83_s`), and had grown long enough (4800+ lines) that a fresh cut was
due on accumulated-context grounds alone — the exact reason the `v82`→`v83` cut gave for itself.

**`roadmap_v83.md` is kept and is not superseded as a record** — the whole `v83` line's release
history (`v83_b`…`v83_s`, eighteen point releases) lives there under `# SHIPPED IN THE v83 LINE` and
was NOT copied here. Go there for how something was built or why a guard is shaped the way it is;
this file stays current through the whole v84 line.

> **⚠️ WHAT WAS CARRIED, AND WHAT WAS NOT.** Carried: this protocol block, the findings that govern
> the open sections, `§0`/`§0i` with their reconciliation, the standing RULES (now including two new
> ones earned in the `v83` line — see below), **TRACK T** (unchanged — still nothing open) and **THE
> LARGER PLAN** (the folded `implementation_plan.md`) — `PLAN §7.0`/Track A is now FULLY SHIPPED
> (CP1–5; CP6 stays a CONDITIONAL, not a queued slice) and `PLAN §12` was already shipped in the
> `v83` line, so neither needs a new number here; no new PLAN track is being handed off at this cut.
> Not carried: the `v83` release entries, and the `v83_s` SESSION_PROMPT's own "buildable now"
> backlog — that carries forward through the SESSION_PROMPT lineage instead (see
> `SESSION_PROMPT_v84_a.md`), the same way it always has; this file has never been where per-cut
> backlog lives. Nothing open was dropped — the cut is by KIND, not by judgement about what still
> matters, so no item had to be assessed to survive it. This is exactly the `v82`→`v83` cut's own
> carry/drop split, repeated verbatim — see that file's own header if the reasoning is unclear.
>
> **Also carried, informally**: two DISCUSSION-ONLY notes from the very end of the `v83` line
> (Windows installability for a non-coder; mic-based spoken-reply recording/comparison) — neither
> ruled, neither queued, both explicitly "not required for now." They stay in `roadmap_v83.md`'s own
> shipped section (search "📝 Note") rather than being promoted to a `PLAN §X` here, since promoting
> a discussion note to a plan item is itself a decision nobody has made yet.
...
Standing rules are in the "Rules earned in session 28…34" blocks, plus a new one from the `v83` line
(search "Rules earned in the v83 line") — read the **"⚠️ How the rules are NUMBERED"** note before
citing one.
...
7. A version bump to a new BASE needs its own roadmap. **This is that roadmap for `v84`.**
...
9. **A live model call needs a live test, not just a plausible prompt.** The `v82` line's own two
   biggest corrections (the `writing`-feedback model choice, `v82_e`; the advanced-furigana worked
   example, `v82_i`) were both cases where the FIRST reasonable-looking design was wrong, and only
   generating against the real model and reading the actual output caught it — an abstract
   instruction is not what moves model behaviour, a worked example or a measured comparison is. This
   PLAN §12 cut is exactly this kind of feature (a new live tutor-priming prompt): budget for at
   least one live generation per real decision point, not zero.
10. **Ask before restarting a dev server you did not start, and before deleting data you did not
    create** — this came up for real, twice, in the `v82` line (a stale server found already running
    mid-session; two now-incompatible `writing` lessons found on disk, one the user's own). Check
    `lessons.json`'s mtime and the server's reported version before touching either.
...
**The standing rules run to 37, but the numbering in this file is not continuous, and that is a
...
("Rules earned in session 34"), and again through 36–37 ("Rules earned in the v83 line", added at
this cut — a whole release line rather than one numbered session, since that is the unit this
project's cuts actually happen at now).
...
6. **Version** — bump `APP_VERSION` in `server.js` if it's a new release. NOTE (v49): the static
   ...
   **Point releases use an alphabetic suffix** (user, v70): the base cut is the bare number and is
   implicitly `a`, so the sequence is `v77` → `v77_b` → `v77_c` → … — the same convention the v69–v83
   lines ran. **This is the `v84` line.** Roadmaps are per BASE version, so point
   releases do not each get one — this file stays current through the whole v84 line.
...
**Shipped by: Claude Code**, closing the `v83_s`-era discussion-only note ("Windows installability for
a non-coder," `roadmap_v83.md`) at the user's own explicit direction ("what's next, back to the
installation scripts?" → asked which of the two laid-out tiers, the user picked **Tier 1**).

`install.sh` (`README.md`'s "Option A") is POSIX `sh` — Windows has no `sh` by default, and there is no
one-line installer for it. Tier 1, per that note's own framing, needed no new code: both hard
dependencies already ship real native Windows installers (Ollama's own `.exe`, Node's own `.msi`,
both double-click/no-terminal), and the repo itself needs no `git` — GitHub's "Download ZIP" works.
Added a `## Windows` section to `README.md` (between Option C and "Open in browser"): install
Ollama + Node via their own installers, download the repo as a ZIP, open a terminal in that folder
(Windows 11's right-click "Open in Terminal" vs. Windows 10's Shift+right-click "Open PowerShell
window here" — the one genuinely OS-specific step), then the same `ollama pull`/`node server.js`
commands Option B already documents for Linux/macOS. Recommends the lighter `qwen2.5:7b` by default
(a non-coder audience is less likely to have the RAM/disk for the ~20+ GB `qwen3.6:35b-a3b`), pointing
back at Option A for the bigger model if their machine can take it.

`install.sh` line 16 already said "read [README.md] if you would rather do any of this by hand or
need Windows/WSL notes" — this section is what that comment was pointing at; it existed as a forward
reference before the content did.

⚠️ **Not tested on an actual Windows machine — reasoned, not measured, and says so in the README
itself.** Unlike `install.sh` (verified with real end-to-end runs against a real Linux box before
shipping), there is no Windows box in this environment to verify against; the first real Windows run
belongs to the user. `test/unit-install-script.test.js`'s existing README/`install.sh` cross-checks
(the `qwen2.5:7b` alternative must appear in both) still pass — this section didn't touch anything
those guards watch, and re-running confirmed it.

**Tier 2** (`winget`+PowerShell, a real one-click installer) remains discussion-only, not queued — the
user picked Tier 1 specifically; see `roadmap_v83.md`'s own note for what Tier 2 would involve if ever
asked for.

---

### `v84_m` — interim speech results (fewer beeps, live text) and a universal floating "heard" pill

**Shipped by: Claude Code, on two direct follow-ups from real device use, straight after `v84_l`
landed**: *"there are still a lot of beeps from the speech recognition; suppress them all. the
currently recognized word should be shown in a floating pill."*

**1. Fewer beeps — `interimResults` turned on.** `v84_l` already collapsed per-phrase restarts into
ONE `continuous:true` session per question, but the user reported it wasn't enough — Android's speech
service is widely reported to fire its silence timeout more readily when interim results are off (the
engine has nothing to report until a phrase fully finalizes, so a pause that a live partial transcript
would have kept "alive" instead reads as dead air and ends the underlying platform session — an
`onend` this app can't tell apart from a genuine pause, so it correctly, faithfully resumes, incurring
another audible restart). Turning `interimResults` on is the standard documented mitigation. Full
suppression remains impossible — nothing in the Web Speech API lets a page silence the tone itself,
confirmed again this release, not just carried forward as a prior finding — so this is a REDUCTION,
not an elimination, and said so plainly rather than overclaiming "suppress them all" was fully done.
A same-config session-reuse optimization (skip the stop/restart entirely between two consecutive
same-language questions) was designed, then DELIBERATELY REJECTED after it was found to silently
defeat `v84_k`'s own mutation-tested stale-generation guarantee — see "Rejected" below.

**2. The currently recognized word, in a floating pill.** `#mic-heard-pill` now floats above the
persistent bottom-bar mic (`#speech-mic-wrap` gives it a `position:relative` anchor, the same pattern
`#tutor-fab`/`#tutor-fab-badge` already used). With `interimResults` on, `_speechListenSession`'s
`onresult` now distinguishes `r.isFinal`: a not-yet-final result calls a new `onInterim(text)` — shown
live, neutral grey, never self-clearing (the next event naturally supersedes it); a final result still
calls the unchanged `onPhrase(alts)` — on a MATCH the pill turns green and stays (the round is about to
advance on its own); on a MISS it turns red-ish and self-clears after `_MIC_HEARD_CLEAR_MS` (2.5s,
unchanged from `v84_i`'s original framing). This is now UNIVERSAL — `_speechHandlePhrase`'s 'type' and
'syn' branches populate it too, not just 'mcq' — replacing the old per-MCQ-only `#mcq-mic-heard` span
(`cGrid`'s `speakable` parameter, which existed only to drive that span, was removed from the function
and all 11 call sites rather than left as a dead parameter once nothing consumed it any more).

**Rejected: reusing an open session across two consecutive same-language questions.** On the surface
this looked like a second legitimate way to cut beeps — if question A and question B both want a
`type`-kind session at the same resolved locale, why tear the recognizer down and pay another
start/stop tone for B? `_speechHandlePhrase`/`_speechHandleInterim` already re-read `APP.cur` fresh on
every call, so it even looked SAFE. It is not: `test/unit-speech-recognition.test.js`'s stale-generation
test (`v84_k`'s own mutation-tested fix) depends specifically on `_speechStopListening()`'s `.stop()`
call CANCELLING the old question's still-pending recognized phrase before it can ever reach the new
question's DOM — that cancellation, not the generation counter alone, is what guarantees a question
change fully discards whatever the mic was still resolving. Skipping the stop to save one tone would
have silently reopened exactly the class of bug `v84_k` closed: a slow, late-arriving word from a
question the learner already left, landing in the next one's answer field. Implemented, mutation-
tested against, caught the regression, and reverted before this release shipped — worth recording so a
future session doesn't reach for the same "obviously safe" optimization without re-deriving why it
isn't.

**Testing**: `test/unit-speech-recognition.test.js`'s mock now models `isFinal` on results (the real
API's own shape) and gained an `{interim:'text'}` step type. Existing tests needed no logic changes —
only the mock fix — proving the interim/final split didn't alter scoring behaviour, which the test
suite itself demonstrated by initially failing on EVERY match/mismatch assertion the moment `isFinal`
was introduced without a corresponding mock update (every mocked "phrase" was being read as an interim
by the new code, since the old mock never set `isFinal` at all) — a real, if self-inflicted, breakage
caught immediately by the green→red baseline check, not shipped and found later. Three new tests cover
the pill directly: an interim result shows live and never arms the self-clear timer; the SAME
stale-generation guard that protects `_speechHandlePhrase` also protects `_speechHandleInterim`
(mutation-tested by removing the guard — the test failed exactly as expected); a final miss shows
styled distinctly and DOES arm the timer. All mutation-tested (a false version of each assertion made
to fail, then reverted) plus the two-line reasoning about the rejected reuse optimization above was
itself proven, not assumed, by implementing it and watching the existing stale-generation test go red.

⚠️ Not live-verified on a real device — proven against a mocked `SpeechRecognition` constructor
(interim/final dispatch, pill styling/timers) plus one throwaway-port desktop-browser check of the
pill's actual computed style and bounding-box position (floats correctly above the mic button, all
three visual states — interim/match/miss — render with the intended colours). Whether this measurably
reduces beeps on Android, and whether the floating pill is legible/positioned well on a real phone
screen, both still need the user's own hardware.

---

### `v84_l` — continuous listening (far fewer beeps), a mute toggle, and a listening animation

**Shipped by: Claude Code, on THREE direct user follow-ups, landing together** (the third arrived
mid-session, before the first two ever reached `main`): *"please suppress all (or most, if you think
some are important) sound effects (beeps) from the new speech recognition system. also we want it to
be active all the time, except the microphone icon is pressed to mute input."* Then, separately:
*"when the speech recognition is active, can we have a permanent animation (a wave or dots) that is
animated when speech is actively recognized/interpreted by the system, and still if not?"*

**1. Fewer beeps.** Nothing in the Web Speech API lets a page suppress the audible tone Android Chrome
plays on every `SpeechRecognition.start()` — that control simply does not exist at the JS level. The
`v84_g`…`v84_k` design listened ONCE then restarted after EVERY single result (a mismatch, plain
silence, anything) — for a continuously-listening feature, that meant a beep roughly every few
seconds for as long as a speakable question was on screen. The fix: `rec.continuous = true`
(`_speechListenSession`, replacing the one-shot `_speechRecognizeOnce`). ONE recognizer instance now
stays open across MANY spoken phrases within a question, firing a phrase callback repeatedly without
ever stopping/restarting the underlying audio session — the tone now plays once per QUESTION (or once
per resume from mute), not once per phrase. A `'type'`/`'mcq'` MATCH still explicitly stops the
session (the correct-answer TTS reveal is about to play, and a still-open mic risks hearing its own
voice); `'syn'` never stops on a match, since `syn_select` genuinely wants MORE words next. The
browser's own silence timeout still exists even in continuous mode — when it fires with no error, the
session resumes automatically (still "always listening"); a genuine hard error (permission denied, no
mic, offline) stops for good, toasted once.

**2. The pill is now a MUTE TOGGLE, not a "retry."** With continuous listening already covering
"try again," the old click-to-retry meaning was redundant — the user's own framing directly replaces
it: a tap now flips `APP.micMuted` (mirroring `APP.muted`'s own not-persisted-across-loads shape).
Muted stops the session immediately and shows a distinct red "muted" pill state (vs. the blue
"active" one); unmuting resumes listening for whatever's currently on screen via the SAME
`_speechMicRefresh()` every render already calls. Muted is a STANDING preference — surviving a render
to a different speakable question, not reset per-question, matching "except the microphone icon is
pressed to mute input" (not "except for this one question").

**3. A listening animation.** The Web Speech API exposes no "hearing something right now" signal
(`interimResults` stays off — see `v84_g`'s own reasoning for why), so there is no honest way to make
an indicator react to actual speech; the user's own "and still if not" confirms that's fine — one
continuous animation for as long as the session is genuinely open, not an attempt to fake real-time
audio reactivity. Three small dots (`.mic-dots`, CSS `@keyframes mic-dot-bounce`, staggered
`animation-delay`) replace the 🎤 glyph while `.listening` is present — the SAME class the pill
already carried, added/removed by the exact session lifecycle `_speechStartSession`/
`_speechListenSession` already owned; no new state was needed, only a new thing to show for it.

**Two real bugs found by this session's OWN mutation-testing and live rendering, neither reported by
the user:**
- **An ordering hazard.** `_speechStopListening()` was originally called BEFORE `check()`/
  `pickChoice()` on a match. Stopping a session can fire its `onend` event SYNCHRONOUSLY (confirmed in
  the test mock; real browsers vary but the hazard is real either way) — and `onend`'s own "should I
  resume?" logic reads `APP.cur.answered` to decide. Stop-then-check meant that read happened before
  `check()` had set `answered`, so the just-answered question's session would resume into a SECOND,
  entirely superfluous session instead of staying stopped. Caught immediately by the very first
  mutation-test of the new continuous-session code (an assertion expecting exactly one `.start()` call
  got two) — fixed by reordering to check-then-stop, documented inline so a future edit doesn't
  silently reintroduce it.
- **An inline-style specificity bug spanning `v84_k` AND `v84_l`, invisible to every test written so
  far.** The pill's `background`/`border`/`opacity` had been set in its own inline `style=""` since
  `v84_k` — and an inline style always wins over a stylesheet rule regardless of class specificity, so
  `.active`/`.listening`/`.muted`'s colour and opacity changes had been complete no-ops the ENTIRE
  time; only the dots-vs-icon `display` swap (built in THIS release) ever visibly worked, because
  nothing inline was fighting it. `test/lib-dom.js` cannot catch this class of bug at all — it does
  not implement real CSS cascade/specificity — so it was invisible to every one of this arc's
  mutation-tested unit tests across five releases. Found only by actually starting a throwaway server
  on a spare port (the running server was reading the MAIN checkout, not this uncommitted work — a
  second confirmation of the `v84_g`-era branch/`main` lesson) and reading the pill's real COMPUTED
  style in a live browser, not assumed correct because the rules existed and the tests were green.
  Fixed by moving the default look into a base `#speech-mic-pill{}` stylesheet rule and keeping the
  state rules as compound `#speech-mic-pill.xxx` selectors, which out-specificity it.

**Testing**: `test/unit-speech-recognition.test.js` rewritten again around the continuous-session
model — a new mock (`mockSessions`) scripts what EACH successive `.start()` call does (not just what
happens within one), letting tests assert on SESSION COUNT directly (`window.__micStarts`/`__micStops`
/`__micAborts`), which is the only way to actually prove "fewer beeps" rather than just "still
correct." Covers: one session absorbing several phrases with zero restarts; a `'type'`/`'mcq'` match
stopping the session vs. a `'syn'` match NOT stopping it; natural (soft) session-end auto-resuming;
a hard error stopping for good; mute stopping immediately and unmute resuming; muted surviving a
render; the stale-generation guard re-verified under the new long-lived-session shape; and a SOURCE-
level guard (the one claim in this file lib-dom genuinely cannot observe by rendering) pinning that
the inline style never re-gains `background`/`border`/`opacity` and that the state rules stay
compound selectors. Mutation-tested throughout, including both bugs above and the new CSS guard
itself. **A second process lesson, same shape as `v84_k`'s own**: two more test blocks (mute/unmute,
and the natural-session-end resume) each left an orphaned, still-listening session with a pending
timer that would have auto-resumed forever once fired — caught only because the test PROCESS itself
hung well past when all assertions had already printed "OK," not because any assertion failed. Fixed
by giving every such mock a way to actually terminate, the same discipline `v84_k` had to learn once
already.

**Testing overall**: no new `test/run.js` entries. `docs/index.html` rebuilt. No `ui.json` change.
...
**Shipped by: Claude Code, on direct user follow-up** to `v84_h`, after live-testing on a real
Android phone: *"currently the microphone is only shown for MCQ where the source language is shown
(which is ok), but not for MCQ where the target language is shown (where it makes more sense). also
we do want a field next to the microphone that shows the last spoken phrase/word, and clear words
with some timeout if they don't match, and turn green as soon as speech input matches an answer."*
Two things, both real gaps rather than scope changes:

**1. `mcq_source_target` (`tMcqEI`) was missed at `v84_h`, not deliberately excluded.** It is a very
common vocabulary MCQ — shown the SOURCE word, choices in the TARGET language (`mkMcqEI`'s own
`choices:shuffle([v.target,...w])`) — but its render function is SHARED with the script-primer
`_intro` glyph/chip items, and `v84_h`'s audit only read those `_intro` branches closely, reasoning
by association that the whole function was glyph-related. It was not: the function's DEFAULT
(non-`_intro`) branch is the ordinary vocabulary case, and its choices are genuinely target-language
text — exactly the case the user named as "makes more sense." Now passes `speakable:'target'` to
`cGrid`, same as `mcq_article`/`mcq_plural`/`mcq_conjugation`; its own `_intro` branch (unaffected,
still the glyph/chip case) stays unwired, same reasoning as `tLMcq`'s equivalent branch already had.

**2. The MCQ mic now shows what it heard — a direct reversal of `v84_g`'s original "never shown"
design**, once the user explained why: seeing the transcript is worth more than hiding it, especially
now that BOTH directions (target-choice and source-choice MCQ) have a mic. `#mcq-mic-heard`
(`_micShowHeard`, a small label next to `#mcq-mic-btn`) shows the raw transcript on every result: a
MATCH turns it green and leaves it standing (the card is about to advance on its own); anything else
shows the mismatched word too, then self-clears after `_MIC_HEARD_CLEAR_MS` (2.5s) — the user's own
"clear words with some timeout if they don't match." The clear timer lives ON the element itself, so
a second recognition pass started before the first one's timer fires cleanly supersedes it rather
than racing it. The CHOICE-tapping behaviour is unchanged: a match still taps the correct button via
the real `pickChoice` path (green, advances); a miss still changes nothing on the card itself — only
the new heard-label reacts to a miss, never a heart or a wrong selection.

**Testing**: `test/unit-speech-recognition.test.js` — the scoping section now covers `tMcqEI` in both
directions (its default branch DOES get `'target'`, its own `_intro` branch still does NOT), and two
new behavioural sections drive a real MCQ speech match/mismatch through `#mcq-mic-heard` — a match
shows the heard word AND turns the label green; a mismatch shows the heard word WITHOUT the green
class and then genuinely waits past `_MIC_HEARD_CLEAR_MS` (a real ~2.7s wait, not simulated) to
confirm it actually self-clears. Mutation-tested: reverting `tMcqEI`'s own `speakable` wiring, the
clear-timer scheduling, and the `match` class toggle each independently turned the corresponding new
assertion red.

**Testing overall**: no new `test/run.js` entries — baseline stays 267/233. `docs/index.html`
...
**Shipped by: Claude Code, on user request** — TWO features, landing together (same conversation:
the user asked a feasibility question about voice recognition, got pointed at the discussion-only
"recording a spoken reply" note in `roadmap_v83.md`, then asked for BOTH this and a second, unrelated
follow-up in one message).

**1. Speech recognition, browser-native, reused for answer checking.** The user's own framing —
*"exploit the voice recognition capabilities of the phone, in the correct target language"* — is
exactly the "browser API" option the `roadmap_v83.md` note had already scoped as the simplest of
three (recording is trivial; local Whisper-class STT is a real new infra decision; pronunciation-
quality scoring is out of scope). Chosen explicitly over standing up a local model, with the
trade-off named plainly in the code: inconsistent support (weak/absent in Safari/Firefox), and on
Chrome the audio goes to Google's own servers — not local, unlike this app's Ollama story
generation.
- **Mechanism** (`_speechRecognizeOnce`, `test/lib-dom.js`-independent, pure client code): one
  recognition pass in the SAME locale `_speechLocaleFor` already hands TTS for the open lesson —
  the app's existing per-chapter/storyline speech-locale resolver, reused unchanged. Recognized text
  is compared to data the app ALREADY carries (`ex.correct`), the identical `normDiacritics`
  forgiving-compare the typed-answer path has used since long before this release — no new
  comparison logic, matching the roadmap note's own framing exactly.
- **Typed-answer exercises FIRST** (`listen_type`/`type_plural`/`type_conjugation`, the user's own
  "first only when a lesson asks to type the word") — a mic button next to `#type-in`. A MATCH fills
  the canonical answer and checks it immediately (green, advance, same as a correct typed Enter). A
  MISS fills the input with what was actually HEARD, so the learner can see and correct it, but
  never auto-submits — a misheard word must never silently spend a heart the learner didn't actually
  get wrong.
- **Then MCQ, but deliberately NOT every MCQ** (the user's "we can apply it any MCQ as well") — a
  mic button was threaded through `cGrid`'s new `speakable` parameter, but only wired at the THREE
  call sites whose `choices` are confirmed TARGET-language text: `mcq_article`, `mcq_plural`,
  `mcq_conjugation` (verified by reading the exercise builders, not guessed from the type names —
  `mkMcqIE`/`mkLMcq`'s `choices` are `v.source`-based, i.e. SOURCE-language translations, and
  `comprehension_mcq` is in `SOURCE_LANGUAGE_TYPES` server-side; recognizing target-locale speech
  against source-language text would just never match, so those three types were deliberately left
  unwired rather than shipping a mic button that silently never works). A match taps the CORRECT
  choice via the real `pickChoice` path (green, advances); a miss changes NOTHING — never auto-
  selects a wrong choice on a misheard word, same non-punitive rule as the typed path.
- **Feature-detected, gracefully absent**: no mic button renders at all without
  `window.SpeechRecognition`/`webkitSpeechRecognition` — Safari/Firefox learners see the exact same
  UI as before this release.
- **Testing**: `test/unit-speech-recognition.test.js` — feature detection (both directions), the
  MCQ scoping claim (source-checked against the actual `mkMcqIE`/`tComprehension` bodies, not just
  asserted), a matching AND a mismatched typed answer, a matching AND a mismatched MCQ speech
  answer, and that a genuine recognition error toasts once without a later "didn't catch that" toast
  silently overwriting it (an earlier draft had exactly that bug — a null `alts` from a real error
  and a null `alts` from plain silence were treated identically, so the caller's own no-match toast
  clobbered the more specific error message; fixed by having the caller do nothing at all on a null
  result, since `_micListen` already reported anything that needed reporting). Mutation-tested: the
  MCQ-scoping guard, the correctness-match logic, and the error-toast-priority fix were each broken
  in turn and confirmed to turn the corresponding assertion red.
- **Live verification not possible in this environment**: this container's sandboxed preview browser
  cannot grant real microphone access, so there is no live human-voice pass — see the `⚠️ OWED BY
  THE USER` note this release adds to the session prompt.

**2. A "reply ready" speech-bubble badge on the tutor fab.** Raised by a direct question — *"when I
send a grammar/meaning request to the tutor... is it still running when I close the tutor popup with
×?"* — answered by reading `_tutorSend`/`refreshTutorAvailability`: closing is a pure CSS toggle,
nothing aborts the in-flight `fetch`/stream, and the reply lands into `_tutorState.history` regardless
of whether the widget is open to show it. The gap that answer surfaced: nothing signalled a reply had
landed while the learner wasn't looking, so the user asked for exactly that sign.
- **`_tutorNoteReplyLanded()`** fires at all three places a reply actually lands (the JSON-path push,
  the stream's `done` frame, and the stream-cut-short fallback) and no-ops if the widget is currently
  open (the learner already saw it arrive live). **`_tutorClearUnread()`** fires ONLY when
  `toggleTutorWidget()` reopens the widget — never on a timer, a screen change, or anything else, so
  the badge stays up for exactly as long as the reply genuinely went unseen.
- The badge itself (`#tutor-fab-badge`) is real markup inside `#tutor-fab`, a small pill reading
  "🦉 I have an answer!" (`ui.json`'s `tutor.reply_ready`) positioned above the fab; tapping the badge
  itself reopens the tutor, same as the fab button.
- **Testing**: `test/unit-tutor-reply-badge.test.js` — the markup (badge inside `#tutor-fab`, starts
  hidden, reopens on tap), a reply landing OPEN never raises it, a reply landing CLOSED raises it AND
  still saves the reply into the thread, and reopening — and ONLY reopening — clears both the badge
  and `_tutorState.unread`. Mutation-tested: removing either the note-landed or clear-unread call
  turns the corresponding assertion red.

**Testing overall**: baseline moved from 265/231 to **267/233** (two new unit tests, both outside the
`--quick`-excluded e2e block). `docs/index.html` rebuilt. `ui.json` gained 4 `en`-only keys
(`ex.mic_tooltip`, `ex.mic_no_match`, `ex.mic_error`, `tutor.reply_ready`) — 659→663.
