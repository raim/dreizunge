...
> **STATE AT HANDOFF: CUT AS `v65`; suite 116 green; check-inline 0 on both builds.**
> v65 adds **learner accounts + server-side state**: progress, learned ledger and tutor thread now
> follow a signed-in learner between devices. See `v65_session1_notes.md`.
> ⚠️ **Deployment note:** passwords are hashed (scrypt) but transport is NOT encrypted. Put TLS in
> front of this before exposing it beyond localhost/LAN.

**v64 shipped** (streamed tutor replies). This file supersedes `roadmap_v64.md`.

## ✅ v65 — learner accounts + server-side state (SHIPPED)
- Own store `learners.json` — **never** lessons.json, which build-static publishes to GitHub Pages.
- scrypt + per-user salt, timing-safe compare, HttpOnly/SameSite cookies, session tokens hashed at
  rest, no account enumeration, login throttling, no password logging.
- Server wins on load; localStorage stays the offline fallback; first login with an empty server
  copy MIGRATES this device's history up rather than wiping it.
- `GET /api/learners` teacher summary: chapters, words, last seen, hardest words by wrong-count.
- Guarded behaviourally by `unit-learners`.

## 🔭 What's left
- **Teacher dashboard UI** — the summary endpoint exists; nothing renders it yet. Small, high value.
...
- TLS guidance / a warning banner when accounts are used over plain HTTP on a non-loopback host.
