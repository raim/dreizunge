# Session prompt — written at the `v79_k` cut (end of session 34)
...
zero-dependency Node language-learning app). Fresh session picking up from the **`v79_k`** cut.
...
```
node test/run.js                          → expect 214 checks
node test/run.js --quick                  → expect 190
...
**2. Four things need a LIVE pass, and only I can do them.**
