...
## 7d. `v73_k` — a lesson type nothing checks was being stamped QC-clean

From the user's question "are word form and synonym lessons covered by the automatic QC?". They are —
`word_forms` → `qcCheckCloze` over `items`, `synonyms` → `qcCheckSynonymSet` over `words`, and the
corpus agrees (77 synonyms lessons: 11 stamped, 17 flagged; 61 word_forms: 14 stamped, 4 flagged).
Asking the question surfaced a different type that is not.

**`comprehension` carries `questions`, not `vocab`/`sentences`**, so it fell through to the generic
scan, which iterates exactly those two keys and found nothing. `_lessonQcRan` stayed `true`,
`flagged === _flaggedBefore` held trivially, and the lesson was **stamped clean having been examined
by nothing**.

The stamp is not cosmetic. `_runQc` skips any lesson already stamped by the same model:

```js
if (lessonIdx === null && !onlyFlagged && !force && ls.qcAt && ls.qcBy === OLLAMA_QC_MODEL) { skipped++; continue; }
```

So a comprehension lesson was marked QC-clean **permanently**, unread. Seen in the wild: the
comprehension lesson of `Churros und Chaos` carries `qcAt` and `qcBy: translategemma:12b` from the
user's run, with no checker having touched it. 1 of 28 comprehension lessons is affected today —
low only because most predate the last sweep.

Fixed by adding `comprehension` to the explicitly-out-of-scope list, so it reports "not checked"
rather than "checked and clean". **This makes a gap honest; it is not a judgement that the type is
unqualifiable.** A real checker belongs here — "does each question follow from the story, are its
distractors defensible?" is precisely what QC is for, and it is the one part of `v71_l` that no
headless test can reach. It needs a new prompt and a live model, so it is queued rather than guessed.

The new assertion covers **all six** out-of-scope types rather than comprehension alone, so the next
type added to (or forgotten from) that list is caught by the same guarantee. Revert-verified on two
of them.

### Also in `v73_k`: three items recovered from a roadmap boundary

The user asked whether an old "storyline-wide / chapter-wide QC menu" TODO still existed. It did —
and an entire `[OPEN — cosmetics deferred in v71_q]` block of three items had vanished when
`roadmap_v72.md` was cut from `roadmap_v71.md`, appearing zero times in v72, v73, `HANDOVER.md`, or
in the `future_development.md` written earlier this same session. All three re-verified as still
open and restored to `roadmap_v73.md`.

**The scan gap is the transferable part.** `future_development.md` grepped for unchecked checkboxes
and read the topical idea documents. It did not walk `[OPEN — …]` / `[QUEUE — …]` blocks inside
superseded roadmaps — prose, no checkbox, and exactly where a deferred item dies, because the next
base cut rewrites the roadmap. The idea documents are safe; nobody deletes them. **The losses are at
the base-version boundaries**, and only the v71 → v72 boundary of 28 roadmaps has been checked.
