...
> **STATE AT HANDOFF: CUT AS `v61`; suite 113 green; check-inline 0 on both builds.**
> v61 adds **Stage 1 of the tutor arc**: an end-of-chapter comprehension chatbot that focuses on the
> words the learner got wrong. See `v61_session1_notes.md` for the full design rationale.
> **i18n owed:** ~35 en-only keys accumulated across v57–v61 (all mirrored as placeholders) — one
> `translate-ui.js` run on a warm Ollama box clears them.

**v60 shipped** (learner UI + v60.1–v60.8 amendments — see `v60_session1_notes.md`). This file
supersedes `roadmap_v60.md` (now historical) and carries the protocol plus every open item forward.

## ✅ v61 — end-of-chapter tutor (SHIPPED)
- Own **tutor model role** (defaults to the story model), runtime-switchable, with its own reasoning
  checkbox reusing v60.7's `thinkOpts` (budget ×2.5, timeout ×3 when on).
- **`POST /api/tutor`** — stateless; the client sends the chapter story + wrong/known words + the
  conversation; the server returns one reply. All untrusted input capped; history flattened into the
  single user message (no change to the llm choke point).
- **Error-focused prompt**: one question at a time, weaves in the learner's wrong words, bounded to
  known vocabulary, prefers hints over answers.
- **🦉 panel on the completion card**: backend + story required, never on drills, resets per chapter.
- Guarded by `unit-tutor`; `unit-reasoning-toggle` extended to the third role.

## 🔭 The tutor arc (decided, not yet built)
Architecture review of `concept_graph.md` + `chatbot.md` found ~60–70% of the needed learner-state
substrate already exists (`_learnedLedger` with wrong-counts, progress, coverage, story summaries).
Therefore the tutor was **decoupled from the concept graph**:
- **Stage 2 — persistent assistant.** Same endpoint generalized; reachable from the main page and
  storyline pages, and ideally per-question. Needs a RAG *selection* strategy over all completed
  stories (keyword/recency over existing stores — no vector DB, zero-dep constraint). Likely wants
  server-side learner state for cross-device, and streamed replies for UX.
- **Stage 3 — concept graph (optional).** Concept ontology + `teaches:`/`prerequisites:` annotations
  on every lesson, mastery over concepts instead of chapters. Enables rigorous spoiler avoidance and
  cross-domain links. Real authoring/QC project; the tutor works without it and gets smarter with it.
