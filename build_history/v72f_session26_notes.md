...
Shipped: `v72_a` … `v72_f`, then the `v73` cut. `APP_VERSION` = `v73`.
...
## 9. `v72_f` — story context and `num_ctx`, audited then fixed

User asked two questions in sequence: do we cap the story anywhere else, and where do we pass the
current chapter versus the whole chain. Both were audits first. **My first answer to the first
question was wrong and the audit for the second one caught it.**

### What the audit found

**Size.** Five caps existed and they were inconsistent: a 1200-char slice on vocab, 300/400/600-char
per-chapter excerpts for storyboard and titles, `CHAIN_STORY_CHARS = 40000` for chain assembly,
`clip(body.story, 4000)` for the tutor, and nothing at all on word-forms. Only 2 of 7 lesson calls
sized `num_ctx` — and those two, synonyms and comprehension, were exactly the two that exceeded
Ollama's ~4096 default. Correct, but by hand each time.

**Correction, found while writing the docs.** I reported "vocab caps the story at 1200 chars". That
is true of **one of `generateOneLesson`'s four prompt branches**. The table branch sends the whole
story; the two own-text branches send the whole story **and its whole translation**. The corpus has
no translations, so that branch measures as "fits" only because its input is absent — with one of
similar length it reaches ~4,700 tokens, over the default, and a translation is the entire point of
that branch. `generateErrorHunt` also embeds the whole story, inline, and asks for it back.

**Chain.** Only `generateComprehension` consumes `chainStory`. Four of its five call paths supplied
it; `generate()` did not — while calling `collectChainVocab` two lines above. So the same lesson saw
749 chars created-with-chapter and 4,139 added-afterwards: **5.5x**, for identical output.

### What shipped

- `num_ctx` sizing on **all five** full-story generators (was two)
- `generate()` collects the chain story, using the synthetic-node pattern the arc path already had
  for the same pre-persist reason
- `unit-generation-context.test.js` — new, and general rather than one-off

### The guard nearly repeated the session's own mistake

My first draft checked `prompts.json` for `{story}`, found five entries, and printed *"all accounted
for"*. server.js interpolates `${story}` inline in **five more** places — including error-hunt and
three of the four `generateOneLesson` branches. A guard seeing half the problem while reporting
completeness is worse than no guard. It now checks both and pins the inline count, so a sixth site
fails the suite.

That is the fourth vacuity slip this session and the first one I caught *before* it landed — by
writing the documentation, which forced me to enumerate what I had actually verified.

### One guard I loosened on purpose

`unit-reasoning-model-safety` pinned error-hunt's opts as a literal `{ think: useThink, timeoutMs:
... }`, so adding `ctxTokens` broke it while every requirement it protects still held. Rewritten to
assert the two properties independently. **A guard that fails on unrelated additions trains people
to edit the guard**, which is how a guard stops guarding.

### Documentation

`DOCUMENTATION.md` §3 gained "What each generator actually sees" — story context per format, and the
`neutral` / `reinforce` / `extend` vocab modes, framed as the two independent decisions they are:
*recycling words as material* versus *avoiding them as targets*. `INTERNALS.md` gained the traps:
sizing is per-call-site, `prompts.json` is half the picture, the 1200 cap covers one branch of four,
and chapter-only is now **load-bearing** for synonyms and word-forms — both validate model output
against the current chapter, so feeding them the chain would make every quote fail its own check.

---

## 10. Rulings and handover prep (end of session)

**Grammar and conjugation stay story-free — RULED.** They receive 8 extracted keywords and never the
story. Gender, article, plural and verb paradigms are dictionary properties of a word; a passage
cannot make the answer more correct. A synonym is the opposite case — its validity depends on which
sense the sentence picks out — which is why `v72_d` gave *that* generator the story and why the two
should not be generalised together. Recorded in **four** places on purpose (`DOCUMENTATION.md`,
`INTERNALS.md`, the roadmap's ready list, and an assertion in `unit-generation-context.test.js`),
because the shape looks exactly like the pre-`v72_d` bug and would otherwise be "discovered" again.

**Three things in the handover would have misled a fresh session:**

- **Decision 2's evidence no longer exists.** It described "six grammar lessons repeat a `target`".
  Those lessons left with the corpus replacement between v72 and v72_b. What is live is **2 synonyms
  lessons with a duplicate `base`**, both Arabic. Same defect class, different data — and a session
  that went looking for the six would have found nothing and concluded the item was fixed.
- Two owed-list rows overlapped (`v72_a + v72_b` vs `v71_i–v72`) and one had an empty reason cell.
- The session notes were still named `v72b_…` after five more changes shipped. Renamed to
  `v72f_session26_notes.md` and references updated.

**The next session is now specified as two parts**, `lib-dom` then cleanup, in that order — several
cleanup candidates are tests that pin source text *precisely because* they cannot read the DOM back,
so `lib-dom` decides how much of the cleanup is possible.

The cleanup list is deliberately **evidence-backed rather than aspirational**: seven items, each
naming the defect that produced it (four vacuous assertions, two guards that broke on unrelated
edits, three tests that broke on an extracted function gaining a dependency, per-call-site `num_ctx`
sizing, `_SENT_END_RE`, presentation forms, and the two undocumented subsystems). It carries one
caution: in this codebase "refactoring" has a specific failure mode — a second copy of a rule that
then drifts — and session 26 alone found two such pairs that had agreed by coincidence for releases.
**Prefer removing a second copy over reorganising code that works.**

---

## 11. `v73` — antonyms get a looser standard than synonyms

User feedback from a live run: the new synonyms look good, but there are **too few antonyms**.

`v72_e` had applied one strictness rule to both relations — *"the same standard: true opposites that
fill the SAME grammatical slot and would be substitutable in the quoted sentence."* That was wrong,
and the UI strings say why:

| exercise | question put to the learner |
|---|---|
| synonyms | `ex.syn.q_synonyms` — **"similar to {word}"** |
| antonyms | `ex.syn.q_antonyms` — **"opposite to {word}"** |

The synonym round implies the word could be *replaced*; the antonym round asks only for *contrast*.
Requiring an antonym to be substitutable in the quoted sentence was therefore **stricter than the
question being asked**, and it suppressed perfectly good opposites — a gradable pair, a directional
pair, or an opposite that needs a different construction all failed a test the exercise never
applies.

The new rule keeps the part that matters and drops the part that does not:

- **kept:** it must be a genuine opposite of the SENSE the quoted sentence establishes. This is the
  whole point of `v72_d` and must not be relaxed — an opposite of the wrong sense is still wrong.
- **kept:** same part of speech, so the options read consistently.
- **dropped:** substitutability in the quoted sentence, and fitting its grammar.
- **added:** an explicit instruction to be GENEROUS within those limits, naming gradable pairs,
  directional pairs and plain negations, with `[]` reserved for words that truly have no opposite.

Revert-verified in both directions: restoring the substitutability requirement fails, and loosening
the SYNONYM side fails too — the asymmetry is the point, and a future edit that "harmonises" the two
rules breaks the suite.

> Worth noting how this was found: not by a test, but by the user running the thing. The suite could
> confirm the prompt says what it says and that antonym-only entries survive — it has no way to
> notice that a correct rule was suppressing good content.
