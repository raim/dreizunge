# Session prompt — written at the `v79_l` cut (end of session 34)
...
zero-dependency Node language-learning app). Fresh session picking up from the **`v79_l`** cut.
...
```
node test/run.js                          → expect 215 checks
node test/run.js --quick                  → expect 191
...
- **`v79_l`** — on the Android that reads English as Nigerian: pick a variant from the new selector
