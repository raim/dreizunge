...
Cheapest first: live highlight refresh after answering; the vocab list showing only words NOT in the
story; sentences counting toward the word counter; the story on ALL progress cards; those cards using
the SAME renderer as the lesson panel (they still call `_storyParasHtml` directly — a 4th and 5th
renderer). **The sixth is NOT small: "a wrong answer should decrease the solved counter" turns the
