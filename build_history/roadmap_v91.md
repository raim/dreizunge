# Dreizunge roadmap — v91

**This is the `v91` line.** Cut from `roadmap_v90.md` at its last commit (the `v90_aa` release), at
the user's own suggestion ("we could consider a cut to v91") — the same shape as every prior cut.

The `v90` line ran **twenty-seven point releases** (`v90`…`v90_aa`). Among much else it closed: the
**suite mutation audit** (five passes, `tools/branch-mutation.js`); the **`⚠ Ollama unreachable`
flapping**, which was IPv6 loopback and never the wlan; the **i18n audit re-derived** (86 certain,
not 143); **ONE INPUT FIELD** for the generation wizard and the whole bottom-bar rework; **scraping a
story straight from a URL** including Wikipedia through its own API; and **the four user-reported
issues** it was handed — all-null text analysis, the tutor's two silent drops, phrase-level analysis
display, and the vocabulary article asymmetry.

**`roadmap_v90.md` is kept and is not superseded as a record** — the whole `v90` line's release
history (`v90`…`v90_aa`) lives there under `# ✅ SHIPPED IN THE v90 LINE` and was NOT copied here.
Go there for how something was built or why a guard is shaped the way it is; this file stays current
through the whole v91 line.

> **⚠️ WHAT WAS CARRIED, AND WHAT WAS NOT.** Carried: this protocol block, the open items that are
> genuinely still open (**each cross-checked against the `v89` AND `v90` shipped lists** — see the
> reconciliation note below), the findings that govern the open sections, the GPU and multi-user
> prioritisation sections, `§0`/`§0i` **whole**, and the standing RULES (now including the `v90`
> line's own block). **Not carried**: the `v90` line's `# ✅ SHIPPED IN THE v90 LINE` section.
>
> ⚠️⚠️ **THIS CUT WAS MADE AS A RECONCILIATION, AND THE RECONCILIATION IS THE POINT.** A cut that
> copies the open section forward buys nothing — the open section is the large half. What it found:
>
> - **Two whole sections were carried as UNBUILT after shipping.** *"ONE INPUT FIELD FOR THE
>   GENERATION WIZARD — SPEC ONLY, NO CODE"* (133 lines) shipped across `v90_s`…`v90_y`, and
>   *"SCRAPE A STORY STRAIGHT FROM A URL — MEASURED, NOT BUILT"* (192 lines) shipped at `v90_m`,
>   `v90_q` and `v90_r`. **325 lines of instructions to build things that already existed**, which a
>   session reading in order would have started on.
> - **Item `V`** read *"Still open, not started"* while `roadmap_v89.md` recorded it shipped at
>   `v89_al` — and the `v90` cut's own header already listed V as closed. The summary was
>   reconciled; the body was not.
> - **Item `E`** asked for "a LIVE reproduction with the raw model response captured"; `v89_t` did
>   exactly that, from the user's own log. This file's short list already said so — **two statements
>   about one defect, in one document, disagreeing.**
>
> ⚠️ **`§0`/`§0i` were carried WHOLE and deliberately not compressed.** They are dense with USER
> RULINGS, several marked "do not re-derive", and a compression pass over a document this size is
> how `v89` rule 1 happened. Shrinking them is its own task, with the user in the loop.
>
> ⚠️ **A STALE POINTER, INHERITED TWICE AND STILL TRUE.** **TRACK T** and **THE LARGER PLAN** live in
> `roadmap_v88.md`, not here and not in `roadmap_v89.md`/`roadmap_v90.md`, though `roadmap_v89.md`'s
> index claimed otherwise for forty releases. **`CLAUDE.md`'s own warning applies to roadmaps too: a
> pointer that names a file goes stale, so check it before trusting it.**

### What is in this file, in order

| section | what it is |
|---|---|
| **OPEN AT THE v91 CUT** | fresh, top-of-file summary of everything still genuinely open, reconciled against the `v89` AND `v90` shipped lists — then the findings that govern the open sections, then `§0` / `§0i` themselves, then the standing RULES |
| **SHIPPED IN THE v91 LINE** | this line's own release history, newest first |
| **TRACK T** — ⚠️ **in `roadmap_v88.md`, not here** | the text-focused progress card. Steps 1–4 and `§T7` all shipped in the v81 line; nothing open |
| **THE LARGER PLAN** — ⚠️ **in `roadmap_v88.md`, not here** | the folded `implementation_plan.md`, cited as `PLAN §X`. **A bare `§3` is this file's item; `PLAN §3` is Track C.** `PLAN §12`, `PLAN §7.0` Track A (CP1-5) and `PLAN §13` are fully shipped; `PLAN §7.0` CP6 is open (a CONDITIONAL, not a queued slice); `PLAN §2.4`/Track A4 is shipped as its four-milestone core plus `v85`–`v90`-line follow-ups |
...
block, **SEVENTEEN from the `v89` line**, and the **`v90` line's own block** — read the
**"⚠️ How the rules are NUMBERED"** note before citing one. ⚠️ **`v89`'s rule 1 is the one to read
first if you are about to make a large mechanical edit to a file**: a substring used to BOUND an edit
deleted 2,518 lines of this very document, and only a commit made minutes earlier saved it. The
`v87` line's rules are in `roadmap_v87.md`'s own copy.
...
7. A version bump to a new BASE needs its own roadmap. **This is that roadmap for `v91`.** ⚠️ This sentence read "for `v88`" for three whole release lines — it is version-specific and nothing guarded it; the two sentences either side of it ARE guarded (`unit-roadmap-version`) and stayed correct. Fixed at the `v91` cut.
...
test refuted it within a day.** The full measurement is in `roadmap_v90.md`'s `v90_aa` entry. What
this line inherits:

- A forced per-lesson QC over a chapter with **8 of 8** asymmetric pairs: `26 checked, 2 flagged`,
  **neither flag an asymmetry**. (A per-lesson run never takes the "already QC'd" skip.)
- The exact `qcCheckPair` prompt, CAPTURED from the running server via `fake-ollama`'s `FAKE_LOG`
  and replayed: **`OK` in all six arms** — `translategemma:12b` AND `qwen3.6:35b-a3b`, with
  asymmetric siblings, symmetric siblings, and no sibling block at all.
- ⚠️ **A bigger model is NOT the lever**, and the "follow the lesson's convention" clause is NOT the
  cause (a plausible culprit that measurement killed — removing it changes nothing).
- ⚠️ **A FOCUSED single-question prompt IS a partial lever**: **4/7** on `qwen3.6:35b-a3b`, fixing
  2 of 3 real asymmetries where the buried rule fixes 0 of 3, and correctly leaving verbs and adverbs
  alone. **But both failures CREATE asymmetry, and EVERY failure involves the ELIDED article `l'`**
  (`la`/`il` succeed). Fix the elision case and re-measure before this touches the corpus.

**USER RULINGS ALREADY GIVEN — do not re-ask:**
1. **The shape**: split QC into a pure translation check (`translategemma`) and a lesson-consistency
   check on the bigger model that explicitly covers articles. ⚠️ With the correction from the
   measurements: the split must carry a **DEDICATED, single-question check** — the same buried rule
   on a bigger model is measured to do nothing.
2. **PROPOSE ONLY.** It writes a QC flag with a suggested fix, exactly like every other QC finding,
   and the user applies or dismisses it. **It must not auto-apply.** The reason is the 4/7 score: a
   wrong verdict currently CREATES asymmetry, and a flag costs one dismissal where a write costs a
   corrupted pair.

⚠️ **THE COVERAGE HALF IS REAL AND SEPARATE, and is NOT the same problem.** Only **51 of 660** vocab
lessons have ever been QC'd, and **21 of the 23** chapters with asymmetric pairs have never been QC'd
at all. `v90_z`'s post-generation checkbox addresses the FLOW and is worth keeping — it simply cannot
fix an asymmetry until the check itself can. The existing backlog is **126 pairs across 21 chapters**;
re-derive it with `node build_history/probe_article_symmetry_v80j.js`.
...
⚠️⚠️ **THE RATES BELOW ARE STRUCK: THEY ARE A MIXTURE, NOT RATES.** They pool lessons generated by
DIFFERENT MODELS under DIFFERENT PROMPTS — `qwen2.5:7b` scored 0/43, and the strong ARTICLE SYMMETRY
rule landed part-way through the period. Held at one model and one format the same corpus gives 2.5%
before `v85_r` and 33.3% after; see *"THE CORPUS RE-MEASURED BEFORE THE EXPERIMENT"* below. **Do not
quote 11.6% as the rate of anything.** Struck rather than deleted because these numbers are what
motivated `v91_a`, and because the SHAPE they show — one direction, overwhelmingly German-as-source —
survived re-measurement intact.
...
German on the right WITH its article, the target on the left WITHOUT. ✅ **This claim is NOT struck —
re-measurement strengthened it**: restricted to de→it, the direction is not 92% but absolute, 50 of
50 with zero exceptions.
...
### ⚠️ A CHEAP, HIGH-VALUE EXPERIMENT — ⏳ RUNNING (user: *"now let's do the schema-order experiment"*)
...
**Design as actually run** (`probe_schema_order.js`, scratchpad — not in the repo). The system prompt
is the REAL one, captured from the running server via fake-Ollama's `FAKE_LOG` rather than retyped;
only the manipulated spans are touched and every replacement asserts its anchor first. Production
parameters (`maxTokens 1600`, `temperature 0.15`, `think:false`), real de→it chapters from the
corpus, all arms over the SAME chapters so each chapter is its own control. A 2×2, not the 1×2
proposed above — see the finding below for why — crossing schema order (`target`-first shipped vs
`source`-first) with the ARTICLE SYMMETRY rule (shipped vs bullet removed), plus a later `rule-pos`
arm. Each reply is checked for WHICH key the model actually emitted first, so the manipulation is
verified rather than assumed. It has taken every time.

⚠️ **The proposed outcome measure is too rare to work, and the fix is to measure the MECHANISM.**
Asymmetry is a per-lesson event at a single-digit base rate, so eight chapters could never separate
the arms on it. The probe therefore also records the ARTICLE RATE on each side separately. That
catches the effect directly, and immediately did.

---

### 🔬 THE CORPUS RE-MEASURED BEFORE THE EXPERIMENT — AND THE 11.6% IS A MIXTURE, NOT A RATE

⚠️ **Everything in this subsection is OBSERVATIONAL and complete; the experiment above is what will
settle causation.** It is recorded because it changes which hypothesis is worth testing.

**1. ⚠️⚠️ THE RULE WRITTEN TO STOP ARTICLE ASYMMETRY COINCIDES WITH THIRTEEN TIMES MORE OF IT.**
The strong ARTICLE SYMMETRY rule — the one with the dictionary-convention explanation and the
`German → French` worked example — landed at `v85_r` (`986e3a1`, 2026-08-26). The one-line version
dates from `v69`. Holding the MODEL and the FORMAT constant, de→it only:

| `qwen3.6:35b-a3b`, `lessonFormat=json`, de→it | lessons | pairs | asymmetric | fully-asym |
|---|---|---|---|---|
| generated BEFORE `v85_r` (one-line rule) | 25 | 199 | 5 — **2.5%** | 0 |
| generated AFTER `v85_r` (strong rule) | 9 | 72 | 24 — **33.3%** | 2 |

⚠️ **The 11.6% in the table above is a MIXTURE across prompt eras AND models** — `qwen2.5:7b` scored
0/43 — so it is not the rate of anything and should not be quoted as one.

⚠️ **HONEST LIMITS, stated because the number is startling**: n = 9 lessons after the cut, and **ONE
chapter — `Richiamando la figura di Alcide De Gasperi` (`tp_…0184`), whose TWO lessons are each 8/8 —
supplies 16 of the 24.** (An earlier draft of this entry said "two chapters"; that was wrong, and the
correction matters because one chapter contributing two-thirds of the effect is a weaker result than
two independent ones would be.) The user also hand-corrected articles in an Italian lesson, which
biases the "after" figure DOWN, not up.

⚠️ **ONE CONFOUND CHECKED AND CLEARED.** That chapter exists TWICE (see the duplicate below), and the
stale copy has NO German translation — a lesson generated from it would have been TRANSLATING rather
than ALIGNING, a different task with a different pull toward the German dictionary convention. It
does not contaminate the figure: it carries `lessonFormat=undefined` and is excluded by the
model+format filter. Splitting the whole corpus by translation-presence leaves 2.5% / 33.3% exactly
unchanged. ⚠️ Re-check this if the filter is ever loosened.

**2. THE DIRECTION IS ABSOLUTE, NOT MERELY STRONG.** Across the whole de→it corpus, **50 of 50**
asymmetric pairs are Italian-bare ↔ German-with-article. **Zero** run the other way. That is exactly
the pairing the `v85_r` rule forbids by name.

**3. IT IS PER GENERATION CALL, WHICH CONFIRMS THE ONE-DECISION STORY FROM A SECOND DIRECTION.**
Lesson position does not matter (lesson 1: 14.7%, lesson 2: 12.7%). But WITHIN one chapter — same
story, same vocabulary, same prompt — `Leere Aula` lesson 1 is 3/8 asymmetric and lesson 2 is 0/8;
`Stille Prüfung` lesson 1 is 4/8 and lesson 2 is 0/8. The chapter is not the unit. The call is.

### 🆕 A FAILED GENERATION IS LEFT BEHIND AS A SECOND CHAPTER — found while auditing the above

`Richiamando la figura di Alcide De Gasperi` is stored under **two topic ids**, and it is not a
double save:

| | `tp_…0108` | `tp_…0184` |
|---|---|---|
| `generatedAt` | 2026-09-07 **14:19** | 2026-09-07 **17:25** |
| story | 1366 chars, md5 `a0fc8559` | 1366 chars, md5 `a0fc8559` — **identical** |
| `storyTranslation` | **0 chars — MISSING** | 1450 chars |
| `generationStats.lessonFormat` | `undefined` | `json` |
| lessons | 1 | 2 |

Same pasted source text, same settings, three hours apart. The first run produced no German
translation; it was regenerated and the second is complete. **The failed one was never removed, and
it is still a chapter the learner can open** — with lessons generated against a story that has no
translation.

✅ **DELETED at the user's request** (`DELETE /api/lessons/delete?id=tp_…0108`, via the API rather
than an edit to `lessons.json`, because the running server holds the store in memory and would have
overwritten a file edit on its next write). Corpus 363 → 362 topics; the guarded count in
`SESSION_PROMPT_v91_f.md` was updated with it. Checked first and all clear: no chain children, no
storyline membership, zero references in `learners.json`.

⚠️⚠️ **A CLAIM THIS ENTRY FIRST GOT WRONG, CORRECTED BY AUDITING THE OTHER SIX.** It said *"NOT a
September regression — 7 of 34 de→it topics are untranslated, so this has been happening for
months."* **That was wrong.** `storyTranslation` is a feature that ARRIVED, and the older topics
simply pre-date it:

| month | topics | with a translation |
|---|---|---|
| 2026-04 | 8 | 0% |
| 2026-05 | 79 | 3% |
| 2026-06 | 161 | 0% |
| 2026-07 | 43 | **51%** ← adoption |
| 2026-08 | 47 | **100%** |
| 2026-09 | 24 | 96% |

All six remaining untranslated de→it topics are 2026-05-25 … 2026-06-22 — **pre-feature, not
failures**, and nothing to fix. The September case was therefore the OPPOSITE of routine: the only
untranslated chapter in a 100%-translated era. ⚠️ **The lesson is about method, not about
translations**: a rate measured inside one language pair (7 of 34) was read as a trend, when the
corpus-wide view (268 of 362 untranslated, 246 of them before July) showed it was an artefact of when
the feature landed. **Check whether a "defect" predates the feature it violates before calling it
one.**

### ⭐ `translationMeta.origin === 'failed'` IS AN EXACT MARKER — 2 hits in 363 topics, both real

Auditing the other six turned up the one signal that matters. Corpus-wide, `translationMeta.origin`
is `generated` (331), `skipped-same-model` (19), `unknown` (8), `user-provided` (3) — and **`failed`
exactly ONCE now, twice before the delete**. Both `failed` topics are the genuine defects: the Alcuin
chapter (checked in the pre-delete backup — it carried the identical marker) and the one below.
**No false positives, and it catches the pre-feature topics not at all, which is correct — they never
failed.** Anything built here should key on this field, not on an empty `storyTranslation`.

**The second case**: `tp_17886126817580000291`, en→de, 2026-09-05 12:51, a 122-char pasted story,
**0 lessons**. Its own parent in the same chain, generated 34 minutes earlier with an identical
setup, succeeded:

| | parent 12:17 | stub 12:51 |
|---|---|---|
| `translationMeta.model` | `qwen3.6:35b-a3b` | `(none)` |
| `ms` / `promptTokens` | 38929 / 110 | **null / 0** |
| `generationStats` | full record | **absent (no key)** |
| result | 5 lessons, translated | 0 lessons, no translation |

⚠️ **`ms: null`, `promptTokens: 0` and an absent `generationStats` mean NO MODEL CALL WAS EVER MADE.**
This is not a model producing bad output — the pipeline never reached the model, and the chapter shell
was saved regardless. The parent's lesson 1 took **456 seconds**, so a cancelled generation is the
most likely cause. ⚠️ **The stored data cannot separate "user cancelled" from "failed before the
first call"** — if that distinction matters to the fix, `origin` needs to record which.

It was harmless where it sat — last chapter of a two-chapter chain, no children, no storyline, parent
healthy. ✅ **DELETED at the user's request**, after they were told that (unlike the Alcuin case) it
is not a duplicate of a working chapter and removing it discards the pasted text. The whole topic and
its 122-char story were saved to the scratchpad first, alongside a byte-copy of the store. Corpus
362 → 361; the guarded count was updated with it; the parent chapter is intact (5 lessons, translated).

⚠️ **`origin: 'failed'` now matches ZERO topics** — the marker can no longer be demonstrated against
live data. ✅ **So it was preserved in the tree**: `test/fixtures/translation-failed-topics.json`
(both failures, captured from the byte-copy backups, plus FIVE look-alike shapes) and
`test/unit-translation-failed-marker.test.js`. **Do not conclude from a clean corpus that the failure
mode does not exist — read the fixture.**

⚠️ **THE FIXTURE'S VALUE IS ITS NEGATIVES, and the guard's §3 is the assertion that earns its keep**:
it proves the NAIVE detector (empty `storyTranslation`) produces FALSE POSITIVES on two of the five
healthy shapes, so nobody can later "simplify" a check down to an emptiness test. §2 alone would not
catch that — a wrong marker passes a friendly fixture. All five mutations were caught; the fixture was
restored by byte copy (`v87_l`).

⚠️ **A DETAIL THE FIXTURE HAD TO RECORD AS ITS OWN FIELD**: `generationStats` is **absent** (no key)
on the en→de failure, not `null`. `JSON.stringify` drops an undefined key, erasing the difference, and
an earlier read of it here used `|| null` and reported `null` — corrected in the table above. The
fixture carries `generationStatsState: 'absent' | 'null' | 'present'` so the distinction survives.

⚠️ Deleting the two topics also made `docs/index.html` stale (it bakes the corpus). Rebuilt with
`node build-static.js`; the guarded baseline moved to **387 / 319** and the corpus count to **361**.

The open questions stand, but rest on **two** cases, not seven: does a failed or cancelled generation
abort the save or leave a partial chapter, and should lesson generation refuse to run against a
chapter with no translation, since the prompt's shape ("STORY … / GERMAN TRANSLATION …") assumes one.
⚠️ Note the three OTHER zero-lesson topics (en→ja, all 2026-08-23) have `origin: generated` — a
DIFFERENT failure, lesson generation rather than translation. Do not fold them into this item.

⚠️ **A SEPARATE ALARM RAISED AND CLEARED IN THE SAME AUDIT, recorded so nobody re-raises it**: 537 of
1223 lessons have zero `vocab` AND zero `sentences`. That is **not** a defect — `comprehension`,
`word_forms`, `synonyms`, `inflections`, `conjugation`, `grammar`, `math` and `intro_script` lessons
keep their content in their own fields. Of the 98 with no populated array at all, 96 carry it in a
...
sections before being written here — the check that found items `V` and `E` stale and two whole
sections carried as unbuilt after shipping. The fuller diagnoses live in the carried sections further
down and, where noted, in the older roadmaps.*

**🆕 Raised or found during the `v90` line, still open:**
- **⚠️ `.bmodels-pop` CARRIES A LATENT STACKING BUG** (`v90_aa`) — it lives inside `#bottom-bar`,
  which is a stacking context (`position:fixed` + `z-index:900`), so it can never out-rank a
  body-level `#tutor-widget` (`z-index:901`). That is the exact bug that made `#jobs-pop` move out of
  `#jobs-fab`. Pre-existing; **needs a live check with the model picker and the tutor both open**;
  deliberately not folded into `v90_aa`'s request about three other controls.
- **⚠️ A NEW CLASS OF i18n DEFECT: a PRESENT key written TOO EARLY** (`v90_aa`). The signed-out
  account badge rendered the literal text `acct.signin`. `ui.json` has always had that key — it was
  written before `loadUIStrings()` resolved, and nothing re-applied it. ⚠️ **`v90_k`'s audit can
  never find this class: it looks for ABSENT keys.** A sweep for "elements written from `t()` outside
  `applyUIStrings()`, from a path that can run before strings load" is its own piece of work and is
  NOT done. One instance is fixed; the class is not swept.
- **The four-level analysis browsing idea** — the user's own framing ("Perhaps rather for the
  roadmap"): paragraph → sentence → phrase → word, one surface. `v90_z` shipped only the one-token
  fix (clicking a word shows the phrase it belongs to), deliberately. The phrase rung now exists and
  is proven, which is what makes the rest approachable.
- **`translate-select`** (generate a storyline in another source language) is deliberately OUTSIDE
  the wizard's new input field, because it takes no input text at all. **Confirm or move it.**
- **⚠️ TEN `en` keys await hand translation, not three.** `v90_aa`'s own write-up said three
  (`text_explorer.phrase`, `form.fetch_url`, `pdf.no_article`) because it tracked only the keys IT
  and `v90_m` had added. Measuring `ui.json` at the `v91` cut found **ten** absent from every
  language — the other seven are `form.gen_input_lbl`, `form.gen_scan`, `form.gen_drop_one`,
  `qc.story_btn`, `translation.opt_edit`, `translation.opt_regen`, `toast.translation_saved`.
  **A per-release tally of "keys I added" is not the same question as "keys still untranslated"**,
  and only the second one is the user's actual backlog. See the SHORT LIST for the one-line
  re-derivation.

**Buildable now, no decision needed:**
- **⚠️⚠️ `e2e-wikipedia-fetch` §4 REACHES THE LIVE INTERNET, AND IN THIS ENVIRONMENT IT PASSES FOR THE
  WRONG REASON.** Found at the `v91` cut: the full suite failed this one check once
  (`a Talk: page does not come back as an article`) and then passed **15/15 standalone**, which is
  the shape that usually means "flaky, ignore it". It is not.
  - The MediaWiki API half IS hermetic (a stub, via `WIKIPEDIA_ORIGIN`). But the `Talk:` case exists
    to prove the URL **falls through to the GENERIC scrape path** — and that path fetches the real
    `https://en.wikipedia.org/wiki/Talk:South_Tyrol` over the network.
  - ⚠️ **Measured directly**: that live fetch currently returns `status 200` with **0 bytes of HTML**.
    So the extraction fails, `ok` is false, and the assertion passes **because the body was empty**,
    not because the scraper correctly refuses a Talk page. ⚠️ **A guard passing for the wrong reason
    is this project's most expensive recurring defect** — see the containment/vacuity rules.
  - For it to FAIL, the fetch must SUCCEED and the scraper must extract an article — which is what
    happened once under full-suite load. A timeout would have made it PASS.
  - **The fix has a precedent in the same file's own header**: `e2e-fetch-url` drives a STUB
    PUBLISHER rather than the live web. Point the generic path at a stub serving a Talk-page-shaped
    document, so the assertion tests the SCRAPER's judgement instead of the weather.
  ⚠️ **Do not "fix" this by re-running it or by marking it flaky.** The `v89` flake audit's whole
  finding was that the two "known flaky" tests were never flaky and the real defect was elsewhere.
...
- **The `ui.json` translation backlog — RE-MEASURED AT THE `v91` CUT, and the carried number was
  badly stale in the GOOD direction.** The `v90` line carried "`fr`/`de`/`it`/`es` are 59 keys short,
  the rest sit at 75–110". Actually measured now, over 747 `en` keys and 32 translated languages:
  **28 languages are exactly 10 keys short**, `tr`/`hi`/`hr` are 11, and `ko` is 20. The user has
  done a great deal of hand translation across the `v90` line.
  ⚠️ **The 10 missing in EVERY language are this line's own recent additions** — `pdf.no_article`,
  `form.gen_input_lbl`, `form.gen_scan`, `form.gen_drop_one`, `form.fetch_url`, `qc.story_btn`,
  `text_explorer.phrase`, `translation.opt_edit`, `translation.opt_regen`, `toast.translation_saved`.
  ⚠️⚠️ **NEVER carry this count forward — re-derive it**, which is one command and is why the count
  went stale for a whole line:

  ```
  node -e "const u=require('./ui.json');const en=Object.keys(u.en);
    Object.keys(u).filter(l=>l!=='en'&&typeof u[l]==='object')
      .map(l=>[l,en.filter(k=>!u[l][k]).length]).sort((a,b)=>a[1]-b[1])
...
# ✅ THE FOUR USER-REPORTED ISSUES — ALL SHIPPED IN THE v90 LINE (`v90_z`), ONE WITH A CORRECTION

> **The full report text, the diagnoses and every measurement are in `roadmap_v90.md`** — its `v90_z`
> entry, the `v90_aa` entry, and the original report section kept there and marked closed. Only what
> a future session must not re-derive is carried here.

1. **All-null text analysis** — a fixed 1536-token output cap, proven on the real model
   (n=24 stopped at eval **1520**; n=25 and n=39 came back `done_reason=length` with 0 resolved).
   Fixed by sizing the budget from the token count (~66 output tokens per input token) plus one
   retry on a detected truncation. ⚠️ **`ctxTokens` is not optional past ~2700** or Ollama
   truncates the PROMPT instead, silently.
2. **The tutor dropped ~1 in 5 questions** — `_storySelExplain`/`askTutorAboutQuestion` PUSHED the
   student turn (persisting it to `learners.json`) and then hit `_tutorSend`'s silent
   `if(busy) return`. Now queued. ⚠️ **the tutor entry in the jobs popover is SYNTHETIC**,
   rendered off `_tutorState.busy` alone — "the job appeared in the popover" is NEVER evidence that
   a request reached the server.
3. **The vocabulary article asymmetry** — ⚠️ **NOT closed. See "OWED RIGHT NOW" at the top of
   this file.** `v90_z` believed it was, on a correlation; `v90_aa` withdrew that.
4. **Phrase-level analysis display** — the `phrases` data had existed since CP2 shipped and nothing
   read it. A display change, not a pipeline one.

⚠️ **carried item `T`** ("two questions initiated via text-selection → grammar click were
never answered") was **the same bug as #2** and was closed for free at `v90_z` — after sitting open
since the `v86` line waiting for a live reproduction. **The protocol says to cross-check a carried
item against the SHIPPED list; this one also needed checking against the fix about to be made.**

## ✅ ONE INPUT FIELD FOR THE WIZARD, and ✅ SCRAPE A STORY FROM A URL — BOTH SHIPPED IN THE v90 LINE

> ⚠️ **BOTH OF THESE WERE CARRIED THROUGH THE WHOLE v90 LINE AS UNBUILT, AND BOTH HAD SHIPPED.** The
> wizard spec was still headed *"SPEC ONLY, NO CODE"* and the URL section *"MEASURED, NOT BUILT"* at
> the v91 cut — 325 lines of instructions to build things that already existed. A session reading
> them in order would have started building the second one.
>
> - **The one input field** shipped across `v90_s` (first step, nothing lost), `v90_t` (a multi-file
>   drop is refused rather than quietly resolved), `v90_u`, `v90_v`, `v90_w`, `v90_x` and `v90_y`.
> - **The URL scrape** shipped at `v90_m` (scoped to schema.org `NewsArticle`), `v90_q` (Wikipedia
>   through its own API, plus a correction that JSON-LD coverage is much worse than first measured)
>   and `v90_r` (a scraped article gets its paragraphs back).
>
> **The full record of both, including every decision and every measurement, is in
> `roadmap_v90.md`.** Nothing is owed. `INTERNALS.md` §6b carries the feature → function map for the
> wizard router, the bottom bar and `news-article.js`.
...
### ~~E. Chapter-title post-pass failures~~ (from `v85_r`) — ✅ CLOSED at `v89_t` / `v89_y`

⚠️ **This item was waiting for "a LIVE reproduction with the raw model response captured", and it
GOT one — `v89_t`.** The user's own server log showed `0/1 titles came back named` on all three
attempts; the cause was the parser not reading an array of bare strings, fixed there, and the
SILENCE was fixed at `v89_y` (a ⚠️ badge on the chapter card, cleared by any applied title or a
manual rename). The short list in this file already recorded that as fully closed while this body
still asked for the reproduction — **two statements about the same defect, in one document,
disagreeing.** Found at the `v91` cut.
...
### ~~V. Multiple image upload for comic generation~~ — ✅ SHIPPED at `v89_al`

⚠️ **CARRIED AS "Still open, not started" FOR THE WHOLE v90 LINE, while `roadmap_v89.md` recorded it
as `v89_al — item V: multiple images, each its own panel`.** The `v90` cut's own header even LISTED V
among the items closed during the `v89` line — so the summary was reconciled and the body was not.
Found at the `v91` cut by grepping every carried item's letter against both shipped sections, which
is the check the protocol asks for and which nobody had run.
...
6. **Version** — bump `APP_VERSION` in `server.js` if it's a new release. NOTE (v49): the static
   ...
   **Point releases use an alphabetic suffix** (user, v70): the base cut is the bare number and is
   implicitly `a`, so the sequence is `v91_a` → `v91_b` → `v91_c` → … — the same convention the v69–v90
   lines ran. **This is the `v91` line.** Roadmaps are per BASE version, so point
   releases do not each get one — this file stays current through the whole v91 line.
...
## Rules earned in the v90 line

*Numbered locally, like every other line's block — cite them as "`v90` rule N", never bare. Read the
**"⚠️ How the rules are NUMBERED"** note above before citing one.*

1. **⚠️⚠️ WHEN A MEASUREMENT IS USED TO TALK SOMEONE OUT OF BUILDING SOMETHING, RUN THE FALSIFYING
   EXPERIMENT FIRST.** `v90_z` read "QC'd de→it pairs are 0 of 64 asymmetric, never-QC'd are 65 of
   274" as evidence that the article rule WORKS, and used it to argue against building a dedicated
   check. It was a CORRELATION — those lessons were QC'd by the model that was later measured to
   miss 8 of 8; they were already symmetric. **The confirming experiment was named in `v90_z`'s own
   write-up and never run**, because the user chose the option that appeared not to need it — and
   the argument for that option WAS the unverified claim. The user's live test refuted it in a day.
2. **"It exists" is not "it works", and only the second licenses a decision.** Twice in one session a
   request for a "new" mechanism turned out to be a request for one that already existed
   (`qcCheckPair`'s article rule; the `#post-gen-qc-cb` checkbox). Checking for existence was right
   and saved real work on the second. Treating existence as sufficiency was wrong on the first.
3. **⚠️ TWO SIGNALS THAT ALWAYS FIRE TOGETHER ARE ONE SIGNAL.** `unit-analysis-truncation` detects a
   truncation two ways (`done_reason === 'length'`, or a parse failure leaving every token
   unresolved). Its first fixture triggered BOTH, so deleting *either* arm left the guard GREEN —
   and after a fixture was added for the first, deleting the *second* did too. It took three
   fake-Ollama fixtures (`ZZZCUT`, `ZZZNODR`, `ZZZWORSE`) to separate them. **When a guard covers an
   OR, every arm needs a fixture that fires it ALONE.**
4. **⚠️ A MUTATION THAT SILENTLY MATCHED NOTHING READS EXACTLY LIKE A SURVIVING ONE.** Two
   "survivors" in the `v90_z` session were `str.replace` calls whose anchor did not exist. **`cmp`
   the file against its backup and assert it actually changed** before believing a survivor — the
   mutation harness must fail loudly when its own edit is a no-op.
5. **A mutually-masking pair of guards should be REMOVED, not documented.** The tutor's pending flag
   was cleared in two places; deleting either left the guard green. Deleting one of them made the
   code simpler AND made both mutations red. Contrast with a genuine equivalent mutant (the retry
   predicate's redundant conjuncts), which is kept and explained because it becomes load-bearing the
   moment the surrounding function changes.
6. **⚠️ A PRESENT `ui.json` KEY CAN STILL RENDER RAW.** The account badge showed the literal text
   `acct.signin` because `t()` ran before `loadUIStrings()` resolved and nothing re-applied it.
   **`v90_k`'s unlocalized-string audit structurally cannot find this class** — it looks for ABSENT
   keys. Heal such strings from `applyUIStrings()` rather than by reordering init, so a UI-LANGUAGE
   CHANGE fixes them too.
7. **⚠️ A STACKING CONTEXT CAPS EVERY DESCENDANT'S z-index.** `#bottom-bar` is `position:fixed` +
   `z-index:900`, so a panel parented inside it can never out-rank a body-level `#tutor-widget`
   (`z-index:901`), whatever z-index it is given. That is why `#jobs-pop` lives at body level and is
   positioned in JS. **Before re-parenting a floating panel for free CSS alignment, check what
   stacking context it would be entering.**
8. **⚠️ A "SPEC ONLY / NOT BUILT" SECTION IS A CLAIM WITH A SHELF LIFE.** Two whole sections (325
   lines) were carried through the entire `v90` line telling a session to build things that had
   shipped inside that same line. **At every cut, grep each open section's own heading against the
   shipped list** — and the same for every carried item's letter, which is what found items `V` and
   `E` stale at the `v91` cut.
9. **Verify a layout claim by MEASURING THE RENDERED GEOMETRY, not by reading the CSS.** `v90_aa`'s
   alignment was confirmed by reading back `getBoundingClientRect().left` for each button and each
   panel in a real browser (16→16, 133→133, 221→221). ⚠️ `test/lib-dom.js` gives EVERY element the
   same rect and a constant `offsetWidth`, and its `innerWidth` is 390 — a layout assertion there
   proves arithmetic, never geometry.

---

# ✅ SHIPPED IN THE v91 LINE
...
**ZERO `ui.json` keys. No code change.** Cut from `roadmap_v90.md` at the `v90_aa` release, at the
user's own suggestion. The reconciliation is the deliverable — see the **⚠️ WHAT WAS CARRIED** block
at the top of this file for what it found: two whole sections carried as unbuilt after shipping
(325 lines), and carried items `V` and `E` both stale against `roadmap_v89.md`'s own shipped list.

⚠️ **`§0`/`§0i` were carried WHOLE and deliberately not compressed** — they are dense with user
rulings marked "do not re-derive", and a compression pass over a document this size is how `v89`
rule 1 happened. Shrinking them is its own task, with the user in the loop.
