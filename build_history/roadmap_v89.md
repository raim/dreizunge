...
| **⚠️ the savedList projection — ~~for the FOURTH time~~ CORRECTED at `v90_a`** | ~~Chapter `source` had to be added to the whitelist.~~ **It was already there** (`source: l.source || null`), and the line added here was a redundant DUPLICATE KEY with no behavioural effect. Found by the projection audit this very claim inspired; the duplicate is removed and the record corrected at `v90_a`. The trap itself is real and is recorded three times in the projection's own comments (`v74_i`, `v79_n`, `v89_y`) — this was simply not a fourth instance |
