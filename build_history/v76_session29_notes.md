...
## `v76` — the cut, and the card gate table

No product change. `APP_VERSION = 'v76'`, `docs/` rebuilt, suite unchanged at 174/151.

Produced for the progress-card rework:

- **`build_history/v76_card_gates.md`** — the measured AS-IS truth table for the completion card.
  32 rows across BOTH gate families (classic completion-driven, and mixed-driven coverage), derived
  by RUNNING `showComplete`, not by reading it. `build_history/probe_gates_v76.js` is preserved:
  re-run and diff after any change to the card.
- **`build_history/roadmap_v76.md`** — the user's rework notes merged into the queue, with
  everything from the v75 line carried forward IN FULL rather than by reference (the v71→v72
  boundary lost three items that took until `v73_k` to recover).

### The first version of the table was wrong, and saying so is the point

It reported `storyUnlocked() == false` for learners in the very state where every gated lesson was
marked complete, and `true` for teachers. Rather than publish it, the disagreement was chased: the
probe was seeding `progress.completed` on a chapter with a **`mixed`** lesson, where the learner
path does not read those booleans at all — it goes down the coverage branch. It was also seeding
`coverageThreshold: 0.8`, silently adding a second gate that the default (1) makes a no-op.

The rebuilt probe drives each gate through what it actually reads **and asserts the gate moved
before recording the row**, so a mislabelled row reports itself instead of being published under a
state it is not in. Table A (classic) now has all 16 rows agreeing; table B (mixed) has 4 that do
not, and those 4 are the finding below rather than a defect in the table.

### Findings worth carrying

1. **`comp-back` is hidden in all 32 rows** — the back button the rework wants already exists and is
   already dead.
2. **`comp-storyboard` is hidden in all 32 rows** — storyline-level content on a chapter card.
3. **`comp-story-unlocked` does not mean the story is unlocked** — it is the preview label, shown
   while locked whenever `canGenerate` or teacher mode is on. It will collide with the rework's
   real unlocked card; rename it.
4. **`comp-drill` is grey or hidden in all 32 rows** — never enabled on either chapter. Check before
   redesigning the button row around it.
5. **`v74_l`'s hide-list is barely observable** — in the rows where it applies, those buttons were
   already hidden for other reasons. It may be carrying less weight than its test implies, which is
   relevant to the ruling on whether it survives.

### OPEN, and it should be settled first

On a mixed-driven chapter a learner could not reach `storyUnlocked` even with every lesson complete
and every question id from `_lessonQidUniverse` marked solved:

```
seeded APP.progress.solved[topic] : 86 keys
topicCoverage(true)               : { solved: 0, total: 31 }
```

86 in, 0 counted, total 31. Either `APP.progress.solved` is not the store `topicCoverage` reads, or
the two key questions differently. **Stated as an open question, not a bug** — the seeding may be at
the wrong level and it was not chased further. It matters because coverage is the gate for every
mixed-driven chapter, and it is invisible from the classic-set tests.

---

## `v76_b` — Serbian and Croatian were not in the drop-downs

User-reported. `sr` and `hr` were added to `languages.json` in `v75_g`; they resolved, got a script
table, got TTS codes, passed every existing test — **and never appeared in either menu.**

**The two language menus are hand-written `<option>` markup in `index.html`, not generated from
`languages.json`.** Adding a language is a two-file change and only one file had a guard. Measured:
`#src-lang-select` and `#lang-select` each carried 30 of the 32 codes; the two missing were exactly
`sr` and `hr`. No other select in the file carries a language list (27 selects scanned).

**Fixed by adding the options, not by generating them** — the two menus are deliberately ordered
differently (`lang-select` leads with Italian, `src-lang-select` with English), so generating from
`languages.json` would silently reorder a user-visible menu. Generation is the better end state and
is noted for the roadmap; the guard is what makes the duplication safe meanwhile.

`unit-lang-menu-coverage` (new) asserts both directions — every `languages.json` code is offered in
BOTH menus, and no menu offers a code absent from the data (a dead entry selecting a language the
app cannot resolve) — plus that every offered language has `name`, `flag` and a well-formed `tts`.
`names` is deliberately NOT required: it falls back to `name`, and a new language legitimately has
only `names.en` until the pass runs. Revert-verified three ways.

### `--langnames`: the second matrix nobody was filling

`languages.json` carries, for every offered language, its name written in every OTHER offered
language — that is what the menus label their options with. It is a 32x32 = **1024-cell matrix, and
151 cells were empty.** The breakdown is the interesting part:

```
29 UI languages missing exactly 2   -> sr, hr          (the two new ones, as the user suspected)
sr, hr missing 32 each              -> no column at all (likewise)
lb missing 31 of 32                 -> PRE-EXISTING, since Luxembourgish was added
```

The `lb` hole was not reported and nobody knew about it: every Luxembourgish menu label in the app
has been falling back to the English name.

Added `node translate-ui.js --langnames` (with `--check` / `--dry` / positional codes, like the rest
of the script). Batched by UI LANGUAGE rather than per cell — one request per target language asks
for every missing name in it, which is both the natural prompt and far fewer calls. It reuses
`validateEntry`/`isBlocking` from `ui-qc.js` rather than inventing a second rule set, per that
file's own standing note that the writer and the auditor must agree.

**The writer re-emits `languages.json` in its own hand-written shape** rather than
`JSON.stringify`-ing it. That is not cosmetic: stringify reflows 67 lines into ~1100 and makes every
later diff unreadable — I did exactly that by accident in `v75_g` and had to redo it. Verified round
trip: same data, same 67 lines, and it refuses to write a file that will not re-parse. Pinned in
`unit-lang-menu-coverage`, including a non-vacuity check that the serializer actually emits added
names (a serializer that returned its input unchanged would satisfy the other three assertions).

### A no-op I nearly shipped with a confident comment

`ui-qc.js` maps `sr: 'Cyrillic'`, and Serbian is digraphic, so "add `sr: ['Latin']` to `EXTRA_OK`"
looked obviously right. **It changes nothing:** `SCRIPTS` has no `Latin` pattern, so Latin is never
tested for any language. Serbian Latin already passes; Serbian Cyrillic passes via `LANG_SCRIPT`;
Croatian (Latin-only, absent from `LANG_SCRIPT`) correctly rejects Cyrillic because Cyrillic IS
tested. Reverted, with the finding recorded in the file so the next person does not re-add it.

### A third source pin broke on spelling this session

`unit-ui-qc` pinned the literal `if (!QC_MODE && (CHECK_ONLY || totalMissing === 0))` and failed
when `--langnames` added a second exemption to the same line — behaviour for `--qc` unchanged.
Repinned on the claim (`--qc` is exempt from the early exit) in a way that lets further exemptions
join without breaking, and revert-verified that removing the `--qc` exemption still fails it.

**That is three in one session** (`if (!u)` in `v75_h`, its own first replacement, and this one).
Standing rule 9: **a source pin should match the CLAIM, and the narrowest region that carries it —
never a whole line of code that unrelated changes will also touch.**

### Data files

`lessons.json` (301 topics, 84 storylines), `learners.json` and `ui.json` were replaced with the
user's current copies before any change; suite green on them at 174 before the work, 175 after.
`ui.json` still has `sr`/`hr` as empty stubs — the translate pass has not been run yet.

---

## `v76_c` — `--langnames` shipped broken, and the user hit it on the first real run
...
Two mistakes in one call, both mine, both from **inventing an interface instead of copying the
working one thirty lines above in the same file**:

1. `callLLM` is **positional** — `callLLM(model, system, userMsg, maxTokens, opts)` — and it was
   called with an options bag, `callLLM({ model, system, prompt, host })`. The object travelled all
   the way to the backend and surfaced as an Ollama struct error naming *Ollama's* field rather than
   the caller's bug.
2. It returns `{ text, … }`, not a string. `extractJSON(raw)` would have thrown inside the retry
   loop even after (1) was fixed, and every name would have been silently dropped — the mode would
   have reported "0 ok" for every language and looked like a model problem.

**Why it got through: the only thing ever exercised was `--langnames --check`, which never calls
the model.** Detection was measured, serialization was measured and revert-verified, and the request
path — the only part that talks to anything — was never run. A path that is only ever exercised in
its no-op mode is not tested. That is the lesson, and it generalises past this file.

Fixed, plus two guards:

- **`llm.js` now rejects a non-string `model` up front**, with a message that names the positional
  signature and shows a correct call. This protects every caller, not just this one, and converts a
  confusing backend error into an actionable one at the call site.
- **`unit-langnames`** runs the REAL `--langnames` mode against a stubbed `llm.js` in a temp
  directory with a three-language fixture, and asserts on the ARGUMENTS the mode passes (model is a
  string, system/userMsg are strings, maxTokens is a number in the 4th position) and that the
  response was consumed — `after.sr.names.de === 'NAME_sr'` fails if the result property is read
  wrongly. It also checks nothing outside `names` was disturbed.

Revert-verified three ways, each firing a named assertion: restore the options bag (fires *"callLLM
receives the model as a STRING — an object here is the v76_b bug"*), read `raw` instead of
`raw.text`, and remove the `llm.js` argument guard.

One wrinkle worth recording: the non-vacuity probe in that test — "a well-formed call is NOT
rejected by the guard" — returns a promise that rejects for lack of a backend, and an unhandled
rejection crashes the runner. It has to be caught explicitly; what is being asserted is that the
SYNCHRONOUS argument check let the call through, not that the call succeeds.

### Standing rule 10

**Do not invent an interface you can read.** Both halves of this bug were visible in `translateLang`
in the same file. Copy the working call shape, then change what needs changing.

### Standing rule 11

**A code path exercised only in its `--check` / `--dry` / no-op mode is untested.** If the only
thing standing between a mode and the network is a flag, the tested configuration is the one that
does nothing.
