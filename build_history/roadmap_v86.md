...
### P. ⚠️ Inflection MCQ distractors sometimes mix grammatical DIMENSIONS — `v86_af` fixed the SINGLE-dimension case; the COMBINED-dimension case resisted TWO live-tested fixes, and may be a pedagogy question, not a bug
...
"datief" report) IS fixed, but the combined-dimension case is not.

**`v86_ag` tried the obvious fix, live-tested it, and it did NOT work — then reconsidered the
diagnosis itself.** A second worked item (combined tense+person, compliant `formChoices`) was added
to both `default` and `de`, and the SAME real chapter was regenerated against the real model with
the fix in place. `"Infinitief"` STILL appeared as a wrong choice on all 3 items — byte-for-byte the
same failure as before. Two separate live-tested reinforcement attempts on two different parts of
this SAME prompt (`v86_ab`'s `{S}`-language fix, item AJ; this cut's combined-dimension example)
have now both measured ZERO effect on this real model's real output for this real input.

**Reconsidered — this may not be the SAME bug as "datief" at all.** The original report was a
distractor naming a grammatical CATEGORY THAT DOES NOT EXIST for that word class in that language —
Dutch nouns have NO case marking whatsoever, so `"datief"` doesn't just belong to "a different
dimension," it names a category with nothing real to have offered. `"Infinitief"` for a Dutch VERB
is different: infinitive-vs-conjugated is a REAL, common, pedagogically useful distinction — arguably
MORE fundamental to language learning than distinguishing 1st from 3rd person — and is not
linguistically nonsensical the way `"datief"` was. The "direct relatives only" rule, generalized from
the noun-case incident, may simply be OVER-STRICT when applied uniformly to verb mood/finiteness.

**Genuinely open, needing a product/pedagogy decision, NOT more prompt engineering**: should
infinitive-vs-conjugated count as a permitted axis specifically for VERB forms, distinct from case
(genuinely absent for some languages) and other closed grammatical categories the rule should keep
forbidding? Two live-model test cycles have now failed to move this needle via wording alone — a
third attempt without first answering this question is not recommended.
...
Continuation of item P's own residual finding (recorded doc-only, just before this cut): the
`v86_af` "direct relatives" rule was confirmed, via real post-`v86_af` generation data, to still
allow `"Infinitief"` as a wrong choice against a combined tense+person correct answer, three times
in one lesson. `PROMPTS.inflections`'s worked examples (`default`, `de`) had only ever demonstrated
the SINGLE-dimension case (`"plural"`/`"singular"`) — no combined-dimension worked example existed
to imitate.

**Built and shipped**: each example (`default`, `de`) gained a SECOND worked item with a genuinely
COMBINED-dimension correct answer (tense + person — `"Präteritum, 3. Person Singular"` /
`"past tense, 3rd person singular"`), whose own `formChoices` vary ONLY those two dimensions,
never introducing a third. This is a real documentation improvement on its own terms — the prompt
now demonstrates BOTH cases the instruction describes — independent of whether it changes model
behaviour for any one specific input.

**Live-tested against the real reported input, AGAIN** — and it did NOT eliminate `"Infinitief"`:
regenerating `tp_17880367188140000070`'s inflections lesson with the new example in place still
produced `"Infinitief"` as a wrong choice on all 3 items, byte-for-byte the same failure mode as
before the fix. Two separate live-tested prompt-strengthening attempts for two separate parts of
this SAME prompt (`v86_ab`'s `{S}`-language reinforcement, item AJ; this cut's combined-dimension
example) have now BOTH measured zero effect on this real model's real output for this real input.

**Reconsidered, and this may not actually be a bug**: the original "datief" report (`v86_ab`) was a
distractor referencing a grammatical CATEGORY THAT DOES NOT EXIST AT ALL for that word class in that
language — Dutch nouns have zero case marking, so `"datief"` for a Dutch noun is not "a different
valid dimension," it names a category with no real values to have offered in the first place.
`"Infinitief"` for a Dutch VERB is a different situation entirely: infinitive-vs-conjugated is a
REAL, common, pedagogically useful distinction for verbs — recognizing whether a word is conjugated
or in its dictionary/base form is arguably a MORE fundamental skill than distinguishing "1st person"
from "3rd person," and offering it as a wrong choice for "what form is this conjugated verb in?" is
not linguistically nonsensical the way `"datief"` was for a caseless language. The "direct relatives
only" rule, generalized from the noun-case incident, may be OVER-STRICT when applied uniformly to
verb mood/finiteness — a genuine open question, not a further engineering task: should
infinitive-vs-conjugated count as a permitted axis specifically for verbs, distinct from case
(genuinely absent for some languages) and other closed grammatical categories? **Not decided.**
Recorded in `roadmap_v86.md`'s item P for a future session (or the user) to rule on — NOT resolved by
guessing, and NOT chased with a third live-model test cycle without a product decision first, given
the now-established pattern that textual reinforcement alone isn't moving this specific needle.

`test/unit-prompt-examples.test.js` gained §7: both examples now carry exactly 2 worked items, the
second's `formLabel` is genuinely combined-dimension, and its `formChoices` avoid the mood/case
"forbidden" categories the original report's own logic identifies (a documentation-consistency
guard — it verifies the EXAMPLE complies with the rule it demonstrates, not that the real model
does; that second claim is honestly reported as FALSE above, not conflated with the first). Mutation-
tested (an `"Infinitiv"` distractor injected into the example correctly fails the guard).

Baseline: `node test/run.js` → 299 checks, unchanged (no new test file). `--quick` → 256, unchanged
(one known-flaky `unit-tap-word` failure in the full run, reconfirmed pre-existing/unrelated all
session). `docs/index.html` rebuilt (`APP_VERSION` bump only). `lessons.json`/
`canonical-analysis.json`/`ui.json` unchanged since `v86_af` (336/97/714, no drift).
`APP_VERSION = 'v86_ag'`.
