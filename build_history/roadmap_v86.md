...
edits (still 336/97, no drift since `v86_ab`). Live interactive click-through was SKIPPED, not
performed and reported as if it were: no LLM backend was reachable in this sandbox at verification
time (`canGenerate` false), so the confirm/force wiring is covered by the mutation-tested unit + e2e
suites above, which exercise the exact same code paths end-to-end against a real (fake) server —
not asserted as equivalent to a live click-through, just the honest reason none was done.
`APP_VERSION = 'v86_ac'`.
