...
> **STATE AT HANDOFF: CUT AS `v64`; suite 115 green; check-inline 0 on both builds.**
> v64 adds **streamed tutor replies** — the reply appears word by word, with reasoning suppressed
> mid-stream. See `v64_session1_notes.md`.

**v63 shipped** (per-question inline help — see `v63_session1_notes.md`). This file supersedes
`roadmap_v63.md` and carries the protocol plus every open item forward.

## ✅ v64 — streamed tutor replies (SHIPPED)
- **Tutor-only streaming**: `llm.js` gains `callLLMStream` beside the untouched `callLLM`; stories,
  lessons, QC and translation keep the whole-reply path (and the v59 metering choke point).
- **`makeThinkFilter`**: suppresses reasoning AS IT STREAMS, including `<think>` tags split across
  chunks — `stripThink()` can't be used mid-stream because it needs the complete string.
- **SSE**, opt-in per request; `{delta}` frames then a terminal `{done, reply, tokens}`; abort-safe;
  the JSON whole-reply path preserved as a feature-detected fallback.
- Live bubble replaces "thinking…" at the first token; a truncated stream keeps what arrived.
- Guarded by `unit-tutor-streaming`; the v59 convergence guard was made word-bounded (it fired
  correctly on the shared `_rawCallLLM` prefix).
