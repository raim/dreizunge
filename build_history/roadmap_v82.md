# Dreizunge roadmap — v82

**This is the `v82` line.** Cut from `roadmap_v81.md` at the `v82_a` release, at the completion of
`PLAN §C4` (the Settings Card) — which also closed out the `§C0` router-seam and `§C5`
generation/library-split tracks that came just before it, so this cut lands at a genuine milestone
boundary, the same shape as the `v80`→`v81` cut at "the end of the TRACK T build-out."
**`roadmap_v81.md` is kept and is not superseded as a record** — the whole `v81` line's release
history (`v81_a` … `v81_ad`, thirty-odd entries) lives there under `# SHIPPED IN THE v81 LINE` and
was NOT copied here. Go there for how something was built or why a guard is shaped the way it is;
this file stays current through the whole v82 line.
...
> `v81` release entries. Nothing open was dropped — the cut is by KIND, not by judgement about what
> still matters, so no item had to be assessed to survive it. This is exactly the `v80`→`v81` cut's
> own carry/drop split, repeated verbatim — see that file's own header if the reasoning is unclear.

### What is in this file, in order

| section | what it is |
|---|---|
| **OPEN AT THE v82 CUT** | the findings that govern the open sections, then `§0` / `§0i` themselves, then the standing RULES |
...
7. A version bump to a new BASE needs its own roadmap. **This is that roadmap for `v82`.**
...
6. **Version** — bump `APP_VERSION` in `server.js` if it's a new release. NOTE (v49): the static
   ...
   **Point releases use an alphabetic suffix** (user, v70): the base cut is the bare number and is
   implicitly `a`, so the sequence is `v77` → `v77_b` → `v77_c` → … — the same convention the v69–v81
   lines ran. **This is the `v82` line.** Roadmaps are per BASE version, so point
   releases do not each get one — this file stays current through the whole v82 line.
