...
## ✅ v57 — clickable storyboard panels → lesson sets (SHIPPED; was item 1)
Full details + rationale: `v57_session1_notes.md`. What shipped, against the item's spec:
- **Mapping** (the settled design): ONE rule shared by click handling and framing. A panel's
  model-assigned `chapter` (new per-panel prompt field, sanitized by `composeStoryboardSVG` into a
  clamped `data-chapter` attribute) wins; any panel without one falls back to the deterministic
  equal split `chapter(i) = floor(i·C/P)+1` — fewer panels → each links to the chapter its span
  STARTS with; more panels → links repeat. **Existing boards are therefore clickable with NO
  regenerate** (the user's call — the 409 mirror in the original spec was dropped); one regenerate
  later upgrades them to model-assigned links for free, and 🎨 re-colours preserve links because the
  stored panels carry the field and the recompose passes the CURRENT chapter count (re-clamping
  stale values after chapter deletions).
- **Click plumbing**: ONE delegated document-level listener over
  `.storyline-storyboard[data-sb-chain]` — serves the landing cards, the storyline screen, the new
  lesson-set-page board, and the baked static DOM alike (no per-render init to forget). All four
  render sites carry the hook; `unit-static-landing-parity` now guards `data-sb-chain` too.
- **Locking**: teacher mode / canGenerate navigates freely. Student mode applies the SAME fail-closed
  rule as the storyline screen's chapter cards; a locked panel does not dead-end — it RESUMES at the
  first unlocked, incomplete chapter (toast `storyboard.locked_resume`), reusing loadSaved/progress —
  no second progress system.
- **Lesson-set pages**: the board renders ABOVE the ←/→ cont-nav with a blue frame around this
  chapter's panel(s); frame and links share the one mapping function so they cannot disagree.
- **Panel budget**: 6 panels normally, up to 12 for >10-chapter storylines, wrapped at 6 per row
  (rows centre independently; a full row lands on the original 12px margin). The composer's hard cap
  is 12 (security bound); the 6/12 split is a PROMPT rule (`{maxPanels}` templated), and token
  headroom scales (12000 for 12-panel boards). v55_t's panel-size invariant is kept via a
  fixed-WIDTH canvas; `unit-storyboard`'s fixed-canvas test was reworked (not deleted) into the
  per-row form.
