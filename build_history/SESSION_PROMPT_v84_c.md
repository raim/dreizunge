# Session prompt — written at the `v84_c` cut

*(Rename this file for the version the session WRAPS UP WITH — `git mv` + edit, never keep the old
one alongside. Keep using the double-letter suffix scheme (`v84_d`, `v84_e`, …) unless a future
session has a good reason to switch to `v85_a` instead.)*

I'm continuing development of Dreizunge (a single-file `index.html` client + `server.js`,
zero-dependency Node language-learning app). Picking up from **`v84_c`** — the `dreizunge` PATH
launcher (`bin/dreizunge`, installed onto `~/.local/bin` by `install.sh`, idempotently): starts the
server in the foreground (Ctrl-C stops it, no daemonizing) and opens the browser once it answers,
`--no-browser` to skip — the `jupyter notebook` shape, discussed and deferred at the `v83` cut, built
now on direct request ("ok build the dreizunge PATH launcher"). Verified with a real symlinked-from-
elsewhere run against a stub server AND a real `install.sh` end-to-end run (idempotent install,
genuine working launch — the installed symlink actually started the real app). See `roadmap_v84.md`'s
own `v84_c` entry for the full write-up.

**`v84_b`, condensed** — PWA install support (`manifest.json`/`icon.svg`/`sw.js`, local server only):
browsers can offer "Install App," windowed, no tabs/omnibox. **✅ Registration is CONFIRMED working**
— a sandboxed preview browser failed during the build itself, reasoned as a sandbox restriction, not
a bug; the user then verified it for real in Google Chrome on Ubuntu via `localhost:3000`, same day.
**One separate, real, still-open limitation found in that same follow-up**: a LAN IP over plain HTTP
(tested on Android) gets NO install option — service workers need a secure context (HTTPS or the
`localhost` loopback exception), which a LAN address never satisfies. Same root cause as the app's
own existing insecure-transport warning, not a new bug. Fix (not built): a TLS-terminating reverse
proxy — judged by the user themselves as "probably only required for a future deployment," not an
immediate priority.

**Also fixed at `v84_b`, found while building it** (a test-harness bug, not a PWA bug):
...
2. `build_history/roadmap_v84.md` — its **index table** and **⚠️ Session protocol** block first, then
   ...
   the v83 line" for the two newest), then `# SHIPPED IN THE v84 LINE` for `v84_c`'s launcher and
   `v84_b`'s PWA work + the `lib-dom.js` fix — read all of it before touching any of those areas.
...
## Both the installation-experience items are DONE — closed, don't re-open without a reason

PWA install support (`v84_b`) and the `dreizunge` PATH launcher (`v84_c`) are both shipped and
verified. The one remaining, SEPARATE limitation (a LAN IP over plain HTTP gets no PWA install
option, since service workers need a secure context) is well-understood, not a bug, and its real fix
(a TLS-terminating reverse proxy) is explicitly a future-deployment concern the user themselves
deprioritized — don't build it unless asked.

## My suggestion: re-evaluate `apply-cp-lessons.js` output with `v83_p`'s register fix

The clearest remaining item that closes a real, ALREADY-KNOWN gap rather than opening a new one: the
user's own two `PLAN §7.0` evaluation runs (`v83_n` qwen2.5:7b, `v83_o` qwen3.6:35b-a3b) both predate
`v83_p`'s register-mismatch fix (the "kommen"/"venne" bug). Re-running with `--replace` would show
whether the fix actually produces correct output on real content, not just the hand-built test
fixtures — genuinely unverified until someone looks at real generated output again. Unlike the last
two sessions' picks, this one leans more on the USER's own judgment (reading translations) than
code — flag that up front rather than just doing it silently.
