# Dreizunge roadmap — v83

**This is the `v83` line.** Cut from `roadmap_v82.md` at the `v82_i` release, at the user's own
request when handing off a new feature — not a milestone-completion cut in the `v80`→`v81`/`v81`→`v82`
sense, but the same shape for the same reason: the `v82` line ran nine point releases
(`v82_a`…`v82_i`) across a new lesson-type build-out (`PLAN §D4`, both phases), three independent
bug fixes found and fixed on their own merits (the `order`-exercise length gate, the
`e2e-lesson-edit-roundtrip` timing flake, difficulty-tiered furigana), and had grown long enough that
a fresh cut was due on accumulated-context grounds alone.

**`roadmap_v82.md` is kept and is not superseded as a record** — the whole `v82` line's release
history (`v82_b` … `v82_i`, eight point releases) lives there under `# SHIPPED IN THE v82 LINE` and
was NOT copied here. Go there for how something was built or why a guard is shaped the way it is;
this file stays current through the whole v83 line.
...
> current focus) and **THE LARGER PLAN** (the folded `implementation_plan.md`), now with a new
> `PLAN §12` for the feature this cut exists to hand off. Not carried: the `v82` release entries, and
> the `v82_i` SESSION_PROMPT's own "buildable now" backlog — that carries forward through the
> SESSION_PROMPT lineage instead (see `SESSION_PROMPT_v83_a.md`), the same way it always has; this
> file has never been where per-cut backlog lives. Nothing open was dropped — the cut is by KIND, not
> by judgement about what still matters, so no item had to be assessed to survive it. This is exactly
> the `v81`→`v82` cut's own carry/drop split, repeated verbatim — see that file's own header if the
...
7. A version bump to a new BASE needs its own roadmap. **This is that roadmap for `v83`.**
...
6. **Version** — bump `APP_VERSION` in `server.js` if it's a new release. NOTE (v49): the static
   ...
   **Point releases use an alphabetic suffix** (user, v70): the base cut is the bare number and is
   implicitly `a`, so the sequence is `v77` → `v77_b` → `v77_c` → … — the same convention the v69–v82
   lines ran. **This is the `v83` line.** Roadmaps are per BASE version, so point
   releases do not each get one — this file stays current through the whole v83 line.
...
*The v82 line ran nine point releases and its write-ups stay in **`roadmap_v82.md`** under
`# SHIPPED IN THE v82 LINE`, newest first. They were NOT copied here: they are history, and this file
is for what is open. Go there for how something was built or why a guard is shaped the way it is.*

**The ones a v83 session is most likely to need:**

| release | what, and why you might need it |
|---|---|
| `v82_b` | user UI-feedback batch — shared story panel across question/progress cards, flag buttons, and a real word-highlight/tap regression found and fixed |
| `v82_c` | new lesson type `inflections` (closes the `v80_f` inflection-coverage gap); a story-panel alignment fix; a Japanese word-boundary bug and a furigana pipeline that turned out mostly nonfunctional in the corpus — both fixed and A/B-tested live |
| `v82_d` | three small fixes on `inflections`'s own screens — speech language for `inflection_form`, entry/progress card section order, progress-card ← chapter navigation (the third one is worth reading in full: a fix that passed its own tests was still wrong one level down, caught by user correction) |
| `v82_e`/`v82_f` | `PLAN §D4`, BOTH phases — new lesson type `writing`, the app's first PLAY-TIME-graded exercise. `v82_e` shipped typos+grammar via a new stateless `/api/writing-feedback` route (not a batch generator); `v82_f` reworked the stem into a real source-language-only comprehension question and added LLM correctness judging against the story, on the user's own immediate follow-up. **Read this pair before touching `writing` or before building `PLAN §12`** — the play-time live-model-call pattern `PLAN §12` needs (priming a tutor turn from client-selected text) is the closest existing precedent, alongside `/api/tutor` itself |
| `v82_g` | sentence-ordering (`order`) exercises are now length-gated to ≤5 words — `mkOrder`'s candidate pool filters on `s.words.length` before picking |
| `v82_h` | the `e2e-lesson-edit-roundtrip` flake, diagnosed (not just reconfirmed a fourth time) — `updatedAt`'s millisecond-resolution timestamp collided under load; fixed with a monotonic `stampUpdated()` helper consolidating all nine stamp sites in `server.js` |
| `v82_i` | difficulty-tiered furigana density RESTORED (dead since ~v40) — `sysStory` regained a `difficulty` parameter; the advanced tier's first wording was live-tested and found NOT to produce sparse output, fixed with a concrete worked example and re-verified live |

**`PLAN §D4` is now fully built** — both the typo/grammar phase and the content-correctness phase
the roadmap's own text had flagged as needing a stronger model "likely" (it did not). One measured,
deliberately-left-open rough edge survives: on a completely off-topic answer specifically, the
grading model sometimes folds a content comment into the language-issue list as a fake grammar
"correction" on a sentence with no actual mistake — harmless to the learner, a real but minor
prompt-compliance gap. Carried in `SESSION_PROMPT_v83_a.md`'s own backlog, not re-derived here.
...
`# SHIPPED IN THE v81 LINE`, newest first. They were NOT copied here: they are history, and this file
is for what is open. Go there for how something was built or why a guard is shaped the way it is.*

**The ones a v82 session is most likely to need:**

| release | what, and why you might need it |
|---|---|
| `v81_d` | word colouring is graded only on questions a round can BUILD — **moved the TRACK T colouring numbers** (GREEN 18.6%→27.8%, mean questions/word 2.20→1.79); anything citing the old figures needs re-measuring, not re-citing |
| `v81_e` | `§T7` RULED: HIGHLIGHT ONLY — a wrong answer demotes green via a PARALLEL `wrong` store; the solved store stays monotonic. Reading 2 (mastery decay) is still NOT ruled |
| `v81_i` | the lesson-path SEQUENTIAL lock is removed by user ruling — the STORY GATE (`v80_b`) is now the only lock a lesson-path node carries |
| `v81_j`–`v81_l` | `PLAN §8/B1–B4` — the append-only observations log, the target-language skill registry, and shadow-mode BKT (compares against the existing gate; controls nothing) |
| `v81_m`–`v81_u` | `PLAN §C0`, the router seam — `APP.screen` is the one authoritative route state; nine screens seamed across two user rulings. **Fully done, nothing owed** |
| `v81_q` | `_renderCompStoryboard` deleted — a considered exception to "measure, don't ask": a zero-callers finding was NOT itself permission to delete, a standing prior warning meant the user had to rule on it directly |
| `v81_v`–`v81_x` | `PLAN §C5` — generation split off the landing page into its own `#generation-screen`; "🌍 home" now means the library, not generation. **Fully done, nothing owed** |
| `v81_y`–`v81_ad` | `PLAN §C4`, the Settings Card — shell (`v81_y`), global mute-pill consolidation (`v81_z`), the arrow-control acceptance detail (`v81_aa`/`v81_ab`), read-aloud icon consistency (💬 everywhere, `v81_ab`), teacher-mode consolidated to ONE instance (`v81_ab`), UI language DECOUPLED from "I speak" (`v81_ac`), and the speech-mismatch status pill (`v81_ad`). **Fully done as of `v81_ad` — nothing owed on this track** |
