# Session prompt — written at the `v79_n` cut (end of session 34)
...
zero-dependency Node language-learning app). Fresh session picking up from the **`v79_n`** cut.
...
```
node test/run.js                          → expect 217 checks
node test/run.js --quick                  → expect 193
