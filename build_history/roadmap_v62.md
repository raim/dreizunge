...
> **STATE AT HANDOFF: CUT AS `v62`; suite 114 green; check-inline 0 on both builds.**
> v62 delivers **Stage 2 of the tutor arc**: the tutor became a persistent floating widget on every
> screen, holding one continuous thread, backed by spoiler-safe retrieval over the library.
> See `v62_session1_notes.md`. **i18n owed:** ~40 en-only keys across v57–v62 (mirrored as
> placeholders) — one `translate-ui.js` run clears them.

**v61 shipped** (end-of-chapter tutor — see `v61_session1_notes.md`). This file supersedes
`roadmap_v61.md` (now historical) and carries the protocol plus every open item forward.

## ✅ v62 — persistent tutor widget + spoiler-safe retrieval (SHIPPED)
- **One floating widget** (🦉, bottom-right) on every screen; hidden without a backend. Follows the
  learner via `show()`, the single screen-switch choke point.
- **One continuous thread**, persisted in localStorage (60-turn cap), surviving navigation and reload.
- **Scope** derived from the active screen (global | storyline | chapter | lesson), shown in the
  header, sent with every turn, whitelist-validated server-side.
- **Retrieval** (`tutorRetrieveContext`): completed-chapters-only (hard spoiler guarantee), scope
  narrowing, keyword relevance + recency, hard char budget. Keyword scoring over embeddings keeps
  the zero-dep constraint. ~80k tokens of library → ~2.4k chars of context.
- Guarded by `unit-tutor` (widget/thread/scope) and the new `unit-tutor-retrieval` (spoiler
  guarantee proven against the real function).

## 🔭 Tutor arc — what's left
- **Streaming replies.** Still whole-reply with a "thinking…" state. Wants streaming through the
  zero-dep server + the v59 metering wrapper — real work, deferred deliberately.
- **Server-side learner state.** The thread and ledger live in localStorage, so they're per-device.
  Moving them server-side enables cross-device continuity (and a teacher view of a learner's
  history).
- **Per-question inline help.** A "ask the tutor about this question" affordance inside a lesson.
- **Stage 3 — concept graph (optional).** Concept ontology + `teaches:`/`prerequisites:` on lessons,
  mastery over concepts instead of chapters. Would upgrade spoiler logic from "completed chapters"
  to true prerequisite awareness, and enable cross-domain links. Real authoring/QC project.
