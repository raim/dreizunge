...
**✅ TRACK T's `§T2a` and `§T5.2` are RULED** (`v80_n`): the progress bars STAY, and tapping a word
enters the USUAL lesson flow rather than a single-question mode — which corrects `§T6` step 3 and
puts **`§0h` (question navigation) on TRACK T's critical path**.
