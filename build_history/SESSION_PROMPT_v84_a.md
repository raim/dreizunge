# Session prompt — written at the `v84_a` cut

*(Rename this file for the version the session WRAPS UP WITH — `git mv` + edit, never keep the old
one alongside. Keep using the double-letter suffix scheme (`v84_b`, `v84_c`, …) unless a future
session has a good reason to switch to `v85_a` instead.)*

I'm continuing development of Dreizunge (a single-file `index.html` client + `server.js`,
zero-dependency Node language-learning app). **This is a FRESH roadmap line** (`roadmap_v84.md`,
cut from `roadmap_v83.md` at the user's own request, "push to v84 for a fresh session," purely for
accumulated-context reasons — not a milestone cut). `roadmap_v83.md` is kept and stays the record for
everything `v83_b`…`v83_s` shipped: the whole `PLAN §7.0`/Track A migration (CP1–5, all done), the
first script that writes real lessons into `lessons.json` (`apply-cp-lessons.js`, with two real bugs
found reading its own output), and `install.sh` (the one-line installer, its default model, and its
no-auto-start + resource-check follow-ups). Read `roadmap_v83.md`'s own `# SHIPPED IN THE v83 LINE`
for how any of that was built — none of it was copied into this file.
...
2. `build_history/roadmap_v84.md` — its **index table** and **⚠️ Session protocol** block first, then
   ...
   the v83 line" for the two newest). Nothing is shipped in this file yet.
...
## My suggestion: PWA install support

Small, well-scoped, and it's the direction the user already chose (over Tauri/Electron) when asked
about the "standalone app" question at the end of the `v83` line — see `roadmap_v83.md`'s note. Add a
web app manifest + a minimal service worker to `index.html` so browsers offer "Install App," running
windowed with no tabs/omnibox. Stays entirely in the zero-dependency, single-file spirit — no new
build tooling, no new runtime dependency. Verify on both desktop Chrome/Edge (full support) and note
iOS Safari's weaker PWA support honestly rather than overclaiming it.
