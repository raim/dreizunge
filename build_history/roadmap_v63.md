...
> **STATE AT HANDOFF: CUT AS `v63`; suite 114 green; check-inline 0 on both builds.**
> v63 adds **per-question inline help**: a 🦉 in the lesson topbar that asks the tutor for a hint
> about the exercise on screen — with the correct answer structurally withheld until the learner has
> answered. See `v63_session1_notes.md`.

**v62 shipped** (persistent tutor widget + spoiler-safe retrieval, incl. v62.1/v62.2 fixes — see
`v62_session1_notes.md`). This file supersedes `roadmap_v62.md` and carries the protocol plus every
open item forward.

## ✅ v63 — per-question inline help (SHIPPED)
- 🦉 in the lesson topbar; hint before answering, explanation after; follows the exercise and flips
  mode the moment an answer is graded.
- **The answer is withheld, not merely forbidden**: three independent gates (describer omits any
  field equal to the answer; scope attaches it only when answered; server refuses it otherwise).
  Value-filtering rather than per-type rules, so new lesson types can't reintroduce a leak.
- Multiple-choice options ARE sent (the learner sees them; which is correct stays unknown).
- Guarded across five exercise shapes in `unit-tutor`.
