...
**v54 (one session + point releases) shipped:** drill lessons ("review my mistakes", v54) with the
...
- **Server:** `composeStoryboardSVG` (the SECURITY BOUNDARY — self-contained so `unit-storyboard`
  ...
  only, charset-whitelists path `d`, escapes all text, renders panels as nested `<svg>` (clips
  overflow), caps ≤5 panels/≤25 shapes, and returns `svg:null` below 2 valid panels. No `<script>`,
  `<foreignObject>`, event attribute, or `href` can be emitted (tag-set + attribute assertions).
- `generateStorylineStoryboard(topics, stories, vocab, srcLang)` beside the summary generator:
  prompt in `prompts.json` (`storylineStoryboard`), the `llm.js` salvage chain, stamped
  `buildGenMeta({type:'storyline_storyboard', model: OLLAMA_MODEL})` (STORY model, v53_d contract),
  console block logs the model. **maxTokens 6000** — the spike hit the 4096 cap and salvage repaired
  a truncated tail; headroom so the model's full intent survives.
- `/api/storyline-storyboard` — synchronous clone of the summary route; persists `sl.storyboard` +
  `sl.storyboardMeta` together; returns a plain string.
- **`llm.js`: per-call `opts.timeoutMs`** (clamped 30s–60min, honored by BOTH the socket timeout and
  the guard timer). Root cause found via the spike: the request timeout is a socket-INACTIVITY
  timeout on a `stream:false` request — the socket is silent for the entire load+generation, so the
  12-min default killed a healthy 35B run. The storyboard call passes 60min (spike measured
  **2.2 tok/s → ~30 min/storyboard** on the user's box). Also: `warmup()` now reports the real error
  instead of mislabelling everything "timed out".
- **Client:** 🎬 button next to 📝 (gated `canGenerate` AND ≥2 chapters), localized title,
  `genStorylineStoryboard()` (⏳ dance; injects the server-validated SVG string only), storyboard
...
- Tests: `unit-storyboard` (whitelist/clamp/escape mutation-tested, panel floor, stamp, route,
  v55_b failure-logging assertions,
  contract, llm.js timeout override). Suite 98 green, check-inline 0 on both builds.
**Owed:** LIVE-TEST **§79 + §79b** (browser+Ollama, incl. the explicit injection check), the TranslateGemma
pass debt grows by **3 en-only keys**: `storyboard.gen`, `storyboard.title`, `storyboard.empty`.

**Still open (deliberately):**
- **⚠ FIRST LIVE RUN CRASHED (2026-07-13) — cause unconfirmed, evidence owed (user).** A 9-chapter
  de run on `qwen3.6:35b-a3b` died with an empty `ollama ps` afterwards; with `keep_alive:-1` that
  means Ollama or its runner DIED (models never self-unload). Prime suspect: memory — every model
  touched since boot stays resident (7B warmed at startup + the 35B on top) and the 9-chapter
  prompt + `num_predict 6000` grows the KV cache. **v55_b shipped the observability** (response
  arrival, raw-excerpt-on-garbage, and route-catch failure logging with elapsed — a 30-min call now
  always leaves a server-side corpse; `unit-storyboard` guards it). The crash itself needs the
  user's `journalctl -u ollama` / `dmesg | grep -i oom` from the run's window. If OOM confirmed,
  candidate fixes: release the previous model on picker switch, finite keep_alive, or restart
  Ollama before storyboard runs. The summary route shares the silent catch — extend the logging
  there if it ever bites. **Run 2 was a DIFFERENT failure, now FIXED (v55_c):** the thinking model
  burned all 6000 tokens in its reasoning block → empty `content` → "empty response". The
  storyboard call now passes `think:false` (with an automatic parameter-less retry for models that
  reject it). Shared hazard note: OTHER generators (lessons, summary, QC) on a thinking story model
  can burn budget the same way — the `opts.think` plumbing is generic; add it per call site if it
  bites (LIVE-TEST §79c).
- **Job/polling pattern** for the route if the ~30-min synchronous wait proves unacceptable in
  practice (the lesson generator's job pattern is the template). The roadmap decided sync-clone;
  the per-call timeout makes it survivable. Revisit after live use.
- Regenerate replaces; **delete shipped in v55_e** — the 🎬 button toggles (generate when empty,
  regenerate/delete menu when a storyboard exists) via `showChoiceDialog` +
  `DELETE /api/storyline-storyboard` (backend-free, idempotent). No offline delete in the static
  build (button gated on `canGenerate`); baked storyboards are read-only there by design.
- Panel quality tuning (more few-shot scene grammar in the prompt; larger models) — the current
  output is deliberately "crude but recognisable".
- **Captions are hover-only `<title>` tooltips (v55_d)** — no always-on text band, no caption on
  touch devices (user's call). If a mobile-visible caption is ever wanted, that's an HTML-overlay
  change to how the SVG string is injected (bigger blast radius than the composer), not a
  composer tweak.
...
**Version note:** v55 is a MINOR/feature cut. Post-v55 point releases are numbered v55_b, v55_c, …
(per user). At the next MINOR/feature cut, resume vNN numbering (v56) and write `roadmap_v56.md`.
