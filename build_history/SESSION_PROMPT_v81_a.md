# Session prompt — written at the `v81_a` cut (end of session 36)
...
- **Entry cards for chapters > 1** — the user asked to skip them and jump to the chapter's progress
  card. **This partly reverses `v80_e`**, whose guard `unit-next-chapter-entry` asserts the opposite
  in six sections. Its own release, with a careful rewrite of that file — not a quick edit.
