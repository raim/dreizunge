...
### ⭐ CAN THE GENERIC PATH WORK AFTER ALL? MEASURED END TO END AT THE `v90_r` CUT — ANSWER: ONLY WITH A REVIEW STOP, AND NEVER AUTOMATICALLY

Asked after `v90_q` measured that **`articleBody` is rare** (4 of 11 titles). Three candidate rescues
were probed in order, cheapest first. **The first two are dead; the third works but is expensive and
imperfect. No code was written.**

#### 1. Microdata / RDFa — DEAD. 0 for 4.

The obvious cheap extension: same schema.org data, older encoding. Probed on the four titles that
carry `NewsArticle` with no `articleBody`:

| | `itemprop="articleBody"` | `property="articleBody"` | `itemtype=schema.org/Article` |
|---|---|---|---|
| Guardian, BBC, Spiegel, Al Jazeera | **absent** | **absent** | **no** |

**A publisher who omits `articleBody` is not publishing the full text in an older encoding — they are
not publishing it as structured data at all.** Do not spend a session on this.

#### 2. The cheap no-DOM heuristic — DEAD, and wrong in BOTH directions

The text IS reachable: it is server-rendered in `<p>` tags (Guardian 1201w, BBC 903w, Al Jazeera AMP
906w). Only Spiegel is thin (148w — paywalled or client-rendered). So this is **not** a JavaScript
problem; the content is present and simply unmarked.

The obvious heuristic — article paragraphs are contiguous, furniture is scattered, so take the
longest run — was **validated against the two articles where JSON-LD supplies ground truth**:

| | true | heuristic | error |
|---|---|---|---|
| Corriere | 499w | 669w | **1.34× — 34% over** |
| NOS | 677w | 321w | **0.47× — misses half** |

⚠️ **Wrong in both directions, and no gap parameter fixes both.** Its first "paragraph" on Corriere is
*"Salva questo articolo e leggilo quando vuoi…"* — a subscription prompt presented as the story's
opening line. Real Readability is per-node scoring over a parsed DOM (link density, class-name
penalties, sibling merging) and needs an HTML parser this repo does not have.

#### 3. ⭐ THE EXISTING LLM CLEANER — WORKS, at a price. Measured on the live model.

`cleanNarrativeText` / `POST /api/clean-text` (the ✨ *"Remove ads & boilerplate"* button) already
does this class of work. Run over the Corriere page's raw `<p>` mass against the 499-word ground truth:

| | |
|---|---|
| input | 898 words, **44% furniture** |
| dropped | **236 words** |
| **recall** (article recovered) | **22/22 sentences = 100%** |
| **precision** (output really is the article) | **22/28 = 79%** |
| commercial furniture (Abbonati / newsletter / Facebook / "Salva questo articolo") | **all gone** |
| **cost** | ⚠️ **262 seconds** — 4.4 min at ~470% CPU, 2,786 tokens, for ONE article |

⚠️⚠️ **WHAT SURVIVED IS THE FINDING, NOT WHAT WAS REMOVED.** All six stray sentences are
**cross-article teasers** — other stories' headlines and bylines from the sidebar:

```
1. "La mamma di Lorena Isceri: «Lei scoprì che Federico Vella aveva un'altra donna…»"
5. "di Lorenzo Padoan / Accordo De Gasperi-Gruber: la storia del patto…"
6. "Serata impegnativa i vigili del fuoco… precipitazioni"
```

**And number 1 is the FIRST LINE of the output** — a chapter generated from this would open on an
unrelated murder.

**The cleaner is good at COMMERCIAL furniture and blind to EDITORIAL furniture**, and the reason is
structural, not a prompt defect: an ad does not look like news, whereas another newspaper story is a
newspaper story. This is exactly the class `analysis-corrections.js`'s own header already flagged —
*"an inline unrelated-article teaser that reads as a fragment no rule can classify"*.

⚠️ **DO NOT TRY TO FIX THIS BY PROMPT-TUNING.** Rule 11 applies before the first attempt, not after
the second: the distinguishing signal between an article sentence and a teaser is **position in the
DOM**, which is information the cleaner never receives. Two rounds of prompt work would measure zero
for a reason already known.

#### The ruling this points to

- **Automatic: NO.** 4.4 minutes and ~470% CPU per fetch, on hardware where `v90_o` exists precisely
  because an automatic post-pass competing with generation cost a chapter — and the failure is not
  benign, since the first line is another story.
- **Opt-in behind the refusal message: PLAUSIBLE.** 6 strays in 28 sentences is genuinely two clicks
  in the review card, against 44% furniture scattered through the raw `<p>` mass. That is the same
  argument that justified the review stop in the first place. The honest shape is a button ON the
  "no article data here" message — *"try extracting it anyway (slow, needs review)"* — so both the
  cost and the review are the user's explicit choice.
- **Default scope is unchanged**: structured data, or an honest refusal. `v90_q`'s Wikipedia path
  widened that far more cheaply than any of the three rescues above.
