# Session prompt — written at the `v90_q` cut
...
zero-dependency Node language-learning app). Picking up from **`v90_q`**. `roadmap_v90.md` was cut at
...
```
node test/run.js                          → expect 372 checks
node test/run.js --quick                  → expect 306
...
snapshot; re-measure fresh at commit time. `APP_VERSION = 'v90_q'`.
