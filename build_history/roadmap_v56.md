...
`v*_session*_notes.md`.

**v55 (one long session) shipped:** the SVG chapter storyboard (v55, browser-verified) with delete,
hover-caption tooltips, colour schemes + story-style-aware art, and a fixed-canvas layout; the whole
QC arc (story QC → bulk story QC in the storyline sweep → per-sentence accept → summary QC, all
sharing one classifier/renderer/reconstruction); the translation provenance stamp (completing the
story+translation+storyboard provenance set); collapsible model selection; and a run of live-found
fixes (llm.js per-call timeout + think:false, module-scope/import guards, live↔static parity).
Details: `roadmap_v55.md` + `v55_session1_notes.md`.

## ✅ v55_v — user TODO triage, small items (SHIPPED)
Done immediately from the 2026-07-14 list; the rest of that list is the OPEN block below.
- **Storyboard console names its storyline** — the generation header now prints
  `Storyline: "<title>" [sl_…]` above the Chapters/Model line (a sweep logged identical blocks
  before, so you couldn't tell which board was running or which failed).
- **Storyboard for SINGLE-chapter storylines** — the 🎬 gate required ≥2 chapters, which was simply
  wrong: panels are SCENES, not chapters, so a one-chapter story has an arc to board. Gate is now
  ≥1; the composer's own ≥2-PANEL floor remains the real constraint. (e.g. "Yusuf and the Lost Cat")
- **Static: the "Backend: Static — built-in lessons only" row is hidden** — no backend to report and
  nothing to click there (no model picker); it was pure noise. LIVE-TEST §89.

## ▶ NEXT BUILD — the 2026-07-14 TODO list (user), largest first

### 1. OPEN — clickable storyboard panels → lesson sets (BIG; do the design first)
Clicking a panel opens that chapter's lesson questions. The hard part is not the click — it's the
**panel↔chapter mapping when the counts differ**, so settle that before coding:
- **Panel budget changes:** max panels **5 → 6**. For large storylines (**>10 chapters**) allow up to
  **12** panels, but keep **6 panels per row** on the main page and chapter displays (so a 12-panel
  board wraps to two rows of 6). NOTE this breaks the v55_t fixed-canvas assumption (one row of
  MAX_PANELS) — the canvas must become rows×6, and `unit-storyboard`'s fixed-canvas/centring test
  must be reworked, not deleted.
- **Fewer panels than chapters** (the common case): the model must tell us, per panel, **which lesson
  set starts with it** — i.e. the prompt gains a per-panel `chapter` field (1-based index), which the
  composer must whitelist/clamp exactly like every other model-supplied value (it is untrusted).
- **More panels than chapters:** links simply repeat (several panels → the same lesson set).
- **Locking:** non-teacher mode locks stories, so panel links must be locked the same way — the
  student runs through the underlying lessons in order, but clicking a panel again **resumes where
  they left off** (reuse the existing progress/resume machinery, don't invent a second one).
- **Lesson-set pages:** show the board above the back/forward nav, with a **frame around the panel(s)
  belonging to this chapter**.
- Re-colouring is free (stored panels, v55_r) — the same applies to re-composing for a new layout, so
  existing boards can gain links/rows without a model call IF the panel JSON already carries the
  chapter field. It won't, so boards predating this need one regenerate (mirror the v55_r 409).

### 2. OPEN — provenance fields: sources + generating user (MEDIUM-LARGE)
Extends the existing "text provenance" item below (they are the same work — merge them).
- Today only a **source file name** is stored (PDF-generated stories, `storyMeta.sourceFile`). Need
  general, user-editable provenance: **URLs, DOIs**, free-text citation, etc.
- Show provenance **above the generation stats, in the same style** (chapter/lesson pages).
- **User login is coming**, so add a field for the **generating user**. Anything without one —
  including all existing stories and lesson sets — reads **"admin"** (a backfill like
  `backfill-provenance.js`, or a read-time default; prefer the backfill so the data is explicit).
- Add a **prominent one-liner** to BOTH the main-page "saved lessons" story display AND the storyline
  page (**above the storyboard**): `by <user>` · `from: <source>`. Both can be links — the `<user>`
  link only becomes meaningful once login exists, so ship it inert.
- Remember the live↔static split: the landing card renders in TWO places (index.html's
  `loadSavedList` AND build-static.js's) — v55_p was exactly this trap; `unit-static-landing-parity`
  guards it, and its hook list must gain the provenance marker.
