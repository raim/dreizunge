...
> **STATE AT HANDOFF (2026-07-20): CUT AS `v59`; suite 107 green; check-inline 0 on both builds.**
> v59 is one feature session on top of v58: **cumulative token usage** (was item 3) — every
> post-generation LLM call (summaries, storyboards, titles, all QC flavors, added/recreated
> lessons) now adds to the owning artefact's totals instead of being invisible.
> **i18n owed:** the pending translate pass covers **12 en-only keys**
> (`storyboard.locked_resume`, nine `prov.*`, `tokens.storyline`, `tokens.initial`) × 29
> languages — one `translate-ui.js` run clears the lot.

**v58 shipped** (provenance fields — see `v58_session1_notes.md`). This file supersedes
`roadmap_v58.md` (now historical) and carries the session protocol plus every still-open item forward.

## ✅ v59 — cumulative token usage across all LLM calls (SHIPPED; was item 3)
Full details in `v59_session1_notes.md`. Highlights:
- **One choke point, not per-site plumbing:** every server-side LLM call converges on the
  `_callLLM` wrapper around the llm.js import; an AsyncLocalStorage meter (node builtin — the
  zero-dep constraint holds) scopes attribution per async context, so concurrent jobs (a
  background QC sweep + a foreground summary) can never cross-attribute. `meterLLMTokens(fn)`
  → `{ result, tokens }`; the routes attribute. The initial generation pipeline is deliberately
  NOT wrapped — it self-accumulates, wrapping would double-count.
- **Attribution decision (the item demanded one):** storyline-level artefacts — summary,
  storyboard, retitle (one call covers the chain), summary-QC — go to the STORYLINE's own
  `sl.tokenUsage`, not chapter 1 and not spread. Chapter-scoped work — story-QC (route and
  sweep), lesson-QC sweeps, add-lesson, per-chapter recreate — folds into the chapter's
  existing `generationStats.totalPromptTokens/totalCompletionTokens`, so "total" finally means
  total. Both keep a compact `tokensByType` tally.
- 12 attribution sites wired, including BOTH summary call sites (the route and the batch
  post-pass — the v53_c stamp test caught the second one) and both batch title steps. A QC
  sweep that comes back all-clean still persists its accounting (nonzero tokens mark the topic
  touched). `totalMs` is untouched — it still means generation time.
- Display: the storyline screen shows the storyline bucket as its own stats line; the chapter
  gen-stats line gains a breakdown tooltip (initial generation + per-type). The v55_s 4-field
  projection keeps its field names — values just grow.
- Guarded by `unit-token-usage` (BEHAVIORAL: the real extracted meter run under interleaved
  concurrent scopes; addTokenUsage semantics; 12 wiring contracts; display + projection).
  Three QC sandbox tests gained meter stubs; the storyboard/summary-stamp call-site guards
  were updated to the metered shapes.
