...
```
node test/run.js                          → expect 227 checks
node test/run.js --quick                  → expect 203
...
Corpus at this cut: **324 topics, 91 storylines, 33 languages, 618 `en` keys**.
`APP_VERSION = 'v80_w'`.
...
**✅ `§0h` question navigation SHIPPED at `v80_p`** — TRACK T's step 3 dependency is discharged.
**Owed: a device pass on an ORDERING lesson and a SYNONYM lesson** — the ledger restores those shapes
but no test fixture drives them.

**✅ TRACK T step 2 COMPLETE (`v80_s`).** The story panel is on EVERY question card and starts
COLLAPSED where the story gives the answer away (word_forms 60.4%, error_hunt 93.6%, synonyms 41.2%,
and the typed kinds, which leak the spelling). Three-state colouring and the asked-span underline are
in. **⚠️ SIX ITEMS OPEN from the user's device pass (`v80_u`)** — in the roadmap's `v80_u` entry.
**Two done at `v80_v`** (live refresh after answering; sentences counting toward the counter, scoped
to words the chapter TEACHES). **Two more done at `v80_w`** (the story on ALL progress cards, and those cards sharing the ONE body
renderer `_storyBodyHtml` — which REVERSED `v77_p`). **One still open:** the vocab list under the
text should show only words NOT in the story, and may include solved synonyms/antonyms. **The sixth is DEFERRED by the user
and written up as TRACK T `§T7`: "a wrong answer should decrease the solved counter" turns the
MONOTONIC solved store into a fluctuating one**, and coverage, chapter completion, the pass mark,
`storyUnlocked` and `_firstCoverageShortLessonIdx` all read it — a chapter could become incomplete
again and a story could RE-LOCK. It needs its own release and a scoping decision.

**Step 3 SHIPPED at `v80_t`** — `tapWord()` resolves a word to a (lesson, question) and enters the
usual run. **Next is step 4**, the gate change, which lands on the `v80_b` code.

**⚠️ `buildExercises` is NON-DETERMINISTIC IN CONTENT, not just order** (`v80_t`). The set of
questions a run holds for a given word differs between builds. **Any test that samples the corpus for
a fixture must accumulate across several builds** — asserting on one made `unit-tap-word` flaky
through six attempts, every failure being the TEST wrong about correct behaviour. Verify a new
corpus-sampling guard with ~15 consecutive runs, not one.

**✅ TRACK T IS FULLY UNBLOCKED — every `§T5` question is settled and `§T6` is buildable.**
`T5.1` measured: green = ALL (mean 1.70 questions per word). `T5.2` ruled: tapping enters the USUAL
lesson flow, not a single-question mode — which corrects `§T6` step 3 and puts **`§0h` (question
navigation) on TRACK T's critical path**. `T2a`/`T5.3` ruled: the progress bars STAY. `T5.4` ruled:
**ACCEPT the mostly-red screen** — red means "not done", and with the bars kept the text is a
secondary display. Only `§T6` step 5 (prompt-side mapping) still needs the user, because it needs a
live model.

**⚠️ Do not re-open `T5.4` by accident.** 84% red is UNFINISHED WORK, not a display artefact. If the
screen should be greener the levers are upstream — chapter completion, the 6% token-highlight
density, and the 47% vocabulary matchability, the last of which is a GENERATION fix.
...
- `build_history/probe_lesson_script_v80h.js` — **new.** Lessons carrying none of their chapter's
  script. **0 of 94 at this cut — the corpus is CLEAN** (was 2 at `v80_m`; and 7 was wrong, see
  `v80_m`). Previously 2 of 95 (was reported as 7 at `v80_h`; four were comprehension lessons and
