...
> **STATE AT HANDOFF (2026-07-20): CUT AS `v60`; suite 108 green; check-inline 0 on both builds.**
> v60 is a learner-UX session on top of v59: **the lesson-set page is now teacher-only**, learners
> skip it and go straight into the questions; the **completion card was redesigned** to Next + Back
> with a progress summary and the full chapter story on unlock.
> **i18n owed:** the pending translate pass now also covers **6 new `complete.*` keys** (en-only),
> on top of the 12 from v57–v59 — one `translate-ui.js` run clears everything.

**v59 shipped** (cumulative token usage — see `v59_session1_notes.md`). This file supersedes
`roadmap_v59.md` (now historical) and carries the session protocol plus every still-open item forward.

## 🔧 v60.1–v60.8 — same-session amendments (all SHIPPED, still pre-deploy under v60)
Recorded in `v60_session1_notes.md`. In order:
- **v60.1** learner = teacher-mode-off (not `_canEdit`); a re-opened complete chapter shows a review card.
- **v60.2** mixed-driven replay loop + frozen progress bar fixed (resume/Next are coverage-aware).
- **v60.3** quitting a question (✕) respects the learner split (→ story/landing, not the lesson-set page).
- **v60.4** story-progress row counts mixed chapters correctly (was 0/N).
- **v60.5** story/meta generation `think:false` (reasoning-model empty-response fix).
- **v60.6** story-translation toggle in the read-story panels (uses the stored `storyTranslation`).
- **v60.7** per-role reasoning toggle (story/lessons, model menu, off by default, auto-bumps tokens+timeout).
- **v60.8** clean completion card (no trophy/%-pill/stat panels; +%-solved bar) & global teacher
  pass-mark threshold (model menu, gates chapter completion, offers the drill below the bar).

## ✅ v60 — learner UI: skip the lesson-set page; redesign the completion card (SHIPPED)
Full details in `v60_session1_notes.md`. Design decisions were confirmed with the user before build:
- **Learner routing:** `loadSaved` branches on `_isLearner()` (= `!APP._teacherMode` — v60.1 fix;
  was `!_canEdit()`, which wrongly kept backend users out of the learner flow). A complete chapter
  a learner re-opens shows a review completion card, not the editor. A learner never
  sees the lesson-set/chapter page — `goLessonSet` still runs the language/dir setup + hash, then
  the learner resumes at the first UNFINISHED counted lesson (`_firstUnfinishedLessonIdx`) and
  auto-starts it. Teachers keep the page as their editing hub. Mirrored in build-static's own
  `loadSaved` override (parity).
- **Completion card = exactly two buttons.** Next opens the next questions directly: next lesson
  in this chapter → else the next chapter's first unfinished lesson (via `loadSaved`, which
  resumes it) → else Next hides and Back is the only forward action. Back → the storyline deck
  (`openStorylineScreen`), or the landing page for a solo chapter. The old drill button and full
  nav pills are now **teacher-only**; the learner card is clean.
- **Card body:** a progress summary (`_compProgressHtml`) showing position ALONG the storyline
  (chapters done/total) and WITHIN the chapter (lessons done/total); on chapter completion the
  **full** chapter story is shown (was a 200-char teaser — the teaser now only appears in the
  teacher "peek before complete" case).
- Guarded by the new `unit-learner-nav` (helper logic, loadSaved routing live+static, card
  structure, full-story-on-unlock, progress summary); three prior tests (drill ×1 label/gate,
  intro-script Next label) were reworked to the redesigned card.
