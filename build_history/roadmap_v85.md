# Dreizunge roadmap — v85

**This is the `v85` line.** Cut from `roadmap_v84.md` at its last commit (the `v84_n` release plus
one code-free roadmap-notes commit right after it), at the user's own explicit request ("let's cut to
v85 for this, and perhaps we start the next steps in a new session") — not a milestone-completion cut,
the same shape as every prior cut for the same reason: the `v84` line shipped PWA install support
(`v84_b`) and the `dreizunge` PATH launcher (`v84_c`), a full mobile-UI follow-up arc (`v84_d`/`v84_e`,
real-device driven), an orphaned `v83_g`-collision fix (`v84_f`), the WHOLE browser-native
speech-recognition arc end to end (`v84_g`…`v84_m` — typed-answer + MCQ checking, then continuous
listening to cut Android's un-suppressible start/stop tone, a mute toggle, a listening-dots animation,
interim results, and a universal floating "heard" pill, each release driven by the user's own real
phone testing), and Windows install docs (`v84_n`, Tier 1 only, by the user's own explicit pick).

**`roadmap_v84.md` is kept and is not superseded as a record** — the whole `v84` line's release
history (`v84_a`…`v84_n`, fourteen point releases) lives there under `# SHIPPED IN THE v84 LINE` and
was NOT copied here. Go there for how something was built or why a guard is shaped the way it is; this
file stays current through the whole v85 line.

> **⚠️ WHAT WAS CARRIED, AND WHAT WAS NOT.** Carried: this protocol block, the findings that govern
> the open sections, `§0`/`§0i` with their reconciliation, the standing RULES (now including three new
> ones earned in the `v84` line — see below), **TRACK T** (unchanged — still nothing open) and **THE
> LARGER PLAN** (the folded `implementation_plan.md`) — plus a NEW plan item, **`PLAN §13`**, added at
> this cut for the generator-page redesign the user proposed and this session assessed/scoped (see
> below; the assessment itself is not repeated here — read `PLAN §13`'s own text for what's in/out of
> scope and why). Not carried: the `v84` release entries, and the `v84_n` SESSION_PROMPT's own
> "buildable now" backlog — that carries forward through the SESSION_PROMPT lineage instead (see
> `SESSION_PROMPT_v85_a.md`), the same way it always has; this file has never been where per-cut
> backlog lives. Nothing open was dropped — the cut is by KIND, not by judgement about what still
> matters, so no item had to be assessed to survive it. This is exactly the `v83`→`v84` cut's own
> carry/drop split, repeated verbatim — see that file's own header if the reasoning is unclear.
>
> **Also carried, informally**: the `v83`-line discussion-only note on mic-based spoken-reply
> recording/comparison (superseded in spirit by the `v84_g`…`v84_m` speech-recognition arc, which
> answered "can we use the phone's own recognition" — yes — but the ORIGINAL note's broader
> pronunciation-scoring question is still open and still "not required for now"); Windows Tier 2 (a
> real `winget`+PowerShell installer) — Tier 1 shipped at `v84_n`, Tier 2 remains discussion-only, not
> ruled out, not queued; and `apply-cp-lessons.js`'s `v83_p` re-verification, ATTEMPTED but not
> completed at this cut (blocked by real machine resource contention, not a code problem — see
> `roadmap_v83.md`'s own addendum note, dated at the `v84_n` cut, for the concrete numbers) — still
> open, still worth doing once the machine has headroom.
...
7. A version bump to a new BASE needs its own roadmap. **This is that roadmap for `v85`.**
...
9. **A live model call needs a live test, not just a plausible prompt.** Confirmed again in the `v84`
   line: the browser-native speech-recognition arc (`v84_g`…`v84_m`) shipped several real bugs found
   only by rendering the actual page and reading a computed style/DOM state, never by reasoning about
   the code — budget for that, not zero, on anything touching a browser API or a live model.
10. **Ask before restarting a dev server you did not start, and before deleting data you did not
    create** — still true; the `v84` line's own instance was checking `git status --short lessons.json`
...
6. **Version** — bump `APP_VERSION` in `server.js` if it's a new release. NOTE (v49): the static
   ...
   **Point releases use an alphabetic suffix** (user, v70): the base cut is the bare number and is
   implicitly `a`, so the sequence is `v77` → `v77_b` → `v77_c` → … — the same convention the v69–v84
   lines ran. **This is the `v85` line.** Roadmaps are per BASE version, so point
   releases do not each get one — this file stays current through the whole v85 line.
