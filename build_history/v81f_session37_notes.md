...
## `PLAN §C1` / `§0.3` — measured, NOT implemented, one ruling owed

The roadmap said "first move is a probe, not an edit", so that is what this was. Finding: the
single-chapter `1/1` and the partly-green header bar are ONE root cause, and it is **not** the index
off-by-one the plan guessed — `_slProgressStats` computes `pct` from `unlockedChapters`, so the bar
measures how much of the deck is OPEN rather than DONE. All 91 storylines show a partly-green bar
before anything is played; all 27 single-chapter decks read 1/1 at 100%.

The `+1` behind it is the `v77_p` USER RULING, so it cannot simply be deleted. The question to answer
is in the roadmap's `PLAN §C1` block: split the LABEL (unlocked) from the BAR (completed), or leave
it. Nothing was changed.

## What is owed after this session

- **A device pass on `v81_c` / `v81_d` / `v81_e`**, per the two checklists above. Both changes are on screens
