# Session prompt — written at the `v90_p` cut
...
zero-dependency Node language-learning app). Picking up from **`v90_p`**. `roadmap_v90.md` was cut at
...
```
node test/run.js                          → expect 370 checks
node test/run.js --quick                  → expect 305
...
snapshot; re-measure fresh at commit time. `APP_VERSION = 'v90_p'`.
