...
// APP_VERSION from server.js, (b) it interpolates that value into info.version rather than a
// literal, and (c) a freshly built docs/index.html carries exactly the server's current version.
// v85_t: the tagline-tooltip half of this (b)/(c) is gone — the version is no longer surfaced in
// the UI at all (see unit-app-motto.test.js). info.version itself is untouched, still derived,
// still worth guarding — that is everything below.
'use strict';
...
assert.ok(!/version: 'v\d+'/.test(build), 'no hardcoded version literal remains in the injected static init');

// The built artifact reflects the server version.
