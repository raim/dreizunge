...
// arrives unstamped, and the fix is to run `node backfill-script.js --write`.
{
  ...
  // v79_a, CORRECTED by the user: this is an EXPECTED side-effect of `reinforce`, not a defect.
  //
  // `tp_17863746762340000193` is a Cyrillic Serbian chapter whose second vocabulary lesson carries
  // Latin words (`reka`, `sanjati`, `vetar`). Its `_genMeta` says `_arcMode: "reinforce"` — the mode
  // that explicitly asks the model to re-train vocabulary from EARLIER chapters, and those chapters
  // were Latin because the user was deliberately switching this storyline from Latin to Cyrillic.
  // The Latin is the prior vocabulary, faithfully reproduced. The plain lesson in the same chapter,
  // generated by the same builder minutes earlier, came out in correct Cyrillic.
  //
  // So a mixed chapter is what a script-SWITCHING storyline looks like while `reinforce` is on, and
  // the guard's "the generator was never told which script to use" message is wrong for this case.
  ...
  assert.deepStrictEqual(_unexpected, [],
    `no topic mixes its language's two scripts — ${JSON.stringify(_unexpected)} do, and a mixed `
    + 'passage means the generator was never told which script to use. Inspect before stamping.');
  assert.ok(ambiguous <= EXPECTED_MIXED.length,
    `at most the expected reinforce-mixed chapters are ambiguous (got ${ambiguous})`);
  console.log(`  backfill-script: ${already} stamped, 0 outstanding, ${ambiguous} known-mixed`);
}
