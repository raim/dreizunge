...
run('unit: provenance is edited on the storyline and inherited by chapters (v89_aj)', 'node', [path.join(__dirname, 'unit-provenance-inherit.test.js')]);
...
run('unit: dialect TTS approximate detection (M1)', 'node', [path.join(__dirname, 'unit-dialect-tts.test.js')]);
  ...
  run('unit: the one-field generation input router (v90_s)', 'node', [path.join(__dirname, 'unit-gen-input-router.test.js')]);
  run('unit: the model pill in the bottom bar (v90_w)', 'node', [path.join(__dirname, 'unit-bottom-bar-model.test.js')]);
  run('unit: bottom-bar toggles + backdrop close (v90_x)', 'node', [path.join(__dirname, 'unit-bar-toggles.test.js')]);
  run('unit: UI language from the OS locale (v90_y)', 'node', [path.join(__dirname, 'unit-ui-lang-os.test.js')]);
...
if (!quick) {
  ...
  run('e2e: the last five blocking LLM routes are listed, cancellable jobs (v88_al)', 'node', [path.join(__dirname, 'e2e-sync-routes-as-jobs.test.js')]);
