...
// lesson 2: two flagged + one plain → expect the plain + exactly one flagged (fill to 2)
const lessons = [
...
// lesson 1: only the two rated, flagged + plain excluded
const l1 = from(1);
assert.strictEqual(l1.length, 2, 'perType=2 picks 2 from lesson 1, got ' + l1.length);
assert.deepStrictEqual(l1.slice().sort(), ['rated1', 'rated2'], 'lesson 1: rated picked over plain/flagged, got ' + JSON.stringify(l1));

// lesson 2: the plain one + exactly one flagged (flagged used only to fill perType)
const l2 = from(2);
assert.strictEqual(l2.length, 2, 'perType=2 picks 2 from lesson 2, got ' + l2.length);
assert.ok(l2.includes('ok1'), 'lesson 2: the non-flagged item is included');
const flaggedPicked = l2.filter(t => t === 'f1' || t === 'f2');
assert.strictEqual(flaggedPicked.length, 1, 'lesson 2: exactly one flagged item used to fill the minimum, got ' + JSON.stringify(l2));
