...
| **card contents, all UNCHANGED internally** | `#gen-card-1` = `.lang-box` (source/target language + script pickers). `#gen-card-2` = the text-source cluster: `#topic-label`/`#topic-input`, `#user-story-checks`, `#pdf-panel`, `#user-story-panel`, `#dialect-panel` — every `onchange`/`onclick` (`onUseStoryCb`/`onUseDialectCb`/`pdfGenerateAll`/`doDialectImport`/etc.) byte-for-byte unchanged. `#gen-card-3` = a CATCH-ALL: `#gen-form-section` (chaptering/style/continue/lesson-type/diff/format) + `#gen-btn-row` (the pre-existing "Generate lessons →" button) — this is milestones 2-5's own future territory; splitting it further is exactly what those releases do, nothing here anticipates it |
| **navigation** | `_genWizardGoto(n)` (clamped `[1,3]` via `Math.max`/`Math.min`) sets `_genWizardStep`, toggles each `#gen-card-N`'s `style.display` (`''`/`'none'`) and each `#gen-step-pill-N`'s `.active` class. `_genWizardNext()`/`_genWizardBack()` are thin ±1 wrappers. Each card's own Back/Next buttons (`#gen-step-back-2`/`#gen-step-next-1`/etc.) are NEW, small, wizard-only — placed ADJACENT to the unchanged markup, never inserted inside an existing id'd container (`#gen-btn-row` in particular stays untouched) |
| **the stepper** | `#gen-wizard-steps` reuses the EXISTING `.pdf-stepper`/`.pdf-step`/`.pdf-step.active` classes verbatim (until now only `#pdf-stepper`'s own per-chunk PDF-import progress pills) — zero new CSS, same visual language at a new, page level. 3 pills, each `onclick="_genWizardGoto(n)"` for a free/ungated direct jump — no validation gates a step |
| **the reset hook** | `show(id)` — the one authoritative route function (`PLAN §C0`) — now also runs `if(id==='generation-screen') _genWizardGoto(1)`, so `goLanding()`/`goLandingClean()` (its only two callers of `show('generation-screen')`) always re-enter at card 1, even if a learner was left deep in the wizard. Mutation-tested: removing the hook is what `test/unit-gen-wizard.test.js` §5 catches |
...
| new `ui.json` (`en` only) | `gen.wizard_step1`/`step2`/`step3`, `gen.wizard_back`, `gen.wizard_next` (662→667 `en` keys) — wired via `_setText`/`t()` in `applyUIStrings()`, with matching static English fallback text in the raw HTML (same convention `#topic-label`/`#gen-screen-title` already use, so there's no flash-of-empty-pill before `applyUIStrings()` runs) |
| the acceptance tests | `test/unit-gen-wizard.test.js` (6 checks): markup nesting (each existing block inside its own card, via `indexOf(needle, lo)` bounded search — NOT unbounded `indexOf`, which would find `.lang-box`'s OTHER pre-existing user, `#lib-lang-box` in the library filter, instead), default state (card 1 shown, pill 1 only active), `_genWizardNext`/`_genWizardBack` clamping at both ends, `_genWizardGoto` direct jump, the `show()` reset hook (mutation-tested), and the `#gen-area`-untouched invariant |
| ⚠️ **div-balance verification method, worth reusing** | after the restructuring edits, a real HTML parser (Python's `html.parser`, stack-tracking start/end tags) was run over just the `#generation-screen`…next-screen region to confirm zero stray/missing closing tags — the same technique `PLAN §C5`'s own `landing`-nesting row used, now proven useful again for a wrap-existing-markup edit specifically |
