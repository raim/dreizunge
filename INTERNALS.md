...
| ⚠️ **the "create storyline now, add lessons later" shortcut is OPEN, not deferred-and-forgotten** | see `roadmap_v85.md`'s own `v85_d` entry for the investigation and the two candidate resolutions (real server-side "zero lessons" scoping vs. a narrower "skip the arc, standard set only, add more later via the EXISTING storyline-screen add-lessons flow" reinterpretation) — a ruling is needed before either gets built |
