...
| **client** | `#comic-extract-btn` → `#comic-extract-cb` + `#comic-describe-cb` + `#comic-generate-btn` (defaults: text ON, description OFF). `_comicSyncGenerateBtn()` DISABLES Generate while neither is ticked — clearer than a toast and costs no third `ui.json` string. `_comicPanelText()` is the ONE place the fallback lives (`extracted \|\| description`), which makes it true at once for the no-text filter, both panel summaries, `comicOpenReview`'s editable set and `_genChapterCount` (`v87_h`). The review card gained an editable description field, shown only when the panel has one |
