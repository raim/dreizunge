...
| **client view: comic panels** | `_comicPanelImageStripHtml(d)` (`v86_q`) — an IMAGE-ONLY strip (no per-panel caption pairing, unlike `_comicStoryPanelsHtml`), prepended in BOTH `_storyBodyHtml`'s `highlight:false` (translation) branch and `_textExplorerBodyHtml` — neither view has per-panel TEXT to pair an image with (translation is one flat string; CP2 has no panel-boundary awareness at all) |
