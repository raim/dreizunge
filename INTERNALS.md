...
| **`_comicExtractPrompt(langName)`** | generalized across all 33 target languages, not hardcoded to German (this session's only actual measurement) — two things phrased CONDITIONALLY rather than assumed universal: case-restoration only applies to scripts that HAVE case at all (the MODEL decides that, per `PLAN §2`'s own "the app does not encode per-language grammar" principle); German's "capitalize every common noun" rule is named as an EXAMPLE of a per-language rule, not asserted universally. The LITERAL worked example that fixed German case-restoration in the probe was deliberately NOT carried over (risk of biasing other languages toward German's pattern) — **live-verified finding (real `qwen2.5vl:7b` call): case-restoration did NOT fire on a synthetic test panel**, plausibly because plain rendered text doesn't read as "comic lettering" the way real panel art does; not yet re-verified against a real panel with this exact (no-worked-example) prompt — see the function's own comment |
...
| **live-verified** (not just the fake) | a fresh `PORT=3457` server (NOT the user's own — a real dev server was found already bound to port 3000, untouched) against the REAL `qwen2.5vl:7b`: full pipeline confirmed end-to-end (checkbox → upload → draw → extract → poll → merge → render), content extraction accurate; case-restoration did not fire on the synthetic test image (see `_comicExtractPrompt`'s own comment) |
