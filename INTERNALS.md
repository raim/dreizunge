...
**Unfinished-project drafts (item R, `roadmap_v87.md`, `v87_d`)** — persists chapter-splitting
output (the PDF/paste-then-split flow's `_pdfChunks`) BEFORE the learner clicks Generate, so a
closed tab or lost connection between "text parsed" and "lessons generated" doesn't lose the work.
Scope: the upload/paste `_pdfChunks` flow only — the "AI generates everything from a topic" path has
no pre-generation client-side parsing state to lose (confirmed by reading `doGenerate()`'s
multi-chapter branch: `gbody` is built straight from form fields, no chunk-splitting step at all),
and the comic-image upload flow (a genuinely different state shape, `APP_COMIC`) is NOT covered —
left as a natural follow-up, not silently assumed done.

| what | where |
|---|---|
| the store | `drafts.json` (server.js, `DRAFTS_FILE` env override for tests) — deliberately its OWN file, same reasoning as `SKILLS_FILE`: ephemeral pre-lesson-generation state, never read by `build-static.js`, must never collide with `lessons.json`'s own schema/migration/dedup logic. Gitignored (`.gitignore`), same category as `learners.json` — operational state, not curated content worth versioning, and may hold text from a personal upload |
| the routes | `POST /api/drafts` (upsert by `id` — an existing id updates in place, a missing one creates fresh; body mirrors `pdfGenerateAll()`'s own `/api/generate-book` request almost exactly so resuming needs no translation), `GET /api/drafts` (summaries only, no chunk text — the popover's own data source), `GET /api/drafts/:id` (full content, for resume), `DELETE /api/drafts/:id` (idempotent) |
| the autosave hook | `_draftSaveDebounced()` (index.html), called from the ONE tail end of `_renderPdfChunks()` — every one of that function's 15 existing call sites (upload, LLM-split, manual merge/split/delete/reorder/retitle, resplit-by-mode) funnels an autosave through for free, no per-mutator instrumentation. Guards to a no-op when there's nothing real to save (`!_uploadMode`/empty `_pdfChunks`) OR when real generation has already started (`_pdfBookId` set — from that moment `bookJobs` owns durability). 1.5s debounce, not per-keystroke |
| discard | `discardDraft()` — acts on THIS TAB's own `_draftId`; called by `_clearUpload()` (explicit "drop this document") and by `pdfGenerateAll()` the instant a book job actually starts (the draft's job is done). `_jobsDiscardDraftById(id)` (the popover's own 🗑) — acts on an ARBITRARY id, since a listed draft may belong to a DIFFERENT tab/session (no owner concept — same as every other job kind); only clears the local `_draftId` if the discarded id happens to match it |
| resume | `resumeDraft(draftId)` (index.html) — fetches the full draft, repopulates `_pdfChunks`/`_uploadFileName`/`_uploadMode` and the form (`selectLang`/`selectSrcLang`/`restoreDiffSelect`/`restoreFormatSelect`/`restoreStyleSelect`, the `use-story-cb` + `_setPasteVisible(false)` incantation `_reconnectBookJob()` already established), then lands on the SAME step `pdfGenerateAll()` would have been reachable from |
| ⚠️⚠️ TWO real bugs found only by actually resuming a draft live, not by reading the code | (1) the resumed chunk list rendered real content into a DOM node that was simply `display:none` — `#pdf-panel` needs its own `.open` class added, the same as every real upload path (`pdfExtractText`'s LLM-split branch, `setSplitMode`, the raw-file-load path) already does at the equivalent point; `resumeDraft()` was missing it entirely. (2) `_genWizardGoto(3)` looked right from the step's OWN LABEL ("3 · Chapters") but `#pdf-panel` actually lives inside `gen-card-2` ("2 · Text") — confirmed by searching the raw markup for `#pdf-panel`'s nearest enclosing `gen-card`, not assumed from the label; card 3 is per-chapter LESSON-TYPE overrides, a different step entirely. Both fixed; the resumed screen was re-screenshotted afterward to confirm the chapter list (titles, word counts, the "own story" checkbox) actually renders |
| integration with item U | `GET /api/jobs` (server.js) merges `loadDrafts()` in as `kind:'draft'`, `link:{type:'draft',id}`, `status:'draft'` (deliberately NOT `'running'`/`'pending'` — nothing is actively happening, so it does not inflate the badge's running-count, the same distinction a paused vs. active download would make). `_jobsRenderList()` gives draft rows their own 📝 icon and an EXTRA 🗑 discard button no other kind gets; `_jobsOpenLink`'s `draft` branch calls `resumeDraft(id)` |
| the acceptance tests | `test/e2e-drafts.test.js` (live server + fake Ollama, 6 checks) — validation, upsert-not-duplicate, full round-trip, `GET /api/jobs` integration (linked, labeled, excluded from the running count — mutation-tested: flipping `status:'draft'` to `'running'` turns that assertion red), idempotent delete, a second independent draft after the first is gone. `test/unit-drafts.test.js` (`lib-dom.js` harness, 5 checks) — the autosave guard's four branches, `discardDraft()`'s no-op-when-idle/delete-when-open behaviour, `_jobsDiscardDraftById()`'s id-matching (mutation-tested: removing the id check turns its own assertion red), the draft row's own markup (icon + discard button, absent on other kinds), `_jobsOpenLink`'s `draft` branch |
| NOT covered this cut | the comic-image upload flow (see "Scope" above); no UI affordance to browse ALL drafts beyond what the popover already lists (there is no separate "drafts" screen) |
