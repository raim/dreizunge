...
| ⚠️ registration in a real browser is UNVERIFIED | attempted only in a sandboxed preview browser, which failed with Chrome's generic "unknown error fetching the script" despite the file serving byte-correct via plain `fetch()` — reasoned as a sandbox restriction (the server never even logged the attempt), not disproven. **Check in a real browser before relying on this** |
