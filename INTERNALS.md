...
`t(cond ? 'a.b' : 'c.d')` — matched neither branch, so **six sites and twelve keys in the client were
never checked**, including keys that a release was actively editing. Closed with a second pass over
each `t(` call's own argument list, carrying **its own non-vacuity assertion** (the first pass's
count cannot show the second one still matches anything, and a silently-empty second pass is exactly
the regression it prevents). **Found by mutation-testing the FEATURE — putting a deleted key back and
watching the guard stay green — not by reading the guard.** When you rely on a sweep, mutate
something it claims to cover and check it actually goes red.
...
boxes are restored BEFORE the await, so a section asserting only on `APP_COMIC.boxes` passes while
the rest of the function never ran at all. `unit-comic-extract-durable`'s seed installs an explicit
...
| the vocab article contradiction | `prompts.json` `vocab.system`, `vocabFromText.system` AND `vocabTable.system` (three prompts carry the rule, not one). `v80_j` removed the per-side clause and added a worked counter-example; **`v91_g` MEASURED that counter-example as having no effect and replaced the whole rule.** The defect was the CHOICE the rule offered ("on BOTH sides … or on NEITHER side"): de→it 23/64 asymmetric — 79% of actual noun pairs — against **0/64** for a rule that DEMANDS both sides. Four prose surgeries (explanation removed, examples reversed, examples deleted, schema example moved) each measured no improvement. The rule now names no article of any language (pure `{L}`/`{S}`, 450 chars vs 861) and says that when one language has no articles the other side KEEPS its own (user ruling). ⚠️ **No longer "unverified by design"** — 5 arms × 8 chapters, de→it 79%→0% of noun pairs, de→nl 86%→12%; the article-less clause edit is the one part NOT re-measured. ⚠️ The SCHEMA example (`{"target":"der Hund"…}`) is load-bearing for FORMAT — removing it measured two lessons with no vocab at all — and was deliberately KEPT |
...
| per-JOB cancel (item AU, `v88_k`) | `_cancelALS` (server.js) — the SAME AsyncLocalStorage pattern the token meter uses, at the SAME choke point (`_callLLM`). `llm.js` publishes an aborter per request via `opts.onRequest`, in BOTH transports; destroying the socket is what stops Ollama. `runCancellable(jobId, fn)` wires `job.abort`, which **nothing had ever set** — cancel used to flip a status while the model ran on. An exported `CANCELLED` sentinel keeps a deliberate stop out of the error state, and `_callLLM` refuses to START a call once cancelled. ⚠️ **A per-item `try/catch` SWALLOWS a cancel** — `_runComicExtractJob`'s per-panel tolerance made a cancelled job report DONE; it re-throws on `CANCELLED` now, and **`_runRecreateJob` has the same shape and is NOT yet covered**. Three job kinds wrapped; the rest cancel status-only |
| IDLE release (item AU, `v88_l`) | `_lastLLMUseAt` stamped in `_callLLM` (before AND after the await — a long generation is USE for its duration), an unref'd 60s interval, `shouldReleaseIdle()` as the testable policy, reusing `releaseConfiguredModels()`. Window **30 min**, the USER's ruling — releasing costs a full model reload next generation, which is why it was asked rather than defaulted. `IDLE_RELEASE_MS`/`IDLE_TICK_MS` override it; `0` disables. Guards: never while a job runs (a job sits BETWEEN calls, so "no call in flight" ≠ idle), release once not per tick, new use re-arms. ⚠️ `bookJobs` is declared INSIDE `boot()` — the module-scope check needs `_bookJobsRef`; the first tick threw `ReferenceError` and the only symptom was "no release", so `_idleTick` is try/catch'd |
...
| **the ONE place text becomes an utterance** | `_ttsMakeUtterance(text, ttsCode, rate)`. Verified by reading every caller, not by grepping the constructor: `_doSpeak`/`_doSpeakLang` → `_speakChunks`, `_doSpeakLangThen` → `_speakChunksThen`, and `_speakAndAdvance` calls it directly. Anything that must apply to ALL speech belongs here and nowhere else |
| **ALL-CAPS is spoken as a word, not spelled out** (item AW, `v88_b`, user-reported) | `_ttsSpeakableText(text)`, applied inside `_ttsMakeUtterance`. A run of **4+** capitals is TITLE-cased (`/\p{Lu}{4,}/gu` → first char + rest lower). The engine's own "a run of capitals is an initialism, so spell it" heuristic is RIGHT for BBC/EU and wrong for a photographed sign (reported: ONTEIGENINGSZONE in `tp_17880367188140000070`), so **3-letter runs are deliberately untouched** — that is where the real initialisms live. Title-case, not lower-case: it defeats the heuristic just as completely while changing the string less. **§4-compliant**: `\p{Lu}` is Unicode machinery, not a hand-authored table — Greek/Cyrillic are handled by the same rule, caseless scripts (Japanese, Arabic, Devanagari) cannot match it and pass through byte-identical. ⚠️ **SPOKEN projection ONLY** — no renderer may call it, or a sign stops looking like a sign; `unit-tts-allcaps` §3 pins "exactly one call site, inside `_ttsMakeUtterance`" at the SOURCE layer, since that is a claim about a SET of call sites no rendered fixture can observe. `_ttsChunks()` splits on SENTENCE boundaries, so a chunk never arrives with a word cut in half |
...
| what | where |
|---|---|
| **the bug it fixes** | started while Ollama was down → *"not found — offline mode"* → offline for the WHOLE session, since `pingOllama()` had one call site and `active` was never revisited. Every generation-gated control then vanishes client-side (`APP.info.canGenerate`), which reads as broken buttons — the user's report was "why have the continue story buttons disappeared?" |
| **server: asymmetric on purpose** | OFFLINE→ONLINE on the FIRST success, every 15s (the case that bit the user; slow recovery IS the complaint). ONLINE→OFFLINE only after TWO consecutive failures, every 60s (one dropped ping is a blip; flipping on it disables the UI mid-session — worse than the bug). Logged only on a transition. `unref()`d so it can never hold the process open, which the e2e suite depends on. Cadences overridable via `BACKEND_RECHECK_OFFLINE_MS`/`_ONLINE_MS`, floored at 250ms |
| ⚠️ **the `BACKEND !== 'none'` condition is NOT the protection** | `llm.js`'s `ping()` returns false unless `BACKEND === 'ollama'`, so a disabled backend can never be detected as up however often it is polled — that is the real guarantee. Removing the server-side guard leaves the e2e GREEN (the deeper one holds); removing BOTH turns it red. The condition is kept as an OPTIMISATION — don't schedule a timer that pings and fails forever for a config that opted out — and the comment says so rather than claiming to be the safety net |
| **client: without it the fix is half a fix** | `APP.info` is fetched ONCE in `init()`, so an open page would stay crippled until reloaded. `_startBackendWatch()` polls `/api/info` ONLY while offline (a healthy session makes no extra requests), then re-renders the three `canGenerate`-gated surfaces (library list, lesson-set path, storyline screen), each guarded independently. Does NOT poll the other way — same asymmetry as the server. Lives INSIDE the static-build exclude region with `init()`; verified absent from `docs/index.html` |
| **verified LIVE, both directions** | real processes, not just the fake harness: healed ~6s after a stand-in backend appeared, one log line, no restart; and after killing it, stayed online 21:54:57→21:56:38 and flipped at 21:56:43 (~106s = two 60s checks) |
| ⚠️ **three vacuity defects in the new e2e, all found by mutation** | (1) §4's 3s wait was shorter than the 15s default poll — hence the configurable cadence; (2) after restructuring, §4 ran after the `finally` that closed the stand-in backend, so its premise was false — it now starts its own; (3) two independent guards enforce §4's claim, so it cannot attribute the behaviour to one, and now says so |
| ⚠️ **harness trap** | `lib.js`'s `boot()` derives its port from the process id, so two servers booted in ONE test process collide — the second fails to bind and every request silently reaches the FIRST. A nested second boot made §4 query the already-healed server and "fail" while the code was correct. Boot the second only after stopping the first |

---

**`v87_o` — multi-type add-lessons on the lesson-set page; prior-story context for image
descriptions** (two user requests; full write-up in `roadmap_v87.md`'s own `v87_o` entry).

| what | where |
|---|---|
| **multi-type add-lessons** | the lesson-set card's hand-written `<select>` is DELETED; its Generate button opens `_pickLessonTypes()` — the same modal the storyline page uses, over the same `renderLessonTypeChecks`/`ADD_LESSON_TYPES` the book form ticks. `doAddLesson()` gained `fmtOverride`, so `doAddLessonMulti()` calls it once per ticked type SEQUENTIALLY with no duplication of the per-type logic (client-side `mixed`/`intro_script` paths and the LLM path untouched). Absent, the other add-lesson surface behaves exactly as before |
| ⚠️ **the dialect gate had to MOVE or be silently lost** | it lived as `.opt-ai-authoring` on the deleted `<select>`, swept by `openAddLesson`. `ADD_LESSON_TYPES` gained `ai: true` on the eight authoring types and `renderLessonTypeChecks` an `allowAi` filter (same `undefined` = "not asked" shape as `needsStory`/`needsScript`). Strictly better: the registry is the ONE list every add-lesson surface reads. **Recorded**: the LIBRARY menu's `<select>` never carried those classes, so that surface never had this gate — pre-existing, out of scope here |
| **`openAddLesson` with no `<select>`** | shows the math-instruction and vocab-mode rows up front on that card: the format is chosen AFTER the panel opens, so there is no value to key them off, and both are per-RUN settings ignored unless a consuming type is ticked. Hiding them would mean a math instruction could never be supplied from that card |
| **prior-story context** | `_comicDescribePrompt(langName, priorStory)`. TWO sources, both "the story so far": the continued-from chain via `collectChainStory()` SERVER-side (the client's `savedList` has no story text, so it sends only `continuedFrom`), and the panels already described in the same batch, accumulated and capped to 6000 chars. Framed "continuity only — do not summarise, continue, retell or quote", since the real risk is describing the CONTEXT instead of the image |
| ⚠️ **a single-chapter fixture cannot prove the chain walk** | the route falls back to `anchor.story`, so stubbing `collectChainStory` out stayed GREEN. The e2e fixture is a TWO-chapter chain, and `fake-ollama` marks a reply `MITKONTEXT` when the block was present, so the test proves the text REACHED the model. THREE mutations red |
| ⚠️ **TWO tests were failing on the LIVE corpus, not on any code change** | `unit-word-progress` and `unit-story-unlocked-card`, 8/8 deterministic. Isolated: they fail at HEAD too, PASS with the committed `lessons.json`, FAIL with the working-tree one — the user's server wrote another chapter (338→339) between release runs. Both had the SAME defect (`v81_d`/`v81_e`'s lesson again): the fixture was chosen by a PROXY for the property the section then asserts. Now each selects by the property ITSELF — render and keep the first candidate that produces marks; try the demotion and keep the candidate only if it happens (cleaning up the wrong-map entry). A substring pre-check was tried for the first and was ALSO insufficient: the real matcher normalises via `_hlKey`/`stripFuri` and splits multi-token entries |
...
| what stayed | `caption` and `inScene` remain separate despite both being extracted text. ⚠️ `v88_ab`: the reason recorded here was FALSE — `_comicStoryPanelsHtml` has always JOINED them. Re-asked at that cut, the user ruled the split stays anyway: two extraction fields, ONE rendered block. The real reason is that the extraction prompt produces them separately and the review card is where a mis-assignment is corrected |
...
| what | where |
|---|---|
| ⚠️ **the measurement that changed the design** | the chapter reported as "did not finish" HAD finished — but 2 of its 6 sentences held a full set of token slots and NOT ONE lemma. `parseAnalysisReply` (canonical-analysis.js) degrades a malformed reply the same way for EVERY token in that sentence, deliberately, so one bad reply cannot abort a chapter that takes many minutes — and nothing ever revisited it. Store-wide: **3 of 51 sentences (5.9%) in 2 of 18 chapters, 86 tokens stranded**, both chapters ones the user reported |
| **`_analysisSentenceUsable(sent)`** (server.js) | "analysed" = at least one token with a real lemma. `computeFrequency`'s own definition of resolved, reused not re-invented. `tokens.length > 0` would have preserved the exact gaps the resume exists to close |
| ⚠️ **there was nothing to resume FROM** | `writeAnalysisChapter` ran ONCE, after the whole chapter. A run that timed out, was cancelled or died with the server persisted nothing. The store proves it: 18 chapters, no partials, because a partial could never be written |
| **`analyzeChapter`'s two hooks** (canonical-analysis.js) | `reuse(sentence, i)` → a sentence to keep; `onProgress(i, soFar)` → fires after each NEWLY analysed one. Both opt-in, so every existing caller is byte-identical (asserted). A reused sentence does NOT fire onProgress — nothing was computed |
| the checkpoint | `_runAnalysisJob` persists after every new sentence with `partial:true`; the final write clears it. A failed checkpoint never fails the run |
| **reuse matches on TEXT, not index** | index matching is right only while CP1's segmentation is unchanged — exactly when nothing needs resuming. Text matching serves a died-mid-run prefix, a story that GREW, and a failed sentence with ONE rule, and cannot pair an old analysis with a different sentence sharing its slot |
| ⚠️ **TWO cache short-circuits, not one** | the route's own, and `_kickOffAnalysisJob`'s (which repeats the test for its OTHER caller, `_runBookJob`'s postGenAnalysis). Teaching only the route about `resume` left the second firing: a LIVE resume request returned `{cached:true}` having done nothing. Found by issuing it against a running server — both sites read correctly in isolation. `v88_b`'s set-level lesson in a new shape |
| `partial` is still `available` | the explorer renders the half it has. What it must not do is satisfy either short-circuit. Asserted with a PLAIN post, no flags — doing it with `{resume:true}` was VACUOUS, since `!resume` opens the gate by itself |
| ⚠️ **a partial would have rendered a TRUNCATED chapter** | `_teStoryHtml` emitted only the gaps BETWEEN located sentences, dropping everything past the last one. Invisible while every analysis was complete; fatal the moment a partial renders. Not routed through `_teGapHtml` — that returns null for a gap containing a blank line, DISCARDING its text, and for a tail the text is the point |
| the dialog | `showChoiceDialog` (already existed) — cancel / "Analyse only what is missing" / "Re-analyse everything from scratch". Its counts come from the SERVER's shadow (`usableSentences`/`totalSentences`), so the number shown and the sentences redone share one definition |
| `force` vs `resume` | mutually exclusive BY CONSTRUCTION: force deletes the record before the shadow is read, so a resume flag alongside is inert. The mutation "force also resumes" stays GREEN — that is the design; the ORDER of those two lines is pinned instead |
| verified live | a real 35B model on the user's own `tp_…093`, store pointed at a scratch copy: "CP2: analysing 1 sentence(s)", `7 sentence(s), 123 token(s), 6 reused`, sentence 5 went 23 unresolved → 23 resolved. Server on a spare port, killed by PID |
| the acceptance tests | `unit-canonical-analysis` (+2, counted over REAL HTTP calls — "the result contains the reused sentence" would pass if it were re-analysed and came back the same), `e2e-analysis` (+4, staged by editing the store, since the fake always answers), `unit-text-explorer` (+2), `unit-analyze-chapters-run` (migrated off `confirm()`). **FOURTEEN mutations red** |
| four vacuous/brittle assertions found by mutation | `/catch \(e\)/` matched the FUNCTION's outer catch, not the checkpoint's; the partial-gate test opened the gate itself with `{resume:true}`; §7's "the user said yes" is now two answers posting two bodies; and the concurrency section counted a RUNNING TOTAL across the whole file, breaking on a correct render of its own claim (now a delta) |
...
| what | where |
|---|---|
| ⚠️ **neither lock ever fired live** | both were scoped `!APP.info.canGenerate && !APP._teacherMode`, and `canGenerate` is true on a running server. The PUBLISHED `docs/index.html` is where they had force — which is where students are. A gate you cannot see in development is a gate nobody re-reads |
| surface 1 | **`_renderChapterCard`'s `_isLocked`** (index.html) — the 🔒 overlay. Gone with it: the transitive "something earlier is unfinished" flag threaded through `_renderChain`'s recursion (`v69.2d`), the predecessor-completion test (`v74_k`), the fail-closed `!prevTopic` clause (`v46` Bug #2), and the started-chapter helper that stopped the gate confiscating work in progress. **What remains is not a progress gate**: `!isFirst && _sets.length === 0` — a fact about the DATA (nothing generated yet), not about the learner |
| surface 2 | **`_sbChapterTarget`** (index.html) — the SAME rule wearing different clothes: a click on a "locked" storyboard panel was silently REDIRECTED to the last unlocked incomplete chapter, with a toast. No 🔒 anywhere to make it visible. Now clamp-and-return; `completed`/`free`/`resumed` all gone. **Kept as a NAME** because `_sbNavClick` and `_sbMarkCurrentPanels` share the panel→chapter mapping and "the frame and the link cannot disagree" is pinned at the source layer |
| ⚠️ **the second copy is the finding** | deleting one and leaving the other is how a rule survives its own deletion. `unit-storyline-chapter-access` §3 is the single place that cross-checks both went |
| the removed key | the resume toast explained a redirect that no longer happens. Deleting a key costs the user nothing to translate; leaving it would have left a dead string AND a guard asserting its existence |
| ⚠️ **two comments had to be REWORDED** | they spelled identifiers the new guards SWEEP for. A comment near a source-scanned pattern must not spell the pattern — it fails the very check it documents. Caught by the guards, not by review |
| ⚠️ **a guard can go WRONG without going RED** | `unit-storyline-lock-hardening` (renamed `unit-storyline-chapter-access` via `git mv`, rewritten) pinned the deleted rule and still PASSED, against the wrong claim — and its own non-vacuity check ("a later chapter is still locked") would have gone red on a CORRECT tree. New claim, deliberately stronger: no progress state may reach the predicate at all, asserted as an ABSENCE over the whole client, because a deleted rule must not survive as a dead branch no fixture reaches |
| ⚠️ **`unit-live-static-progress-parity` re-anchored (rule 29)** | it observed `v74_i`'s shared-completion fix THROUGH the lock. The CLAIM did not change, only the mechanism: `chapterComplete` still decides the CONNECTOR LINE into the next chapter, and `v74_i`'s defect shows there just as sharply (a mixed-driven chapter's pooled prep lessons never get done-flags, so a raw `every()` can never be satisfied). Moved to `path-line done` — **exactly one**, non-vacuous in both directions: a broken rule gives 0 or all. The lock reading became `deepStrictEqual(locked, all-false)` |
| **kept on purpose** | the WITHIN-chapter progression the user was explicit about — the gate chain, `storyUnlocked`/`_storyLockedLesson`, and the storyline screen's full-story lock row (the one 🔒 that legitimately survives there). Do not "finish the job" by removing those |
| verified live | the user's own server with `canGenerate` forced false and teacher mode off, on a real 6-chapter storyline: **six 🔒 glyphs and ten inert card wrappers before, one glyph and zero inert wrappers after**, and the survivor sits inside `.storyline-story`, not on a chapter card |
| the acceptance tests | `unit-storyline-chapter-access` (4 checks, rewritten), `unit-storyboard-nav` §3/§4/§5 (fixtures that used to be redirected now assert the clicked panel is the chapter opened; two lock assertions became absence assertions), `unit-live-static-progress-parity`. **EIGHT mutations red** |
...
| what | where |
|---|---|
| **why it exists** | `comicCreateChapter()` drops any panel `_comicPanelText()` returns nothing for, so a wordless panel (action beat, establishing shot) silently contributed NOTHING to the chapter. A 1-2 sentence description in the TARGET language turns it into real chapter content |
| **`_comicDescribePrompt(langName)`** | a SEPARATE prompt and a SEPARATE vision call — deliberately NOT folded into `_comicExtractPrompt`, which is the most heavily live-tuned text in `server.js` (`v85_k`/`v85_l`, three measured rounds, a German-only worked example proven necessary). Two prompts also fail independently. Asks for 1-2 sentences IN THE TARGET LANGUAGE, only what is VISIBLE, plain present tense, explicitly NOT transcribing lettering |
| ⚠️ **the LAZINESS rule** (user ruling) | `_runComicExtractJob(jobId, images, lang, {extract, describe})`: per panel, extract first if asked, describe ONLY when that produced no caption and no in-scene text. A vision call is one-per-panel and slow (15s-5min), so describing every panel of a lettered page would roughly double the wait for a result never used. Absent flags = extract-only, so an older client is unchanged; both false is rejected by the route |
| ⚠️ **an extraction ERROR is not "no text"** | a failed extraction is transient and re-runnable; substituting a description would hide it behind plausible content. Errored panels surface the error and get NO description — pinned by its own e2e section |
| **client** | `#comic-extract-btn` → `#comic-extract-cb` + `#comic-describe-cb` + `#comic-generate-btn` (defaults: text ON, description OFF). `_comicSyncGenerateBtn()` DISABLES Generate while neither is ticked — clearer than a toast and costs no third `ui.json` string. ⚠️ **`v88_ab` REPLACED the fallback rule with COMBINE** (user ruling): the chapter text is `extracted` AND `description`, joined by a BLANK line so `_storyParasHtml` renders two `<p>` blocks. `caption`/`inScene` keep their single newline — one block. The rule now lives in **`_comicTextFromFields(fields)`**, and `_comicPanelText(b)` is a one-line wrapper over it, which makes it true at once for the no-text filter, both panel summaries, `comicOpenReview`'s editable set, `_genChapterCount` (`v87_h`) and — new at `v88_ab` — the RENDER side (`_comicPanelsHaveText`, `_comicStoryPanelsHtml`), which used to keep its own copy. The review card gained an editable description field, shown only when the panel has one |
| **`ui.json`** | TWO new `en` keys: `form.comic_describe`, `form.comic_generate`. The review card's description field reuses `form.comic_scene_ph` (726→728) |
| ⚠️ **fake-ollama ordering** | the new `comic_describe` kind MUST be tested BEFORE `comic_extract`: both prompts legitimately open by naming the same context ("a single panel cropped from a comic page"), so the extract pattern matches the describe prompt too. Found by watching a describe call return the extraction's canned reply. Also gained `FORCE_NOTEXT` (unlabeled prose → parses to empty) as distinct from `FORCE_EMPTY` (empty body → `callLLMVision` REJECTS it as a failure) |
| **LIVE-VERIFIED** | real vision model, real corpus panel (`Gratis en Kostenlos`, `nl`, extracted caption empty — the exact wordless case), fresh server on port 3199 (the user's own on 3000 untouched): returned two correct Dutch sentences describing only what is visible. Not English — which is the whole point, since the description IS the chapter text |
| the acceptance tests | new `test/e2e-comic-describe.test.js` (6 checks: describe-only makes zero extraction calls; both+text found makes ZERO describe calls; both+no text describes; an error is not described; older client unchanged; neither → 400). `unit-comic-extraction.test.js` +3 sections. TWO mutations red. Five existing tests updated for the renamed control / widened buffer |

---

**`v87_k` — BUG FIX: the lesson-set story reader is now the IDENTICAL render the progress card uses**
(user report; full write-up in `roadmap_v87.md`'s own `v87_k` entry). ⚠️ **OVERTURNS `v80_t` for this
panel, on an explicit user ruling** — it is a tappable surface now.

| what | where |
|---|---|
| **the change** | `renderStoryText`'s non-explorer branch was an open-coded copy (`furiHtml` → `_highlightVocabHtml` → `_storyParasHtml`); it now makes the byte-identical call `_renderCompStory` does: `_storyBodyHtml(d, { text: showingSource ? full : null, highlight: !showingSource })` |
| ⚠️ **the reported symptom was one of FOUR defects** | comic panel IMAGES (the `o.text == null && d.comicPanels` branch was unreachable because the function was never called — the same shape as `v86_a`, fixed once and reintroduced by a card that re-implemented instead of reusing); the translation view's card/padding wrapper; TRACK T's THREE-state `_wordStateMap` (the copy still passed the superseded `v74_n` two-shade `solved` array); and `.story-selectable data-tutor-select="1"`, whose absence meant `PLAN §12`'s "select text, ask the tutor" **never worked on this card** |
| ⚠️ **tap and three-state colouring are ONE switch** | inside `_highlightVocabHtml`, `stateByKey` drives both the `wp-*` state classes and the `wp-tap`/`tapWord` affordance (see its own `v80_t` comment). They cannot be separated, so "identical to the progress card" necessarily means "tappable". Put to the user with that trade-off named; they ruled fully identical. **The architecture line moved**: every `_storyBodyHtml` caller is a tappable, run-bearing surface, and this card joined them — the read-only panels (`_renderSavedStory`/`_renderChainStory`) still open-code the two-shade highlight and are unaffected |
| **why edit/QC survive it** (the user's condition, checked not assumed) | `#story-body` is a SIBLING of `.story-hdr`, so the marks' `event.stopPropagation()` has no ancestor handler to swallow (`toggleStory()` is bound to the header alone, which is why its icon buttons need stopPropagation and the body does not). `toggleStoryRepair()` rebuilds the body from `APP.lessonData.story` rather than scraping the render, so marks never leak into what is edited; `saveStoryEdit()` reads `innerText` only in that already-plain state. `runStoryQc()` posts `{topicId}` and reads nothing from the DOM |
| the acceptance tests | `unit-lesson-set-story-explorer.test.js` §6 pins EXACT STRING EQUALITY with `_renderCompStory`'s own call (the user's "ideally identical code" — far stronger than "both contain an `<img>`"), §7 covers edit + QC. `unit-story-panel-alignment.test.js` (`v82_c`): the two-shade assertion became a state-class one (same claim, new vocabulary) and the no-tap assertion was **INVERTED, not deleted**, so a regression back to inert marks still fails. THREE mutations red |
| ⚠️ **an assertion deliberately NOT written** | "no `<mark>` survives into edit mode" — `lib-dom` does not implement `innerText` (plain property), so assigning it does not clear `innerHTML` as a browser would; that check would test the STUB. The load-bearing claim (the editable text IS the story) is asserted directly instead |
...
| **`_comicPanelText(b)`** (replaces the removed `_comicBuildStoryText()`) | returns ONE panel's own text — no cross-panel joining. Since `v88_ab` a one-line wrapper over **`_comicTextFromFields(fields)`**, the single home of the rule; `b.text` (wizard) and a saved `comicPanels[i]` (flat) are the two field shapes it serves |
...
| **⚠️ the `ui.json` keys are `form.image_*` (item AP, `v88_f`)** | The whole branch was renamed from `form.comic_*`/`form.use_comic` to `form.image_*`/`form.use_image` (user: *"uploading comics is just a possible application of the tool"*). **Element ids, function names and CSS classes are UNCHANGED** — `#comic-panel`, `APP_COMIC`, `comicOpenReview`, `.comic-story-panel` — so grep for `comic` still finds the code; only what the LEARNER READS moved. Only three English strings actually contained the word; the rest carried it in the key name alone, so their translations carried across untouched. `form.image_detect` is the one string that GAINS it ("Detect comic panels", the user's own carve-out — there the word does real work). ⚠️ The static markup holds an English DEFAULT for each label (`#use-comic-lbl`, `#comic-help`, `#comic-choose-lbl`, `#comic-detect-lbl`), rendered before `loadUIStrings()` and baked into the static build — **no guard covers those**, they must be updated by hand alongside any wording change |
| **item AM (`v88_c`) — a fresh upload PRE-SELECTS it** | `_comicFinishSetup()` calls `comicUseWholeImageAsPanel()` as its last act, so an uploaded image arrives with one whole-image panel already drawn (user request: *"as if the button ... had been pressed already"*). Nothing is taken away — drawing a box, auto-detecting, or pressing the button all REPLACE it, since both writers replace rather than append. The commonest upload is one photo of one thing, for which the whole image IS the panel; the old default (no panels, every downstream control inert) made the common case do the most work. **⚠️ Placement is the entire risk.** `_resumeComicDraftFrom()` deliberately does NOT route through `_comicFinishSetup()` (see the drafts table's own `resume` row), so a restored draft keeps its own boxes; put the pre-select anywhere more general and a resumed draft becomes one giant box. Mutation-tested by ACTUALLY adding the call to the resume path and watching `unit-comic-review-autopath` §6 go red — and that mutation is what exposed that the section's `fetch` stub had to be URL-aware (the real resume path calls `loadSavedList()`, which needs ARRAYS; a one-shape stub killed the path before it restored a box, i.e. the section would have passed **vacuously**). This is also the per-image act **item V** needs: the user's ruling is *"if multiple images are uploaded, mark all images as one panel, but still allow the user to modify, add and resort panels. each panel is one chapter"* — this behaviour repeated per image, over an editable panel list, feeding `comicCreateChapter()`'s existing one-chapter-per-panel formation (`v85_p`) |
...
| **⚠️ `v88_ab` — the description is COMBINED, and the single-panel renderer reads `d.story`** | User ruling replacing `v87_l`'s fallback: chapter text = extracted lettering AND description, two `<p>` blocks. Measured first — 2 of 18 panels had a description suppressed by extracted text, at length ratios of 0.09/0.08, both the same photographed sign whose body simply was not extracted (a THIRD chapter from that sign, `tp_17881715830570000091`, extracted it correctly into `inScene`). The rule moved into **`_comicTextFromFields`** and all three copies delegate. ⚠️ **The ruling opened a third `story`-vs-`comicPanels` divergence** (after `v86_g` and `v88_e`): re-deriving a LEGACY chapter under the new rule shows the description on the highlighted story panel and nowhere else — item AY's asymmetry again — and a story EDIT would duplicate it via `v86_g`'s own sync. Fixed by **`_comicStoryPanelsHtml` reading `d.story` directly when `comicPanels.length === 1`**, deriving per panel only for multi-panel; a no-op on all 18 existing chapters (measured: the old derivation reproduced `story` exactly for every one), a guarantee thereafter. `v86_g`'s sync is KEPT but is no longer the protection (`v87_p`'s ruling) and deliberately does NOT delete `description` (item AN). **Does NOT rewrite stored data** — the two reported chapters keep `story: "De Manteling"`. Guards: `unit-comic-story-text.test.js` §4/§4b-gate/§4c/§4d, `unit-comic-extraction.test.js` §6c/§6d (§6d asserts the PUBLISHED build, `v88_r`'s rule), `e2e-save-story-comic-sync.test.js` §5. Ten mutations red |
| **⚠️ `v88_ac` — a deleted chapter used to leak its CP1/CP2 analysis forever** | `DELETE /api/lessons/delete` cleans up the topic, storyline membership, the `continuedFromId` chain and every flag — but never `canonical-analysis.json`. Invisible in normal use (nothing can address a dead id) and unbounded: **5 of 20 entries were orphans, 12% of the file**, oldest 5 days. `deleteAnalysisChapter()` had existed since `v86_ac` for the force-re-analyse path; nothing called it from the delete route. Now called, targeted at the ONE deleted id. ⚠️ **Deliberately NOT a load-time sweep** over "entries with no matching chapter" — that is one partial read of `lessons.json` away from wiping every analysis on the box, and `e2e-analysis.test.js` §9 asserts an UNRELATED chapter's entry survives the delete precisely so the sweep version cannot pass. Two mutations red (leak restored; sweep substituted). The 5 existing orphans were pruned in the same release. **⚠️ Also found, NOT fixed**: `canonical-analysis.js` writes `reviewed: false` on every token at three sites and **nothing reads it or ever sets it true** (533/533 tokens at the default) — the schema slot for item AI |
| **⭐ `v88_ad` — item AI first cut: the curator OVERLAY over CP2's analysis** | `analysis-corrections.js` (its own module: `build-static.js` cannot require `server.js`, and `canonical-analysis.js` drags in `llm.js`). User rulings: corrections are STICKY across a re-analysis, the editor is the existing token popover, the first cut targets the unresolved tokens. ⚠️ **The key is (sentence TEXT, surface, occurrence) — NOT `tokenId`**, which is `chapterId:sN:tM`, a pure index: a story edit renumbers it and the correction would silently re-attach to a DIFFERENT word. `v88_x`'s own precedent. Merged on READ in `analysisShadowFor` (the single live read path) so an edit shows immediately and `canonical-analysis.json` stays the model's own output — ⚠️ **and in `build-static.js` too**, which reads that file DIRECTLY and is therefore a second surface over the same cache (`v87_k`/`v88_w`'s class); without it every correction would be absent from `docs/`. `analysis-corrections.json` is in `BUILD_SOURCES`. **The asymmetry IS the feature**: `deleteAnalysisChapter` (force re-analyse) LEAVES corrections; the chapter-delete route drops them (`v88_ac`'s leak, from the new store's side). Client: `_teShowWordPopover` gained three inputs + save/clear, tokens carry `data-cid`/`data-si`/`data-occ`, `.te-tok-reviewed` marks curated tokens, and a `te-fixbar` counts the unresolved. Live build only — the static build has no server and a baked snapshot. ⚠️ The popover is tracked BY REFERENCE (`_teWordPopEl`), like `_comicReviewOverlayEl`: an id lookup for a dynamic node is what `lib-dom` auto-vivifies, returning the wrong element. Guards: `unit-analysis-corrections.test.js`, `unit-text-explorer.test.js` (incl. a CROSS-LAYER check feeding the renderer's own attributes into the real server merge), `unit-static-analysis-bake.test.js`, `e2e-analysis.test.js` §10-12. Fifteen mutations red |
| **⭐ `v88_ae` — the curator TABLE, and the story-rewrite warning** | `_teOpenCuratorTable(chapterId)`: one editable row per token in sentence order, an unresolved filter, `comicOpenReview`'s modal shape and attribute-wired handlers (local buffer, so Cancel is a true no-op). Batch route `POST /api/analysis-corrections/:id`. ⚠️ **Sends only the rows that CHANGED** — saving all of them would turn every model value into a "curator correction", mark the chapter reviewed and pin it against future prompt improvements. **Orphans** (corrections whose sentence was rewritten away) are LISTED, never deleted (user ruling): `analysisShadowFor` reports `correctionCount` = what actually APPLIES plus `orphanedCorrections` in full, the table shows them in their own block **exempt from the filter**, and the worklist bar appears for orphans alone — they are findable no other way. **Rewrite warning**: dry-run route `POST /api/analysis-correction-impact/:id` runs CP1 on the CANDIDATE story; the story editor asks Cancel/Continue before committing. Fails OPEN, charges only against currently-applying corrections, returns immediately when nothing is curated. ⚠️ **The dry run must ask the SENTENCE-level question** (`partitionCorrectionsBySentence`), NOT the token-level one (`partitionCorrections`): CP1 tokens carry `text` and split naively (`"lauft."`), CP2's carry `surface`, so the token question over CP1 output matches nothing and calls every correction doomed — the bug the e2e caught. ⚠️ **Lifecycle**: a story save does NOT re-run CP2, so a correction still applies to the stale analysis until the RE-ANALYSIS; the warning is future tense for that reason. One key: `text_explorer.corrections_orphaned`. Ten mutations red |
| **⭐ `v88_af` — the ui.json translation job (listed, cancellable, checkpointed)** | User report: "translation job is not listed in the job popover (and ideally should be cancel-able)". ⚠️ Of the THREE translation triggers, `retranslateStory`/`retranslateChain` were `_jobsTracked` at `v88_b`; **`triggerUITranslation` was the one never wrapped, and `/api/ui-translate` registered no job either** — invisible in every channel at once, for ~19 sequential model calls per language (755 keys / 40 per call). Now `newJob({label})` + `runCancellable`, answering **202 + {jobId}**; the client polls `/api/job/:id` (same shape as `_textExplorerCheckOnce`) and treats `cancelled` as its own outcome, not an error. ⚠️ **A real job, NOT a `_jobsTracked` sync row, deliberately** — a sync row cannot carry a cancel button (`_jobsRenderList`'s `canCancel` needs a server-side job id for `POST /api/jobs/cancel`), which is the diagnosis under the open "some LLM jobs have no cancel button" item; this is the first such route converted and the pattern for the rest. ⚠️ **The label is load-bearing**: `/api/jobs` skips any job without one (`if (!j.label) continue`). ⚠️ Reading the loop found TWO unreported defects: the per-batch `try/catch` **swallowed `CANCELLED`** (`v88_z`'s exact shape — a cancelled job would run every remaining batch and report DONE), and `saveUI` ran ONCE after the whole loop so a cancelled run persisted **nothing** (`v88_x`'s rule). Both fixed; the checkpoint also makes a re-run resume, since the function already skips existing keys. `translateUIToLang(lang, jobId)` takes the id as OPTIONAL — `ensureUIForLang`'s plain language switch is unchanged. Guard: `e2e-ui-translate.test.js`, cancel driven by OBSERVED PROGRESS (poll until `batch 2/3`) rather than a timer. Four mutations red |
| **⭐ `v88_ag` — story/summary QC are listed, cancellable JOBS** | The bug the user's "job not listed" report was actually about (they clarified they had clicked QC, not translate — `v88_af` fixed a real but different defect). ⚠️ **THREE QC entry points, only `/api/qc` was ever a job**: `/api/story-qc` and `/api/summary-qc` both AWAITED the proofread and registered nothing, and neither `runStoryQc()` nor `runSummaryQc()` was `_jobsTracked` — a full-story model call with no row anywhere. Both converted (not just the clicked one): 202 + {jobId}, `runCancellable`, payload unchanged but delivered via `jobDone` as the job's `data`. Labels are load-bearing (`/api/jobs` skips label-less jobs); links point at the topic / storyline. ⚠️ **ONE shared client poller `_qcPoll`** so the two cannot drift — and its first version HUNG THE SUITE: it continued on any unrecognised status, so a stub answering with no `status` re-armed a 2s timer forever and `unit-lesson-set-story-explorer` printed ALL PASSED and never exited (`v88_r`'s rule). Only `running`/`pending` continue now. A cancelled QC restores the button, renders and persists no proposal, and is SILENT (a cancel is not a failure — same distinction as `jobFailOrCancel`). Guards: `e2e-qc-job.test.js`, `unit-jobs-sync-inflight.test.js` (the poller's termination and cancel paths, pinned there because repairing the other file's stub left them unexercised). Six mutations red |
| **`v88_ah` — the curator table is reachable on ANY analysed chapter** | User question that was a bug report: "where can i enter the new text analysis curator table?" — the `▤` button in the text explorer's bar. ⚠️ `v88_ae` gated the WHOLE bar on `unresolved \|\| orphans`, so a fully-resolved chapter with no orphans offered **no way in at all**. Wrong for a review interface, and wrong in the case that matters most: **a confidently WRONG lemma is exactly what an unresolved worklist can never surface**. The table button now renders whenever the chapter has analysed sentences; the worklist JUMP stays conditional on there being something unresolved, and the bar still vanishes when there is no analysis at all. Two `v88_ae` guards were re-scoped (they asserted the old behaviour rather than failing). Three mutations red |
| **⭐ `v88_ai` — an eight-item user batch; closes item `Y`** | **Analysis curation** is now TEACHER-MODE only (`_teCanEdit`, one gate for the bar + table + editable popover) and **lesson-set only**, rendered **BELOW** the text — three surfaces share `_textExplorerBodyHtml`, so the CALLER opts in via `{curator:true}` and a SET-LEVEL guard asserts exactly one does. ⚠️ **The ▤ button shipped INERT**: the chapter id was interpolated with `JSON.stringify`, whose DOUBLE quotes close a double-quoted `onclick`. Use single quotes + `escAttr`, as every other dynamically-wired control here does. The guard that missed it asserted the markup merely CONTAINED the function name — a proxy true of the broken version too. **Artwork toggle** repaints all surfaces (`_slArtRepaint`) instead of only the library — `v86_ad`'s lesson, four surfaces offering the control and one being repainted. **`_storyTapMaybeAdvance`** skips `#comp-story-text` while `APP._textExplorer` is on: the analysed `<mark>`s stopped propagation themselves, but the gaps between them advanced the lesson. **Chapter-management row** moved below the summary, above the chapter cards. **Item Y**: the storyline header's five authoring buttons RELOCATED (not rewritten) into `#sl-screen-edit-pop` behind one pencil, `▶`/`🔗` kept in the header per the user's exclusion; rows mirror each button's own visibility and take their label from its own `title` (hence zero keys), leading icon stripped; the pencil hides when every row is hidden. ⚠️ Fixed a pre-existing bug in the move: `sl-screen-del-btn` sat inside `#sl-tag-editor` and was unreachable unless the tag editor was open. **Idle release 30 → 60 min**. Sixteen mutations red |
| **`v88_aj` — the storyboard/images switch moved into the storyboard menu** | User request: move it into the storyboard popover, remove it from the library, storyline and lesson-set pages. `_slThumbToggleHtml` (a button) became **`_slThumbChoice`** (a menu-entry description); eligibility unchanged and still in one place (teacher mode + a storyboard + comic images), so the menu never grows a dead entry. `v88_ai`'s `_slArtRepaint` is STILL required — one trigger now, but it is opened from the storyline screen while the library card and lesson-set page both display the artwork it changes. ⚠️⚠️ **THIS RELEASE BROKE THE USER'S LIVE APP MID-EDIT**: `_slThumbToggleHtml` was replaced BEFORE its three call sites were removed, and `server.js` serves `index.html` with `readFileSync` PER REQUEST — so `loadSavedList()` threw `ReferenceError` in their browser and the library plus every `#sl=` deep link died. **In this repo a rename deletes the callers FIRST, or lands atomically.** ⚠️ The menu label prefixed an icon onto a string that already had one ("🎬 🎬 Show the storyboard…"); the guards used `includes`, which a doubled prefix satisfies — asserted as EQUALITY now. Two `v87_m`/`v87_n` guards re-scoped (a button placement the user reversed). Five mutations red |
| **`v88_ak` — `e2e-idle-release` was never flaky: the GUARD counted the wrong thing** | Zero product changes. It failed three consecutive full-suite runs (`a still-idle server does not release again on every tick (got 1 -> 2)`) and passed standalone. **Instrumented** (as the session prompt had instructed for two cuts): `releaseConfiguredModels()` releases every configured role model IN PARALLEL, so ONE sweep writes one `keep_alive:0` entry PER MODEL — 2 in the harness (`fake` + the un-overridden VISION model `qwen2.5vl:7b`), arriving 0 ms apart standalone. The test counted ENTRIES as a proxy for SWEEPS and §2 exited on the FIRST entry; under load the second entry of the SAME sweep landed during §3's sleep and was read as a second release that never happened. Metric is now the number of SWEEPS = the highest per-model release count, which no intra-sweep interleaving can affect; §2 waits for a COMPLETE sweep. ⚠️ **Justified by CONSTRUCTION, not by reproduction** — the OLD counting passed 6/6 under three busy-loop CPU hogs, so that load cannot distinguish the versions. A deterministic §0 pins the metric instead. **Still not "cleared"**: on the next failure, capture the entry arrival times rather than re-running. Three mutations red (incl. the server's release-once latch, proving §3 keeps its teeth) |
| **⭐ `v88_al` — the last five blocking LLM routes become listed, cancellable JOBS (closes the cancel-button item)** | User ruling: "convert all five together behind one poller." `/api/retranslate-story`, `/api/storyline-title`, `/api/storyline-summary`, `/api/storyline-retitle`, `/api/writing-feedback` — nine call sites. ⚠️ **A `sync` row could never carry a cancel**: `canCancel` needs a server-side job id, so the fix had to be in the ROUTES, not the popover. ONE shape each side: **`runAsJob(res, meta, producer)`** (mints a labelled job, runs the producer in `runCancellable`, answers 202+{jobId}, producer's return value becomes the job `data`) and **`_jobAwait(response)`** (returns the data, THROWS on error so existing try/catch works, returns **null on cancel** so call sites stop quietly). ⚠️ Validation stays OUTSIDE the producer — a 400/404/503 is an answer about the REQUEST. ⚠️ Polls at **500ms**, not 2s (three sit in front of a watched control; writing-feedback is in the learner's exercise flow). ⚠️ Only `running`/`pending` continue polling (`v88_ag`'s hang). `test/lib.js` gained **`postJob()`**, the test-side mirror, shaped as `{status, body}` so existing e2e payload assertions were untouched. ⚠️ **`_jobsTracked` and the whole `kind:'sync'` path now have NO callers** — marked superseded in place; deleting them is a separate release. Nine mutations red |
| **`v88_am` — the pencil popover on the CARDS (library storylines + chapter cards)** | User request extending item Y. ⚠️ **A second mechanism on purpose**: `_slEditMenuSync` MIRRORS static markup (one header, real elements); cards are built as STRINGS per instance, so **`_cardEditPopHtml(popId, entries)`** builds rows from the button HTML the caller already emitted, with the label it already carried — which is why it costs zero keys and every handler survives. `.card-edit-pop` is `position:fixed`, placed from the pencil's rect and viewport-clamped: a card header is a flex row in a scrolling list, where an absolutely-positioned child is clipped or painted under the next card. ONE popover open page-wide. A leading icon is stripped from labels (the row already shows the button). Library storyline card keeps `▶`/`🔗`; the chapter card has NEITHER (its body is the play affordance), so its row holds only the pencil. ⚠️ Applied to BOTH contexts of `savedItemHtml` (storyline chapter AND standalone library item) — one renderer looking different in two places would be stranger than the decluttering; **one line to reverse**. ⚠️ Three already-documented traps hit here: backticks in a comment inside a template literal; the auto-vivify trap (test created its own element while the code used the vivified cached one); and containment-as-proxy twice (the pencil counted as an action; a label assertion satisfied by the button's own `title`). Seven mutations red |
...
| **⚠️ `v89_z` — ONE text box, reversing `v88_ab`'s "keep the split"** | The user asked why the card had a separate box for text drawn into the scene, and the code answered them: `_comicTextFromFields` joins `caption`+`inScene` with a single newline, every consumer re-joins identically, and a story edit already collapses them (`caption = corrected`, `delete inScene`). **No consumer ever distinguished them** — the split was the extraction contract leaking into the UI. Now `_comicReviewText(buf)` renders both, newline-joined, into one `rows="6"` textarea and `_comicReviewEdit(k,'text',v)` writes the whole value to `caption` and **CLEARS `inScene`**. ⚠️ The clear is load-bearing: a surviving `inScene` gets re-appended downstream, so a line the learner deleted comes back. The prompt's `CAPTION:`/`IN-SCENE:` contract, `_parseComicExtraction` and the stored shape are UNCHANGED — two named slots is a better prompt than one; it just no longer surfaces as two boxes. `form.image_scene_ph` stays in `ui.json` (hand-translated into five languages, free to keep). Guards: `unit-comic-review-card.test.js` §v89_z incl. a round-trip check that what the box SHOWS `===` what `_comicPanelText` DERIVES. Five mutations red |
| **⚠️ `v89_aa` — the capitalization MECHANISM, because the instruction was not one** | `_comicExtractPrompt` has asked for normal capitalization since `v85_k`, with a German worked example `v85_l` proved necessary — and the corpus holds THAT EXAMPLE'S OWN SENTENCE (`ES GIBT EIN LAND, WO DIE KÖPFE…`) transcribed in full caps anyway. The prompt is kept; a mechanism now catches what survives it. **`shoutedRun`/`needsCaseNormalise`** (floor 4, MEASURED over all 20 corpus panels: failures 9-33, deliberate capitals 1-3 — `REIZEN DOOR ZEELAND` and `GRATIS/KOSTENLOS` are real signs and rewriting them would be the bug) gate an automatic pass inside `_runComicExtractJob`. **`textNormaliseChanges`** is the contract — SURFACE only, the sibling of `cleanTextChanges`' deletion-only: word count and LINE count pinned (`v88_z`: a line break is a sign's real boundary), a changed word allowed only as a bounded repair. ⚠️ **`_surfaceKey` folds UP, not down** — a live run caught it: German ß upper-cases to SS, so `GROSSES`→`großes` is a CASE change; folded down it blew the repair budget, was rejected, and the retry returned a worse timid result. **`normaliseExtractedText`** wraps it in `cleanNarrativeText`'s shape (3 attempts, pointed feedback, returns the input UNCHANGED rather than throwing — a throw inside the extraction job costs the panel its transcription). Model MEASURED across 4 real panels: translategemma:12b 4/4, qwen3.6:35b-a3b 3/4 (one item returned entirely lower-case), qwen2.5:14b 3/4 — an earlier draft picked `OLLAMA_MODEL` off ONE sample and was reversed; it uses the QC role, whose default already is translategemma:12b. **`/api/text-qc`** is the on-demand half (batch + `runAsJob`), and ⚠️ **deliberately does NOT gate on the detector** — typos leave no all-caps trace, so gating would ship a typo check that cannot see typos. Client: `_textQcRun` behind `comicReviewQc()` and `pdfTextQc()`; they send DIFFERENT languages (`APP.lang` vs `APP.srcLang`), each matching what its own surface already sends. Guards: `unit-text-normalise` (9 mutations red), `unit-text-qc-ui` (10 red, after 3 vacuous ones were found and fixed) |
| **`v89_ab` — the text QC PROPOSES, reusing story QC's own panel** | User question (*"is this the same QC we already had for story texts?"*) then ruling. **They are NOT the same** — `runStoryQc` (`v55_g`) runs on a SAVED chapter, MAY change words to fix grammar, and is verified statistically; `/api/text-qc` runs on text that is not a chapter yet and is forbidden from changing a word, because **extracted text is a transcription of something real** and "fixing" a photographed sign's grammar falsifies it. But the older one showed a reviewable DIFF and this one applied straight away, so it now goes through the same **`_renderQcProposalInto`** (`v55_n`, already factored for reuse). That function gained exactly two options — **`prop.pairs`** (caller-supplied rows: the text QC's unit is one row per ITEM, not per sentence, because it is a batch) and **`o.cleanKey`** (`qc.clean` says "the story is already clean", wrong for a panel; it passes `ex.writing.no_issues`). Both existing callers untouched, **zero new `ui.json` keys**. `textQcAccept()` applies only TICKED rows via `_qcSelectedIdx`, matching `acceptStoryQc`'s reading (undefined → all, [] → none + `qc.none_selected`). ⚠️ The PDF backup moved to ACCEPT time — taken before the run it would arm undo for a discarded proposal. ⚠️ `_comicReviewClose` drops a pending proposal but **only its own panel's**: the overlay is rebuilt per open, so a survivor would hold an `apply` closure over a replaced buffer. Guards: `unit-text-qc-ui` §1/§2 re-scoped, §3b partial acceptance, §3c ownership. Nine mutations red |
| **⭐ `v89_ac` — ONE QC engine (`qcProse`), two MODES, one picker** | User ruling. Three functions collapsed into one: `generateStoryQc`, `generateSummaryQc` (a near-verbatim copy of it) and `normaliseExtractedText` — the first two are now thin wrappers keeping their route payload shapes, the third keeps the shape its two callers read. ⚠️ **`mode` names a BEHAVIOUR, not a flag on one**: `qcModeSpec` picks a prompt AND a verifier AND a retry policy together — HEAVY = `storyQc` + `classifyStoryQc` + one shot (may change words); LIGHT = `textNormalise` + `textNormaliseChanges` + 3 attempts + hands back the input rather than throwing (may not change a word, because a transcription has ground truth). `scriptPinNote` stays HEAVY-only — `v79_f`'s transliteration risk exists only where rewriting is allowed. `readQcMode` is the single validator for all three routes; `/api/text-qc` defaults LIGHT, story/summary default HEAVY. ⚠️ **Behaviour preservation is proven by a DIFF, not by a green suite**: `test/fixtures/qc-premerge.json` holds 9 outputs captured from the pre-merge functions and `unit-qc-unify-parity.test.js` replays each one's reply sequence through the merged engine. (A first draft read the old source with `git show <tag>:server.js` — fine here, red on any clone without the tag.) Client: **`_qcPickMode(dflt)`** reuses `showChoiceDialog` in front of ALL FOUR QC buttons; the per-surface default is a safety property (extracted text offers light first and primary). ⚠️ Two bugs found in a BROWSER, not by tests: the picker rendered at z-index 300 BEHIND the comic review card's own 400 — `showChoiceDialog` now computes its z-index above every open overlay — and a flagged heavy verdict showed no warning, since the batch proposal carried none; the worst row now escalates to the proposal, inheriting `v86_h` (rewrite still accepts, only corrupt withholds). 23 mutations red |
| **⭐ `v89_ad` — every durable store is written ATOMICALLY (`atomic-write.js`)** | The flake audit's real finding, and it was not in the tests. `unit-ui-journeys`/`unit-word-progress` measured 40/40 standalone, 60/60 under seeded shuffles, 15/15 under 8-way CPU load — clean. The one untried condition: `lessons.json` being rewritten underneath them, which the user's live server does all day. **3 failures in 25 runs** (`SyntaxError: Unterminated string in JSON`), **0 in 25** through an atomic writer. ⚠️ `fs.writeFileSync` on a 10MB file is a truncate plus many `write()` calls; the torn read is the mild consequence, a **crash in that window truncates the corpus and there is NO backup anywhere in this project**. Same exposure on `learners.json` (credentials, written on every answered question), `ui.json` (hand-translated ×5), `drafts.json` (item R's durability story) and `canonical-analysis.json`. It was already known and worked around at ONE site — `reloadUI` skips a parse error "e.g. mid-write". **`writeFileAtomic(file, data, {mode})`** writes `FILE.tmp<pid>` then `rename(2)`; the temp is the target path plus a suffix so it can never cross a filesystem. All SEVEN live-server writes converted. ⚠️ **Two traps that make this not a blind sweep**: `fs.watch(path)` follows the INODE, so `saveUI` must RE-ARM `_watchUI()` or the user's hand edits stop hot-reloading; and `rename` preserves the TEMP's mode, so `learners.json`'s 0600 goes on the temp or credentials are published at 0644 for that window. Guard: `unit-atomic-write.test.js` races a real reader against a real writer process, with a non-vacuity arm requiring the OLD writer to be caught. Seven mutations red, two of them only after the first pass came back green |
| **`v89_ae` — ui.json hot-reload survives more than ONE hand edit** | `fs.watch(path)` follows the INODE. `v89_ad` made `saveUI` re-arm after the server's own atomic write, which covered half the problem: the common way for anything ELSE to save a file is write-temp-then-rename — `sed -i`, VS Code and vim's default all do it. Measured (in-place 1 / 1st rename 4 / 2nd rename **4**): the app picked up the user's FIRST hand edit to `ui.json` and silently ignored every one after it. ⚠️ Pre-existing, and the symptom reads as "my translation is wrong" rather than as a stale cache. **`_watchUI` now re-arms inside its own callback**, not only from `saveUI`; closing and reopening a watch raises no event, so it cannot feed itself (after: 1 / 4 / 7). Guard: `unit-atomic-write.test.js` §5 DRIVES the real `_watchUI` lifted from `server.js` against a real file through one in-place edit and two renames — a source check for "contains a re-arm" could not fail for the thing that matters. ⚠️ Its own first draft settled with `execFileSync('sleep')`, which BLOCKS the event loop so the watcher's `setTimeout` never ran and it read 0 reloads: a synchronous sleep in a test waiting on a TIMER measures nothing. Two mutations red |
| **`v89_af` — storyboard is a JOB; a stalled machine is not a dead Ollama** | Two user reports. **(1)** `/api/storyline-storyboard` AWAITED its generator and answered 200 — no popover row, no progress, no cancel, on what its own comment called "a synchronous ~30-min call". ⚠️ The same defect `v88_ag` fixed for story/summary QC; that release's "all EIGHT formerly-blocking model routes" never counted this one because it is reached from the STORYLINE screen, not a lesson card — the enumeration was of the wrong set. Now `runAsJob` with label + `link:{type:'storyline'}` + `jobStep`, validation still OUTSIDE the job (`v88_al`). ⚠️ **BOTH** client callers converted: the post-generation pass hits the same route and would have read the 202 `{jobId}` as a missing storyboard and dropped it silently. **(2)** The false "Ollama unreachable" on wlan loss — the network was never the cause. Measured: Ollama listens on 127.0.0.1 ONLY, `localhost` resolves from /etc/hosts in 3ms, **360 probes during live wlan flapping gave ZERO failures**, and inference load peaks at 45ms. What the box shows is `llama-server` at 22.4GB, `free` 0, 11.4M pages swapped in — and **1564 `ip-config-unavailable` events in one boot, i.e. DHCP timing out**. The wlan drop and the offline flip are SIBLINGS: the machine stalls and both miss their deadlines. Fix: **`pingFailureIsHard(code)`** — a refusal is proof of absence, a TIMEOUT is not; hard needs 2 checks, soft needs `BACKEND_SOFT_FAILS` (4), and the background re-check gets its own `BACKEND_PING_TIMEOUT_MS` (15s, was 2s) since nothing waits on a 60s timer. `_pingOllama` records `{code, ms}` (`lastPingFailure()`) and the offline line now NAMES the reason. ⚠️ A named function, not a ternary: inline, a mutation flattening it to `true` — the reported bug exactly — left the suite GREEN. Guard: `unit-storyboard-job.test.js`, incl. a BEHAVIOURAL §4 racing a dead port, an accept-then-silent port and a live one. Nine mutations red |
| **`v89_ag` — a running story/book job is cancellable from the popover** | User report: "creating a story has no cancel/open buttons". ⚠️ The cancel was a DELIBERATE exclusion whose reasoning was right and conclusion wrong — `_jobsRenderList` excluded `kind:'book'` because multi-chapter generation "lives in the separate bookJobs store with its own cancel route, which this one does not reach". The answer is to REACH it: a book job is the longest-running thing in the app, `/api/book-job/cancel` already worked, and `pdfCancelBook()` had been calling it all along. `canCancel` widens by exactly one kind and **`_jobsCancelById(id, kind)`** dispatches route AND body (`{bookId}` vs `{jobId}`), resolving the kind from the effective list by id the same way the open button resolves its link. ⚠️ `sync`/`tutor`/`draft` are STILL excluded — no server-side job, so the button would be a lie (the original comment's real point). ⚠️ The route had to learn to tell the truth first: it answered a bare `{ok:true}` either way, and `v88_k`'s ruling is that claiming "cancelled" while the model runs is the bug that button exists to prevent — **a caller cannot be honest about an outcome the route will not report**. Now `{ok, stopped}`, via **`cancelBookJob(bj)`**, a NAMED function because inline a mutation reporting `stopped:true` unconditionally left the suite GREEN (the `pingFailureIsHard` lesson again). The OPEN button is honest, not broken: `link` needs a saved `topicId`, which does not exist until chapter 1 lands. Guard: `unit-book-job-cancel.test.js`, nine mutations red |
| **⚠️ `v89_ah` — the token popover's Save was broken in the MARKUP, and the guard pinned it** | User: "editing text analysis individually didn't work, save had no effect." Never a logic error: `JSON.stringify` emits DOUBLE quotes and they were interpolated into a double-quoted `onclick`, so the browser parsed `onclick="_teSaveCorrection("` and the handler threw SyntaxError on every click — nothing was ever sent. Fixed with `escAttr(saveCall)`. ⚠️⚠️ **`v88_ai` FIXED THIS EXACT BUG on the ▤ button next door and never swept for the second instance** (`v88_ad` had added it); a sweep now finds zero remaining. ⚠️ `v88_ai`'s comment also records that its own guard had asserted the markup merely CONTAINED the function name — and `unit-text-explorer` then asserted the popover contained `_teSaveCorrection("tp_te1",…)` UNESCAPED, i.e. **it required the broken form**. Re-pointed to `getAttribute('onclick')` (entity-decoded, as a browser sees it) plus `new Function(...)` so "the browser can RUN it" is the claim. Every prior test drove the FUNCTION, never the BUTTON — `v89_x`'s lesson, repeated. Second defect found alongside: the dismiss was fire-on-ANY-click `{once:true}`, correct while the popover was read-only, so after `v88_ad` the first click INTO a field killed the editor; now `pop.contains(e.target)` + Escape, listeners torn down in `_teCloseWordPop`. ⚠️ That failure was DESTRUCTIVE — three empty strings is the route's CLEAR gesture, so a fieldless save DELETED the correction; `_teSaveCorrection` now refuses unless `_teWordPopEl` is open (an inputs-only check cannot fail in lib-dom, which auto-vivifies a miss). The blue line is NOT a bug: `.te-tok-reviewed` is `v88_ad`'s curated marker. Guard: `unit-te-popover-save.test.js`, five mutations red |
| **`v89_ai` — the jobs badge updates when a job STARTS** | User: the count only appeared after opening the popover. The old policy — refresh on screen change, 3s poll only while the popover is open — was deliberate and is KEPT; polling was never the answer, because the client knows the instant it holds a `{jobId}`. **`_jobsBump()`** re-reads `/api/jobs` immediately plus once after 4s (so a job that finishes quickly does not leave a stale count). ⚠️ FIVE call sites: `_jobAwait` covers every `runAsJob` route at once, and `_startComicExtractJob`, `_startComicDetectJob`, `_pollBookJob`, `_pollComicBookJob` each run their own poller and never reach it. Guard: `unit-jobs-badge-live.test.js`, incl. §4 proving an idle client issues NO requests (this did not become a standing poller). Five mutations red. ⚠️ The test itself first HUNG after printing ALL PASSED — a stub that never settled left a 100s `_jobAwait` timer pending, which in `run.js` hangs the whole suite (`_qcPoll`'s own recorded hazard); found by instrumenting `setTimeout` |
| **⭐ `v89_aj` — provenance is edited on the STORYLINE and INHERITED by chapters** | User ruling. **`sourcesEqual`** + **`effectiveSource`** (server) and **`provEffective`** (client): a chapter's stored `source` MEANS "this chapter differs", its absence means "inherit", and every read goes through the resolver. ⚠️ Equality treats ABSENT and BLANK alike — the editor always sends four fields, so otherwise nothing would ever match and every chapter would keep a redundant copy. ⚠️ A duplicate is NOT inert: `/api/topic-source` deletes an entry equal to the storyline's, and `/api/storyline-source` frees chapter entries that now merely repeat it — left in place they silently detach that chapter from every later storyline edit. ⚠️ `upsertStoryline` merges `{...existing, ...sl}`, so a `delete` is undone by the spread; clearing writes null then drops the key after the merge. ⚠️ ~~Chapter `source` had to join the **savedList whitelist**~~ — CORRECTED at `v90_a`: it was ALREADY projected, and this added a redundant duplicate key. Not an instance of the `v74_i`/`v79_n`/`v89_y` trap. Client: **`provCompactHtml`** (author · licence, clickable, DOI-expanded via the existing `_provSrcBits`, `event.stopPropagation()` because these lines sit inside clickable cards) painted by **`renderProvInto`** on three surfaces — landing card (a second `.storyline-title-sub`, so "same font" holds by construction), storyline page (with the teacher-gated `openSlProvEdit` pencil), completion card (EFFECTIVE, i.e. usually inherited). ⚠️ The new node first reused `id="sl-screen-prov"`, which ALREADY belonged to the page's provenance/stats footer — `getElementById` handed it to that renderer too and silently broke it; `e2e-pass-mark`'s ordering assertion caught it, renamed `sl-screen-src`. **ZERO ui.json keys** (the `prov.*` family was already translated). Guard: `unit-provenance-inherit.test.js`, nine mutations red |
| **⭐ `v89_ak` — the job-coverage ENUMERATION (not another instance)** | `v88_ag` claimed "all EIGHT formerly-blocking model routes are now listed"; it was wrong twice and a USER found it both times (`v89_af` storyboard, `v89_ag` book cancel). This walks EVERY route block in `server.js` by real brace matching, classifies which reach a model (directly or via a generator), and requires each to be a listed job or carry a REASONED entry in an explicit `EXEMPT` map. ⚠️ A first hand sweep with `awk` over fixed windows misread three routes — **a wrong inventory is worse than none, because it retires the question** — and was thrown away. Result: **21 model-backed routes**, 18 already correct, 1 named exemption (`/api/tutor`: streaming/stateless, in the popover as a client-derived synthetic entry), and **2 real gaps nobody had reported**: `/api/clean-text` (awaited up to THREE model calls per chunk, looped per chunk by `aiCleanChunks`) and `/api/split-chapters` (one call over a whole document). Both now `runAsJob` + label + `jobStep`, validation OUTSIDE the job (`v88_al`), both client callers on `_jobAwait`; clean-text is one job per chunk so exactly one row is live at a time and a cancel stops the BATCH. Two e2e files moved to polling the job. Guard: `unit-job-coverage.test.js` — the enumeration itself, plus "every converted route has a polling caller" and `v89_ag`'s cancel rule as a RULE. Seven mutations red, the decisive one a SYNTHETIC new model route added mid-file and caught by name |
| **⭐ `v89_al` — item V: multiple images, EACH its own panel** | Open since the `v86` line. ⚠️ The recorded ruling ("mark all images as one panel") parses BOTH ways; the user clarified at this cut that it means **each image is a panel** — N images, N panels — and the clarification is attached wherever that sentence is quoted. `roadmap_v88.md`'s other warning ('each image is a panel' vs 'a chapter' diverge once several panels are drawn on one of several images) is settled: images are PANELS, and `comicCreateChapter()`'s one-chapter-per-panel (`v85_p`) does the rest. ⚠️ **THE SEAM**: `dataUrl`/`naturalW`/`naturalH` still mean the ACTIVE page, so ~20 canvas / hit-test / redraw / resize / move sites are untouched; new are **`APP_COMIC.pages`**, `pageIdx`, and a **`page` per box**. ⚠️ **`_comicCropDataUrl` now follows the BOX, not the screen** — it cropped the displayed image, which is right for one page and silently wrong for several. Kept synchronous via a per-page offscreen `Image`, with a whole-page box short-circuiting to that page's dataUrl. The SINGLE-file path is byte-for-byte unchanged (it carries this region's test surface and `v85_u`/`v86_c`/`v88_c`'s bugs); `_comicAddFiles` is the multi path. `comicUseWholeImageAsPanel` now replaces only the ACTIVE page's boxes. Draft: `page` per box plus `comic.pages` (page 0 stays `dataUrl` — older builds still resume, and a ~1MB page is not sent twice); ⚠️ BOTH had to join the server draft whitelist, where `description` was dropped once already (item AN) — a stripped `page` collapses every box onto page 0. Guard: `unit-comic-multi-image.test.js`, eight mutations red; the hit-test one needed a point inside BOTH boxes before it could fail |
| **`v89_am` — the dead `kind:'sync'` path is DELETED** | `_jobsTracked`, `_jobsInflight`/`_jobsInflightSeq` and the `kind:'sync'` branch of `_jobsEffectiveList`, carried as an explicit follow-up since `v88_al` (its own comment said so in place). Confirmed dead first: the only remaining references were the definition and its comments. ⚠️ **The TUTOR synthetic entry is NOT part of this and survives** — its source of truth is `_tutorState.busy`, a flag set from two places that survives a re-render, and it is the only synthetic row carrying a `link`; `v88_b`'s own comment records it was deliberately never migrated onto the registry. The surviving CLAIM from `unit-jobs-sync-inflight.test.js` §5 — the five `v88_al` routes awaited as jobs at all NINE call sites, a claim about a SET no rendered state can observe — moved into `unit-job-coverage.test.js` and that file was deleted (so the check count drops by one). Three orphaned comment paragraphs replaced by one history block ending in the rule that outlives them: **any future blocking route uses `runAsJob` + `_jobAwait`, not a client-side row.** ⚠️ `'sync'` is kept in two cancel-button guards ON PURPOSE (the rule is about any kind with no server-side job) and annotated so it is not tidied away. Three mutations red — including deleting the TUTOR entry too, without which §6 would only be asserting "nothing is there" |
| **⭐ `v89_an` — 29 dead `ui.json` keys removed; the DETECTOR is the finding** | User asked, before the v90 cut, whether every key is still used. 766 → 737 `en`; **957 translated entries** across 33 languages, all hand-written. ⚠️ Deciding a key is unused is easy to get wrong the expensive way, and TWO of three traps were hit live: **`'prefix' + var`** (`_sbSchemeLabel` builds `'storyboard.scheme_' + name` — a literal-only search called five scheme names dead; first pass said 55 keys, corrected 34) and **`var + '_suffix'`** (`_synQ` builds `t(key + '_n')`, and since `_synN` is the count of correct words **that is the string shown MOST of the time** — a prefix-only detector called the PRIMARY string dead; **the USER caught it** by asking for the synonym plurals to be checked first). Third: **`grep`'s `.` is a wildcard**, so `app.tagline` "matches" `app-tagline` and `qc.accept` matches `qc.accepted` — my own verification pass was wrong until keys were matched as whole QUOTED strings. Three keys HELD (a test names each), listed with reasons in the guard's own `HELD` map. 24 duplicate-English groups REPORTED, not merged — `_plural` pairs exist for languages whose rules differ from English, so it needs a per-group ruling. Guard: `unit-ui-keys-live.test.js` re-runs the detector every cut and pins all three blind spots with live examples plus negative partners. Four mutations red |
| **⭐ `v90_l` — the "Ollama unreachable" flapping was IPv6 LOOPBACK, not the wlan** | Reported across three releases and blamed on the network every time. The log line that settled it: `ECONNREFUSED ::1:11434`, refused **in 1-5ms** — a LOCAL refusal, no packet ever left the machine. Measured: Ollama binds `127.0.0.1` ONLY (`ss -ltn`), `::1:11434` → ECONNREFUSED in 6ms, `127.0.0.1:11434` → HTTP 200 in 3ms, and the app's default was the NAME `http://localhost:11434`, resolved on every call. **Which candidate the resolver returns first is what changes when an interface goes up or down** — hence the wlan correlation, and hence `v89_af`'s honest report that a clean wlan toggle could not reproduce it. Fix: the IPv4 LITERAL as the default in `llm.js` and `server.js`, plus `family: 4` pinned when the configured host is a loopback NAME — ⚠️ only loopback, since a real host may legitimately be IPv6-only. All SIX request sites carry the pin: the flap came from `ping()` and the failures from the generators, so one unpinned site reproduces everything. Guard REPRODUCES the fault (a server on IPv4 loopback only; IPv6 refused in single-digit ms), 4 mutations red. ⚠️ Not fixed here and worth its own look: an immediate 3× retry cannot outlast the fault (chapter 2 of 3 died that way), and every spurious `reachable again` triggered a real model warm-up. **Closes the `v89_af` unresolved item** |
| **⭐ `v90_k` — the i18n audit re-derived: 86 certain, not 143** | The client localizes through at least FOUR idioms (`_setAttr`, `_setText`, `set(id, fn)`, and a bare `el.textContent = t(k)` two statements later), so a source scan mis-classifies in BOTH directions — it called `fin-title` hardcoded and `topic-name-big` localized, and both were wrong. ⚠️ **`lib-dom` did not model `<select>.options`**, which `applyUIStrings` walks, so that function THREW in the harness and nothing about it was measurable; adding the getter is what made the re-derivation possible. Method per class: **A** markup title/placeholder with an id — MEASURED, since `getElementById` auto-vivifies a BLANK element, so a value after `applyUIStrings()` means it wrote there (63 → 27 localized, 1 keyed-but-identical, **35 bare**); **B** literals in `showToast`/`confirm`/`alert` — certain, no fallback exists (**10**); **C** markup text with an id — certain only when nothing addresses the element (70 → **10 certain**, 60 undecided); **D** literal attributes with NO id — certain, `applyUIStrings` addresses by id (**31**). **Earned rule: an audit that cannot RUN the code it audits is a list of SUSPECTS** — every wrong entry came from inferring at a distance what a function would do to a string. Guard in `unit-library-sort`, two mutations red |
| **⭐ `v90_j` — the library sort did NOTHING in the static build** | User asked whether it worked; it did not, and every existing check missed it because every existing check drives `index.html`. Three layers: `onLibSortChange`/`onLibSortDirToggle` sat ABOVE `@static-exclude-start`, so `docs/index.html` rendered both controls and neither function was in its bundle (`typeof` → `undefined`, a ReferenceError per click); `build-static.js` re-implements `loadSavedList` with a hardcoded "newest first" plus a second re-sort by target language that would have overridden the key anyway; and ⚠️ the token key would still have compared zeros, because `t.tokens` is a scalar the LIVE projection SUMS — **`v88_j`'s comment claiming static "has the field for free" is wrong, 0 of 355 baked topics carried it**, and is corrected in place. Fix: `_libSortInto` moved below `@static-exclude-end` with both handlers, called by BOTH builders, with a `generationStats` fallback for the token sum. ⚠️ The guard drives the BUILT ARTIFACT and its token assertion had to be rewritten after mutation-testing — "the order differs from the date order" SURVIVED removing the fallback (all-zeros → stable sort → incoming order), so it now computes an oracle from the baked corpus. Four mutations red. ⚠️ **And a correction to my own audit**: the six sort labels were already keyed and translated in 32 languages — the markup English is the first-paint FALLBACK, the exact class I had named one message earlier and then failed to apply to my own list |
| **`v90_i` — the last English label in the story menu** | One granted key: `qc.story_btn` = "Proofread with QC model", the exact string the button already carried, inserted directly after `qc.summary_btn` in `ui.json` so a translator meets the pair together. ⚠️ The guard now walks `_EDIT_MENUS['ls-story'].rows` and fails if any row's button is missing from its table of `t()`-backed titles — a sixth control cannot repeat `v90_g`. The "already translated in 20+ languages" half stays scoped to the four REUSED keys: a freshly granted key would go red on it. Two mutations red |
| **⭐ `v90_h` — a tooltip that moves into a menu becomes VISIBLE TEXT** | User: "the text 'Re-translate (after fixing the story text)' is not in ui.json?" It was not, and neither were three neighbours. Until `v90_g` that cost only a tooltip; once the buttons moved into an edit menu, `_editMenuSync` takes each row's label FROM the button's `title` — so the whole menu read English in every language. **The mechanism that made the relocation cost zero keys is what made this worse.** All four given existing keys (`text_explorer.toggle_title`, `gen.post_gen_analysis_lbl`, `lesson.edit_story`, `models.translation` — the last on the user's own instruction, and already the job label for this call). ⚠️ The 🔍 story-QC title still has no key. ⚠️ **`lib-dom`'s runtime `setAttribute` did not REFLECT onto the property**, so `.title` read empty while being right in a browser and the check could not be written; now reflects for `PROP_ATTRS`, as the DOM does. ⚠️⚠️ **And a real `v90_g` defect the CORPUS caught within the hour**: `/api/save-translation` stamped `origin: 'user-provided'` while writing only `storyTranslation`, breaking the invariant "user-provided ⇒ `userTranslation` exists" — **the stamp claimed a human wrote the text while the record showed nobody had**. Route now sets both; a boot heal in `fixMetaSource`'s pass repairs rows already written that way, firing on that one shape only. Guards: `e2e-translation-edit` §2 and a new §4b that boots a second server and checks a GENERATED translation is left alone. Containment trap hit a third time — the "parenthetical is gone" check fired on the comment QUOTING the request |
| **⭐ `v90_g` — three pencils, an editable translation, a broom (5 user requests)** | `_slEditMenuSync` became **`_editMenuSync(key)` over a `_EDIT_MENUS` registry** — three menus now (storyline header, lesson-set story row, lesson-set title row), each mirroring its buttons rather than re-deciding visibility, labels from each button's own `title` (**zero keys**), pencil hidden when every row is, and **only one open page-wide**. Generate-summary moved from the header popover onto the summary's own `_cardEditPopHtml` menu, same id (`genStorylineSummary` finds it for its ⏳) with ✨. ⚠️ Asked the user which button "the text-analysis" meant (🔬 explorer vs 🔤 analyse) — 🔤; 🔬 and 💬 stay in the row as the LEARNER's controls. **Translation editing** is the story editor with one flag (`APP._storyEditMode`), saving through **`/api/save-translation`** — its own route because `/api/save-story` sets `aiStory`, **collapses the story into a comic panel's caption**, invalidates curator corrections and regenerates the AI hunt, all about the TARGET story; no backend gate (a person is typing) and stamped `(user-provided)`. Panel switches to source, explorer off, AI-hunt diff suppressed, mode cleared on save AND cancel. ⚠️⚠️ **A mutation helper used `git checkout -- index.html` to restore between mutations and discarded every unstaged change in one command** — rebuilt from the transcript. **A restore step must never be a VCS command**; `mut.sh` in this same line already restored from a byte copy. THREE granted `ui.json` keys; nine mutations red; `unit-can-edit-teacher-mode` §4 and `unit-static-summary-edit` both fired and were right |
| **⭐ `v90_f` — the last four zeros, and the audit closes** | The orchestration functions `v90_e` set aside: `startLesson` 0→10/10, `goLessonSet` 0→8/10, `doDialectImport` 0→20/28, `renderEx` 0→19/28. They close over most of the client, so they are driven through `lib-dom`'s `loadClient` rather than lifted — `test/unit-lesson-entry-journey.test.js` is the new home for the first three's entry path. The claims asserted are the ones their comments record as user-reported bugs: the storyline "keep fixed" language setting that `goLessonSet` used to override on every chapter change; `startLesson`'s return value, which `loadSaved` routes on (a silent false strands a learner); `renderEx`'s `v88_r` crash (a stray speech timer landing on a review card, where `C.cur >= length` is false for `undefined`), its drill teardown (`v71_h`/`v71_n`, "studiare asked over and over"), and its listening questions being QUEUED for the audio unlock rather than dropped; and `doDialectImport`'s in-flight state — button disabled, status set, previous report cleared — which no post-hoc assertion can see. ⚠️ **Most remaining survivors are `if (el)` guards mutated to `true`, and `lib-dom`'s `getElementById` AUTO-VIVIFIES a miss** — equivalent by construction in that harness, not gaps. Audit complete across `v90_b`…`v90_f`; `tools/branch-mutation.js` keeps it repeatable |
| **⭐ `v90_e` — the remaining zeros, and a correction to the probe** | ⚠️⚠️ **`v90_d`'s own numbers were wrong once**: `branch-mutation.js` knew the extraction helpers `ext`/`extract`/`extAsync` but not **`lift`**, so `qcProse` was attributed to `unit-qc-correct` alone and scored 0/16 while `unit-qc-unify-parity` was lifting and running it all along (really 7/16). **A missed helper name inflates the zeros.** Pattern widened, pinned by `unit-branch-mutation-tool` §5. Every zero then escalated to the whole `--quick` suite (subtracting `unit-static-freshness`, which every `index.html` mutant reddens): **3 were caught elsewhere and were never gaps** — and ⚠️ **v90_d's guess that `renderEx`/`startLesson`/`goLessonSet`/`loadSaved` were covered by `smoke-render` was WRONG**; it is in `--quick` and caught none of them. Five repaired: `qcProse` 7→15/16 (the whole error path — cancel propagates without retrying, heavy throws where light degrades, script pin is heavy+story; a captured return value cannot record a throw), `_sbMarkCurrentPanels` 0→8/8 (the frame must be CLEARED when the chapter changes), `openStoryboardChapter` 0→4/6 (2 mutually-masking defensive guards), `_editorReadInputsMath` 0→6/6 (an empty field must LEAVE the stored value alone), `_applyUploadCleanup` 0→7/8 (lossless both ways, pages as a copy, and the v69_p report's counts are the FULL text's not the last page's), `_buildGlobalTtsSelectors` 0→13/14 (waits rather than rendering an empty menu; a saved voice the system no longer has is not preselected). Still open by design: `doDialectImport`, `renderEx`, `startLesson`, `goLessonSet` — orchestration functions needing journey-level fixtures, not the lift-and-drive pattern |
| **⭐ `v90_d` — the SAME-ANSWER FIXTURE blind spot, measured** | `v90_b`/`v90_c` asked *is the function executed*; this asks *does the guard notice when it behaves differently*. New probe `tools/branch-mutation.js` rewrites each `if (cond)` inside a function to `true` and to `false` and runs the tests that LIFT it. **86 functions, 1000 mutants, 46% caught, 14 scored ZERO.** ⚠️ A zero means *the lifting guards do not notice*, NOT *nothing notices* — escalate before calling it a gap, and expect equivalent mutants that no fixture can kill. Two clusters repaired: `_splitLongUnit` 0/12→20/24 (it RAN — the over-budget path really executes — but the word-count invariant is invariant under where the cut lands, and the `frag` flags had no assertion at all) and the provenance renderers 3/20→20/20 (`unit-provenance-fields` used `ext()` as a slicing tool and never fed it to `new Function`, so every claim about the rendered line was a regex over its own source). ⚠️ Fell out of it: **`translate-ui.js` wrote `ui.json` with a bare `writeFileSync`** after every batch for hours — `unit-atomic-write` §4's "one-shot maintenance script" exemption is about a LIVE READER, not the word "script" — which the user hit as a `--quick` failure in `unit-library-sort`. The probe is guarded by `unit-branch-mutation-tool` because its failure is inverted and silent (0 mutants prints as 100% caught) — **and that guard was itself same-answer twice on the first try** |
| **⭐ `v90_c` — the audit's remainder, and a correction to its own suspect list** | Seven targets `v90_b` left open, one mutation each. **Four were FALSE POSITIVES**: `showComplete`, `callLLM`, `sourceFingerprint` and `fixMetaSource` are all properly guarded — `fixMetaSource` by `unit-meta-source-heal`, which boots a server against a synthetic UNHEALED store and never mentions the function by name. `_renderCompStoryboard` was an ABSENCE guard the scan misread; `scriptLessonAvailableForSet` was already reasoned about. ⚠️ **The lesson is about the screen**: a static "names a function, never calls it" list is SUSPECTS, 57% noise here, and three of the four were cleared only by the FULL-SUITE tier — the cheap middle tier cleared none, because the guards that catch them are named after the behaviour, not the function. **A screen's output is not a finding until the whole suite has disagreed.** The three that were real are all in `backfill-provenance.js` (`classifyOrigin`, `flagValue`, `stampTranslationMeta` → constants, all 360 green). It could not be `require`d — `const data = JSON.parse(readFileSync(FILE))` at module scope, `FILE` hardcoded to `__dirname/lessons.json` — so `unit-provenance-migration.test.js` COPIES the script into a temp dir beside a synthetic store and runs it as a subprocess, no change to the migration itself. Now proven by running: dry-run default, `.bak` from the PRE-run bytes, five distinguishable origins, four translation stamps, idempotence, all-or-nothing refusals, flag rails that exit 2 and write nothing, `--verify` ignoring its own output. 8 mutations red |
| **⭐ `v90_b` — a mutation audit of the SUITE ITSELF** | The standing rule "mutation-test every guard you write" applied BACKWARDS, to guards already standing. Static screen over 355 test files → **16** assert a function EXISTS and never run it → 17 semantic mutations, `docs/` rebuilt each time so parity/freshness could not fire on the byte change, then the whole 360-check suite for survivors. **9 of 17 survived everything.** ⚠️⚠️ **Two guards were asserting against their own RE-IMPLEMENTATION**: `unit-model-picker` ("Replay the helper logic") and `unit-word-count` ("reimplement and check the contract") each defined a local copy of the function under test and asserted on the copy — 16 assertions that could not fail, proven by running the pre-fix file against two real defects (green both times). ⚠️ `e2e-backend-recheck` held the only hardcoded port in the suite (`19731`), making it the one test that cannot run twice at once — measured, now ephemeral. Seven guard files now RUN what they used to name (`unit-storyline-theme`, `unit-model-picker`, `unit-model-settings`, `unit-static-flags`, `unit-model-picker-roles`, `unit-diacritic-qc`, `unit-word-count`), each with its own mutations red — 21 of them. **Earned rule: "replay the logic" must mean LIFT the logic** — `extract()` + `new Function`, never a retyped copy. Not exhaustive: 17 mutations is a sample, and the screen cannot see guards that run the right function on a fixture where every branch answers the same |
| **⭐ `v90_a` — the savedList projection is a DECISION, not an accident** | `/api/lessons` returns a WHITELIST PROJECTION, while the static build ships whole topics — so a field the client needs but the projection omits **works statically and is silently dead live** (`v74_i`, `v79_n`, `v89_y`). Measured: the projection ships **22** keys, the store holds **46**, **31 are dropped** and 7 are computed-only. A scan of three shapes found NO current violation — and does not claim to prove absence, since the client reads properties off short-named locals. ⚠️⚠️ **It DID find an error in `v89_aj`**: that release added a conditional `source` spread calling it "the FOURTH instance of the trap", but **`source: l.source || null` was already there** — a redundant duplicate key, and the claim was false. Found because the mutation "projection drops source" came back GREEN. Duplicate removed; `roadmap_v89.md` and this file corrected in place. The guard cannot decide reachability, so it makes the NEXT field a decision: every stored key must be projected or sit in a `STATIC_ONLY` map **with a real reason** (`'ditto'` was rejected by its own check). ⚠️ The extractor's first version used `lastIndexOf('{', anchor)`, found an INNER brace and parsed a fragment — its own non-vacuity assertion caught it; it now walks backward balancing braces. Guard: `unit-savedlist-projection.test.js`, six mutations red, the decisive one a brand-new topic field appearing unaccounted for |
| **⚠️⚠️ TWO open modes — `_comicReviewMode` (item AQ, `v88_c`, user-reported BUG)** | The card is reached two ways and, until `v88_c`, confirming did the same thing from both. **`'generate'`** — `#gen-btn` on wizard card 3 (`doGenerate()`'s comic branch): the learner has chosen lesson types and pressed start, so confirm runs `comicCreateChapter()`, unchanged. **`'auto'`** — `_comicExtractCheckOnce()` popping the card the moment extraction succeeds (`v86_v`): the learner has **never seen card 3**, so `#gen-skip-lessons-cb` (which lives there) was still at its default and `comicCreateChapter()` read that as "generate everything" — a full run (translation, `arc[review]`, lessons) on text the user had not finished reviewing. Now the auto path calls `_comicRenderList()` (which PERSISTS the reviewed text — every `APP_COMIC.boxes` mutation funnels through it and it calls `_comicDraftSaveDebounced()` at the top, item R follow-up), then `_genWizardGoto(3)`, and RETURNS. The user's rule: *"just store chapter text as draft, no lessons yet. Translation, lessons, titles etc. should ONLY be generated on the third page."* **⚠️ The reported "card 3 has no generate button" was the SAME defect's second half**, not a separate one: `comicCreateChapter()` sets `_comicBookId`, `_applyLessonCardUI()` computes `busy` from it, and the start row correctly hides for a job that really is running. Fixing the first half fixes it — pinned by asserting `_comicBookId === null` after an auto confirm. The confirm button's LABEL is mode-dependent too (`form.comic_review_save` vs `form.comic_review_confirm`) — a button that misdescribes its own effect is how this reached a user |
...
| the acceptance tests | `unit-comic-review-card.test.js` (6 checks at `v86_v`, a 7th markup-level check added at `v86_x` below). `unit-comic-review-autopath.test.js` (item AQ + item AM, `v88_c`, 6 checks) — guarded at the BEHAVIOUR layer: the sections COUNT `/api/generate-book` requests issued by the REAL `comicCreateChapter()` under a stubbed `fetch`, because "no generation happened" is a state and a source-level check of which function `_comicReviewConfirm` names could not fail for it. §3 is the non-vacuity partner (the card-3 path still generates exactly once) — without it, "never generate" would pass and break the feature outright. SIX mutations red |
...
**`v86_ae` — `inflection_lemma`'s answer-reveal is silent, not a mispronounced target-language word**
(user report, later self-corrected to a DIFFERENT real bug — see `v86_af`/item AJ below)
⚠️ **SUPERSEDED at `v89_c`** — the user asked for the readout BACK, in the target voice, with this
row's trade-off already known. The rows below are kept as the record of WHY it was silenced; the
current behaviour is `v89_c`'s. Its own row is further down this file.
...
| the fix (⚠️ REVERSED at `v89_c`) | `check()`'s `speakOk`/`speakBad` (index.html) resolved to `''` for `ex.type==='inflection_lemma'` on BOTH the correct and wrong path — spoke NOTHING, still auto-advanced (`_speakAndAdvance`'s own `!text` short-delay path needed no change). `inflection_form` completely UNCHANGED (its own SOURCE-language label, fixed at `v82_d`, already works and was not part of this report) |
| the acceptance tests | `unit-inflection-speak-lang.test.js` — §3/§4 REPLACED (not extended) the old `v82_d`-era regression guard that asserted the OPPOSITE; §5 confirms `inflection_form` stays untouched. **`v89_c` re-scoped §3/§4 a THIRD time**, back to v82_d's shape |
...
| a SYNCHRONOUS model-backed route is a synthetic entry too (item AT, `v88_b`, user-reported) | The report was "storyline title re-generation did not appear in the popover"; the measured cause is that the JOB DOES NOT EXIST — `POST /api/storyline-retitle` awaits its LLM calls inline and never calls `newJob()`. FIVE routes share that shape: `/api/storyline-retitle`, `/api/storyline-title`, `/api/storyline-summary`, `/api/retranslate-story`, `/api/writing-feedback`. `_jobsTracked(label, fn, step)` (index.html) generalises the tutor one-off into a registry (`_jobsInflight`, a Map keyed by a per-call `__syncN__` id) that any awaited call joins for its own duration; `_jobsEffectiveList()` maps it to `kind:'sync'`, `status:'running'`, **`link:null`** (nothing to navigate to — a running call has no persisted id, so `_jobsRenderList` correctly omits the open button). **`try/finally`, not `then`/`catch`**: a rejected call must still deregister, or a failed re-title parks a permanent running row and badge count. **NINE call sites across the five routes** — `/api/storyline-title` alone has FOUR (`genStorylineTitle`, `genLessonTitle`, `genSavedItemTitle`, `genStorylineTitleFromScreen`) and `/api/retranslate-story` has two; three were found by the guard's own set-level section, not by reading. `retranslateChain` is tracked PER CHAPTER, not once around its sequential loop. **Zero new `ui.json` keys** — every label reuses the string the surface that already reported the same work uses (`retitle.working_title`/`retitle.working_chapters`, `lesson.generating_title`, `lesson.gen_topic_title`, `lesson.gen_summary`, `models.translation`, `ex.writing.grading`), so the two cannot drift. ⚠️ The TUTOR entry is deliberately NOT migrated onto this registry: `_tutorState.busy` is an existing, correct source of truth and it is the only entry carrying a `link`. ⚠️ Converting the five routes to REAL jobs is the other option and is the right one IF item AU (per-job cancellation) is taken — a cancel button needs a server-side job to cancel |
...
| **⚠️ the box `text` WHITELIST (item AN, `v88_d`)** | `POST /api/drafts` sanitises each box's `text` field-by-field, and **`description` was never listed** — so every autosave through this route STRIPPED it, and a description-only panel saved as a draft came back with nothing to make a chapter from. Found by READING the route while wiring item AO, not from a report; it would also have quietly undone AO's durability (the server writes descriptions onto the draft, the client's next autosave deletes them). Third instance of the `v74_i`/`v79_n` whitelist failure. Now keeps `title` (item AN's own new field), `caption`, `inScene`, `description`, `raw`, `error`. **Guarded at two layers on purpose**: `unit-comic-extract-durable` §4 checks the whitelist literal at the SOURCE, and `e2e-comic-extract-draft` §3b round-trips a real description through the ROUTE. Removing the field left the e2e GREEN at first — `applyExtractionToDraft` writes to the file directly and bypasses the sanitiser — so neither layer alone covered the claim |
...
| **⚠️⚠️ item AO (`v88_d`) — the SERVER writes extraction results onto the draft** | User-reported: *"I keep losing extracted text, when text extraction is started on the mobile phone."* **THREE causes**, all of which had to close: (1) the extract job carried **no `link`** — `_jobsRenderList` renders "open →" only `if (j.link)`, which is the half the user could see; (2) **`_comicExtractCheckOnce()` is the ONLY writer of `APP_COMIC.boxes[i].text`**, so when the originating tab is gone (phone locked past tab eviction, reload, app killed) NOTHING applies the result — this is the actual loss, and it is **not item AE**, which is about a *live* backgrounded tab; (3) **`jobDone()` schedules cleanup at FIVE MINUTES**, so closing (1) alone would give a link that works briefly then fails silently — worse than none. Fix: `applyExtractionToDraft(draftId, panels)` (server.js), index-aligned exactly as the client's `_comicApplyExtraction()`, **called BEFORE `jobDone()`** because that call starts the timer — the durable copy must exist by the time the job reads as finished. Uses `stampUpdated()`, not a raw ISO string (`unit-stamp-updated` §5 enforces the one choke point, and this site genuinely collides: the server write and a client autosave can land in the same millisecond). Best-effort: an unknown draft id must never fail an extraction that succeeded. Client: `comicExtractPanels()` **flushes the draft first** (`await _comicDraftSaveNow()`) — the autosave is debounced 1.5s and this is the one moment where "the draft may not exist yet" costs the whole result — then sends `draftId`. The link carries BOTH ids (`id` = the job, instant; `draftId` = durable) and `_jobsOpenComicExtract()` **prefers the draft**. It opens the review card in **AUTO mode** (item AQ, `v88_c`): nobody arriving from the popover has seen the wizard's lesson card |
| **item AN (`v88_d`) — a typed title is preserved across a RE-extraction** | A job result REPLACES the whole `text` object, so a second extraction pass would delete an authored title — item AN's own loss through a different door, found by the e2e rather than reported. Both writers carry it forward: `_comicApplyExtraction()` (client) and `applyExtractionToDraft()` (server). **They had to be fixed together** — if only one did, the outcome would depend on which writer happened to run |
...
**`v89_b` — swipe the progress card left/right = its own ← / → arrows** (user request)

| what | where |
|---|---|
| the gesture | `_cardSwipeInit()` / `_cardSwipeStart` / `_cardSwipeEnd` (index.html), directly after `_storyTapMaybeAdvance` — the two are complementary on one surface: a TAP plays (`comp-play` then `comp-next`), a SWIPE browses (`comp-prev`/`comp-next`, `v88_r`'s arrows) |
| **the decision, split out so it can be driven** | `_cardSwipeNav(from, to)` → bool. Same shape as `_storyTapMaybeAdvance` and for the same reason: the harness's `addEventListener` is a no-op, so the plumbing must stay thin and the RULE must be a plain function |
| thresholds | `_SWIPE_MIN_PX = 60`, `_SWIPE_X_OVER_Y = 2` (at least twice as horizontal as vertical) |
| it PRESSES, it does not re-derive | `comp-next`'s destination depends on ~7 branches in `showComplete()`. The swipe reads the SOURCE buttons `_syncCompHdrNav`/`_mirrorNavBtn` already copy from, and honours BOTH `disabled` and `display:none` — a gesture has no greyed state of its own to show, so it must not reach a destination the card withheld |
| ⚠️ **the capture-phase click swallow is load-bearing** | `_cardSwipeSwallowClick`, registered on `document` with `useCapture=true`. A horizontal touch drag is NOT a scroll, so the browser still synthesises a `click` on `touchend` — which would ALSO run `tapWord` (swipe from a highlighted word) or `_storyTapMaybeAdvance` (swipe from plain text). Capture on `document` is the outermost listener there is: `stopPropagation()` there means the event never reaches the target and never bubbles back. Disarms after ONE click, expires after 700ms |
| touch only, `passive:true` | a mouse drag over story text is a SELECTION (PLAN §12) and there is no desktop gesture to disambiguate from. Nothing calls `preventDefault` on the touch, so vertical scrolling is untouched |
| scope | inside `#complete-screen`, excluding `#comp-nav-modal` (whose `#comp-storyboard` is the one horizontally-scrolling strip on this screen). `APP._textExplorer` is deliberately NOT excluded, unlike the tap — `v88_ai` made that body inert as a QUESTION surface, and browsing chapters is navigation |
...
| what | where |
|---|---|
| ⚠️ **the rules moved out FIRST** | `inflection-labels.js` (new, pure — no I/O, no model, no logging): `shouldNormaliseLabels`, `buildLabelRequest`, `applyLabelReply`, `labelReplyTokens`. **Two callers**: `normaliseInflectionLabels` (server.js, generation time, inside a job) and `backfill-inflection-labels.js` (the corpus, CLI). Duplicating the per-item fallback policy across two files is exactly the drift this project keeps paying for |
| what is left in server.js | plumbing only — job step, call, parse, logging, cancel. None of it decides anything |
| the script | `backfill-inflection-labels.js`, exporting `planBackfill(store, opts)` and `applyPlan(store, results)` as PURE functions so they can be driven with no model and no disk — the same shape `repair-chapter-links.js` already uses. DRY RUN by default, `--write` (after a `.bak`), `--limit`, `--topic`, `--model` |
| ⚠️ **concurrency is real here** | the user's server writes `lessons.json` on every answered question, and a run spends MINUTES inside model calls between read and write. So: the file is **RE-READ** at write time, and each repair is located by **CONTENT** — topic id → lesson id → the item's own `sentence` + `surfaceForm` + **original `formChoices`** — never by index. An item that changed underneath is REPORTED and SKIPPED, never guessed at |
| ⚠️ `--write` RE-QUERIES the model | it does not replay what the dry run printed. The backend is non-deterministic, so what lands is equally valid but not character-identical. Read a dry run as "is this the right KIND of change", never as a diff to sign off line by line |
| verified against the `.bak` | everything except `formLabel`/`formChoices` **byte-identical** anywhere in the file; 60 inflections items before and after, 28 changed; **0** broken `formLabel === formChoices[formCorrectIndex]` invariants and **0** duplicate choice sets, before AND after; 343 topics / 97 storylines unchanged. `en→ja` lessons came back UNCHANGED — the pass is idempotent where nothing is wrong |
| ⚠️ **known limitation, NOT fixed** | terminology is consistent WITHIN a lesson but not ACROSS the corpus — one German lesson says `Präteritum`/`Präsens`, another `Vergangenheit`/`Gegenwart`. Both are correct German and both are internally coherent as a multiple-choice set, which is all `applyLabelReply` can check. Corpus-wide agreement needs a per-language glossary handed to the model — a feature, not a fix |
| the acceptance tests | `unit-inflection-label-backfill.test.js` (8 sections). §6 is the load-bearing one: a **REORDERED** lesson still repairs each item correctly — the non-vacuity for §5's staleness checks, since a positional match would silently write each repair onto the wrong item |
| ⚠️ **the mutation that stayed GREEN, third release running** | `l.type !== 'inflections'`. The fixture's other lessons were `standard` ones with no `items`, so the item filter dropped them anyway and the branches were indistinguishable. Unlike `v89_d`'s `Array.isArray` and `v89_e`'s `\|\| !APP._swipeEl`, here the GUARD was right and the FIXTURE was too weak — it grew a `word_forms` lesson carrying inflection-shaped items, contrived in this corpus and the only thing that makes "the lesson TYPE is the scope statement, not the field names" falsifiable. **Ask which of the two it is before editing either.** |
...
| what | where |
|---|---|
| ⚠️ **the first half was not the bug it looked like** | "the grammar form is now given in the target language" is not a design statement — the live corpus is genuinely MIXED. Measured: nl/de and it/nl chapters carry TARGET-language `formLabel`s (`"Tegenwoordige tijd, 3e persoon enkelvoud"`, `"imperativo presente…"`), while en/ja, de/en, it/en and en/de carry SOURCE-language ones (`"複数形"`, `"dative singular"`, `"Plural"`) — and one nl/de chapter ("Der Waldpfad") complies too. That is `roadmap_v86.md`'s **item AJ** in the data. **Reading the label with the target voice would fix one half by breaking the other** |
| the ruling | **user: the form label STAYS a source-language explanation.** `speakOkLang`/`speakBadLang` are therefore UNCHANGED (still `srcLang` for `inflection_form`); the lever is the prompt. Accepted cost, stated: chapters already holding a target-language label keep the source voice until regenerated |
| the second half | `check()`'s `speakOk`/`speakBad` no longer carry an `inflection_lemma` branch at all — the type falls into the generic `stripFuri(ex.target)` tail with `speakOkLang` null, i.e. the TARGET voice, exactly as before `v86_ae`. Both paths: the request named the ANSWER, and the wrong-answer reveal is where the correct lemma is shown |
| ⚠️ `v86_ae` is overruled, not refuted | its reasoning still holds — a voice can CLAIM a language tag and sound wrong, which `_ttsMakeUtterance`'s refuse-rather-than-approximate policy (`v55_x`) cannot detect. `v86_ae` took the user's own offered fallback; this cut is the user asking for the readout back with that trade-off known. **If the mispronunciation returns, the lever is the VOICE policy, not this branch** |
| the prompt hardening | `PROMPTS.inflections.system` (prompts.json): an explicit NEGATIVE on both fields (`NOT in {L}` / `never in {L}`), a RULE that PARTITIONS the fields (exactly `surfaceForm`/`lemma`/`lemmaChoices` are `{L}`, plus the quoted `sentence`; everything else is `{S}`) and names the trap out loud, and a RE-READ step before returning the JSON. The bare positive `{S}` is what was already being ignored |
| ⚠️ **measured against the live model — a partial mitigation, NOT a fix** | Three runs of each prompt against the user's own `qwen3.6:35b-a3b`, same real nl/de chapter, no writes: **OLD 0 of 3 runs (0/13 labels) in `{S}`; NEW 1 of 3 runs (5/15)**. It ships because it strictly improves and costs nothing, not because the drift stopped. ⚠️ **The drift is per-RUN and all-or-nothing** — every run was internally consistent, five German labels or five Dutch, never a mix, so the model picks a language for the whole item set. The lever that would settle it (NOT built): a post-parse NORMALISATION pass in `generateInflections`, sending `formLabel`+`formChoices` through the translation route — one extra call per lesson, and a transformation the model cannot skip rather than an instruction it can ignore |
| live-verified on the reporting chapter | `Naturraum für Biodiversität` (nl/de) through the running app, TTS stubbed to record, progress writes neutralised, one exercise per observation: `inflection_lemma` `"geven"` → **`nl-NL`** (silence under `v86_ae`); `inflection_form` `"Voltooid deelwoord"` → **`de-DE`**. ⚠️ The second is the user's ORIGINAL complaint and is now the RULED behaviour — a Dutch label in the German voice — and it persists on every chapter already holding a target-language label until that chapter is regenerated |
| the acceptance tests | `unit-inflection-speak-lang.test.js` §3/§4 **RE-SCOPED a THIRD time** (`v82_d` → `v86_ae` → here), now asserting count, LANGUAGE and that the text is the lemma; §5 is the non-vacuity — the two questions one item builds must resolve to DIFFERENT voices. Six mutations red. ⚠️ Its fake TTS engine now FIRES `onend`; without that, §4's auto-advance was coming from `_speakAndAdvance`'s `START_GRACE_MS` watchdog, i.e. the test measured the wedged-engine safety net, not the ordinary path |
| ⚠️ the prompt guard's honest scope | `unit-prompt-strictness.test.js`'s new inflections section (8 assertions, 8 mutations red) guards prompt TEXT — that the instruction is present and was not quietly reworded away, which is exactly what happened to this same field's worked example at `v86_ab`. It **cannot** guard model BEHAVIOUR. Re-measure the corpus; a green test is not evidence the drift stopped |
