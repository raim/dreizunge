...
**PLAN §C5 — splitting generation from the library** (`v81_v`, stage 1 of N — a NEW track, distinct
from `PLAN §C0`: real UI redesign, not naming/routing. Two user rulings shape it: language pickers
...
| what | where |
|---|---|
| **stage 1 is naming/routing prep, ZERO visual change** — same C0.1-style discipline applied to a NEW split | `goLibraryClean()`/`showLibraryClean()` added as the "home" destination, but **deliberately byte-for-byte `goLandingClean()`'s body today** — both still call `show('landing')`, because the actual screen split (stage 2: moving `#gen-area` out of `#landing` into its own `#generation-screen`, duplicating the language pickers) has not happened yet. Diverging the two bodies is what stage 2 will do; until then they MUST stay identical or "home" and "generate new" would already disagree about a page that is still one page |
| **10 callers rerouted from `showGenerationClean()` to `showLibraryClean()`** | all 7 static "🌍 home" buttons (`btn-topics`, `comp-hdr-home`, `fin-hdr-home`, `sum-hdr-home`, `us-hdr-home`, storyline-screen's, teacher-screen's) plus 3 JS stranded-learner fallbacks (`loadSaved`'s failed-auto-start branch, `compBackToStory`'s solo-chapter branch, `confirmQuit`'s solo-chapter branch). `build-static.js` has a matching copy of the `loadSaved` fallback — updated too |
| **`showGeneration()`/`showGenerationClean()` themselves are UNCHANGED** | measured before touching anything: their 3 remaining callers (continuing a storyline, starting a multi-chapter book job, starting a single-lesson job) all genuinely want the generation FORM, not the library — none needed to move. Confirm this stays true before adding a new `showGeneration*` caller: if it wants "go back to where you started, whatever that was" rather than "open the form", it probably wants `showLibraryClean()` |
| ⚠️ **the duplicated-picker decision is NOT yet built** | the user ruled "keep them synced" (one shared `APP.formLang`/`APP.libFilter` value shown on two screens) over "fully decouple" — chosen specifically because `selectLang`/`selectSrcLang` already have SIX separate historical bug-fix references (`v67.1`, `v76_i`, `v78_q`, `v79_o`, …) for exactly this kind of state coupling going wrong, and decoupling would be a real behaviour change, not a layout one. Stage 2 must wire the new picker as another MIRROR (like the existing footer selectors `src-lang-select-footer-ls`/`-sl` already are), not as an independent `fromForm=true` source |
| the acceptance test | `test/unit-ui-journeys.test.js` — the SAME three journey blocks that used to prove "the 🌍 home button" via `showGenerationClean()` (teacher, lesson-set, storyline-screen) now call `showLibraryClean()` instead, since that is what the real button calls now. `showLibraryClean()` also got its OWN direct hash-reset spy proof, mirroring `showGenerationClean()`'s — needed a SECOND global variable name for the spy's captured original (`origLib`, not `orig`): reusing `orig` across two `C.run()` calls in the same persistent vm context left the first spy's closure still pointing at the second spy after reassignment, causing infinite mutual recursion (`Maximum call stack size exceeded`) — the fix was either a distinct variable name or restoring `history.replaceState` between spies; this file does both |
